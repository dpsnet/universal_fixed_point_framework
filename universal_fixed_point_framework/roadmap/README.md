...
- **剩余开放问题**（Paper I §8.2，已更新）：
  - 非分离 IFS 下界常数 $c$ 的显式最优估计、重叠度热力学形式；
  - 高维可逆系统维数分解、拓扑熵-谱间隙普适不等式；
  - 高维 IFS 收敛率的大规模数值验证（上界紧性测试）；
  - ∞-范畴/弦图/monoidal 结构的严格证明；
  - Leaver 连分数 QNM 求解器、spin-weighted spheroidal harmonics 高精度方法；
  - 有限 $N_c$ 与强耦合下 N=4 SYM 谱方程；
  - 暗物质间接探测谱、冻结-in / 非热产生机制；
