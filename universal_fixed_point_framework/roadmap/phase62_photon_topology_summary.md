...
| 5 | 偏振红移差 κ_Δ 精确值 | 🔶 部分 | **框架内生候选 + 判别性锚定 + 选择原理 + #5×#4 交叉约束（2026-08-11，`paperX_photon_kappa_delta.py` 14/14 + `paperX_photon_kappa_select.py` 11/11 + `paperX_photon_epsilon_kappa.py` 10/10）**：自旋霍尔偏振比（太阳 ~1e-16/白矮星 ~1e-14）与预言带 $[10^{-4},10^{-2}]$ 差 10–12 量级 ⟹ P1 非重述（锚定仅判别器可剔除）；纯框架候选 $S_4^2$/ $S_4/(N_{\text{Weyl}}d_H)$/ $S_4^2N_{\text{Weyl}}/2$/ $S_4^2d_H/2$ 均在带内；选择原理：MDL 最简性 → K_a、手性配对结构匹配 → K_c，候选族收窄 4→2；交叉约束：κ_Δ≤ε_Δ 与 ε_Δ=Δ(15³) 联立 ⟹ 双候选条件性排除，收窄带新候选 S4³=Δ；精确值仍开放（依赖 ε_Δ 关系推导或远期观测） |
| 6 | 推论 4 时间解耦：树级自由传播模方守恒一致性 | 🔶 部分 | S8 `s8_free_propagation`（36/36）：C27/29 定义一致性（定义 2.4 = 标准量子光学，rel 1.3e-16）+ C28/30 树级（忽略真空修正）模方守恒 $|e^{-i\omega nt}|^2=1$；**机制层（2026-08-11）**：Fock Lean 骨架（$[N,H_0]=0$/$[N,a^\dagger]=a^\dagger$/$[N,a]=-a$）+ **JC 定量桥接（`paperX_photon_jc_bridge.py` 14/14）**：共振矩阵元/Rabi/费米黄金规则/树级 vs 机制层破缺 + **dagger 第一性原理（`paperX_photon_dagger_derivation.py` 17/17 + Lean 骨架）**：dagger-假设降级为 Hilbert 内积推论（伴随唯一性 `adjoint_unique` + 共轭转置=内积伴随 `conjTranspose_satisfies_adjoint` + R=D† 检验准则 `dagger_is_adjoint`）+ **R 态射层验证（JC 实例 `jc_R_adjoint`/`jc_R_is_dagger_adjoint`/`jc_R_hermitian`）**；**γ→∞ 未验证**；剩余：纤维丛内积全局构造、完整函子层验证开放 |
...
### 数值脚本（13 个，全部注册 `run_all_tests.py`）
...
| `scripts/paperX_photon_epsilon_kappa.py` | **10/10** | #5×#4 交叉约束：κ_Δ≤ε_Δ 与 ε_Δ=Δ(15³) 联立 ⟹ 双候选 K_a/K_c 条件性排除 + 比值 K_a/Δ=15=S4^{-1}（结构自洽）+ 收窄带新候选 S4³=Δ |
...
**注**：开放问题推进（#1/#2/#3/#4/#5/#6/#7/#8：PhotonTopology.lean + PhotonTopologyFunctor.lean 扩展至 2966/3022 jobs、S7–S9 追加、新增 fiber_orthogonality + hcdelta_dimension + jc_bridge + kappa_delta + kappa_select + dagger_derivation + curvature + p1_consistency + hcdelta_lmin + epsilon_kappa 脚本、run_all_tests 更新）与 2026-08-11 推进（#4 近-Planck 候选锚定、#5 选择原理收窄 4→2 + #5×#4 交叉约束、#6 dagger 第一性原理剔除假设 + 公理完整 Lean 化 + R 态射层 JC 实例、#7 曲率层代数/结构闭合、P1 三恒等式闭环）及 #1/#6 漏洞修正（A3 并置结构 Φ₊、谱化经 D 构造性实现 + 嵌入任意性登记、S8 重命名为树级自由传播模方守恒一致性 + γ→∞ 未验证标注）均尚未提交，待用户确认后提交。
