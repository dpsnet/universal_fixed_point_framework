...
**形式化配套**：Lean `PhotonTopology.lean`（拓扑空间定义、分岔映射、不变量定理）；Agda 镜像。
...
5. **$D \dashv R$ 无粒子假设康普顿散射**：
   ...
   - 推导：Sp 行波拓扑 → R 余伴随映射 → Rec 紧致驻波 → 拓扑扭曲 → λ/ν 偏移
...
| 阶段 | 方向 | 产出 | 状态 |
|:----:|:------|:------|:----:|
| 62A | P1 基础理论 | 研究笔记 `notes/06_photon_topology/photon_topology_theory.md` | ⏳ 待启动 |
| 62B | P1 基础理论 | 数值脚本 `scripts/paperX_photon_topology.py` | ⏳ 待启动 |
| 62C | P1 基础理论 | 论文 `paper/paper44_photon_topology.md`（自包含） | ⏳ 待启动 |
| 62D | P2 红移统一 | 研究笔记红移拓扑推导 + 数值脚本 | ⏳ 待启动 |
| 62E | P3 交叉衍生 | 研究笔记交叉效应定量化 + 数值脚本 | ⏳ 待启动 |
| 62F | P4 形式化 | Lean `PhotonTopology.lean` + Agda 镜像 | ⏳ 待启动 |
...
|:------------|:-------------|:--------|
| Rec 递归驻谱 | Paper I §Rec 范畴 | 直接引用 |
| Sp 传播行谱 | Paper I §Sp 范畴 | 直接引用 |
