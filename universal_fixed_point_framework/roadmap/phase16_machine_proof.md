...
**目标**：完成等级 B 模块，算子谱部分全部机器核验。

**任务清单**：

| 序号 | 任务 | 描述 | 状态 |
|------|------|------|------|
| 1 | Koopman 压缩半群 | $U_R = e^{-A_R t}$ 形式化 | ⏳ 待启动 |
| 2 | m-增生生成元 $A_R$ | 定义域约束、m-增生判定 | ⏳ 待启动 |
| 3 | 谱测度 Lebesgue 分解 | 离散/连续/奇异连续分离 | ⏳ 待启动 |
| 4 | S1–S4 静默判据 | 算子层面证明 | ⏳ 待启动 |
| 5 | Leaver 两弦法复杂度 | $O(N)$ 严格形式证明 | ⏳ 待启动 |
