...
→ **v1.35 已判别排除**（2026-07-29，`scripts/paperX_epsilon_hierarchy.py`，四判据全部不通过）：① **统计显著性**：失配 2.59% = **1017σ**（R = 4.557989 ± 0.000113，ε 的 3 位有效数字主导不确定度）——失配超出数据精度三个数量级，是确定性失配而非近似；② **精确形式检验**：H₁: ε = e^{−3d_H·√2π} 预测 ε_pred = 2.07×10⁻¹⁶，与 Paper II 值 8.12×10⁻¹⁷ 失配**因子 2.55**，而 ε 已知到 0.6%——精确形式被排除；③ **δ 无关性确认**：失配在 d_H 三个变体（ln15 / d_exact / d_fit）间稳定（ΔR 仅 0.054%）——失配不是"等待 δ 精确化的近似"；④ **多重比较判别**：√2π 在 27 个候选常数中仅排名**第 16**——π+√2 与 41/9 以 0.05% 失配居首（比 √2π 好 ~50 倍），且随机比值落在任一候选 2.6% 内的基线概率 = **100%**。"接近某简单常数"在此精度下是多重比较噪声，无判别力。**遗留意义**：−ln(ε) = 37.05 的结构分解仍开放，但"3·d_H × 简单常数"形式已排除，避免未来重复探索。

→ **v1.36 追加判别**（2026-07-29，`scripts/paperX_epsilon_hierarchy.py` S6）：对失配本身提出的候选结构式 **2¹×(15−1)×10⁻³ = 2.8%** 的检验——❌ **舍入伪影的拟合**：① 候选与文档失配值 2.785% 的 0.53% 吻合只对 **−ln ε ≈ 37.1 的舍入值**成立；对精确失配 2.646% ± 0.002% 失配 **5.83%（90σ）**，排除为精确关系；② 同族表达式 k₁×(15−k₂)×10⁻ᵏ 过密——2×(15−2)×10⁻³ = 2.600% 对精确失配的吻合（1.7%）反而优于候选（5.8%），选择 2.800% 无判别依据；③ 父猜想（√2π）已被排除，对其失配项再赋结构是二阶数值拟合。结论与 v1.35 一致：该方向无结构关系。
