...
### 7.9 Cl(1,7) 代数选择第一性推导（2026-08-08，③ 先验导出推进）

**动机**：假设-断言分类账（`notes/00_foundations/spectral_hypothesis_deductive_methodology.md` §4）中 `hNorm`（Cl(1,7) 归一化）为**物理模型断言**（框架输入），③ 先验导出（范畴层纯结构直接导出谱结构）未完成。本小节把 Cl(1,7) **代数结构**（M₁₆(ℝ) 同构、旋量 16、签名 (1,7)）从"查表/勘误"升级为**构造性定理**（`scripts/paperX_cl17_first_principle.py` 7/7 注册 `run_all_tests.py`）——代数选择的第一性 = 8 生成元（k_max 机器证明）× 洛伦兹签名（1 时间）⟹ 代数唯一确定。

**第一性链（F1→F7）**：

| # | 环节 | 依据 | 状态 |
|:--:|:--|:--|:--:|
| F1 | 生成元数 = k_max = 8 | 统一 3 定理（N_active = 3 → 2³ = 8，`Unified3Theorem.lean` 机器证明） | ✅ |
| F2 | dim Cl(1,7) = 2⁸ = 256 = dim M₁₆(ℝ) | Clifford 代数维数公式 | ✅ |
| F3 | Cl(1,1) ≅ M₂(ℝ) | 显式构造（γ₀=σ₃²=+I、γ₁=σ₂²=−I，反交换，张成 4 维） | ✅ |
| F4 | Cl(0,6) 复构造 + Cl(0,7) 体积元 | 标准递推 γ_{2j−1}=σ₃^{⊗(j−1)}⊗(iσ₁)⊗I、γ_{2j}=σ₃^{⊗(j−1)}⊗(iσ₂)⊗I（iσ₂ 实）；γ'₇=体积元 ω（ω²=−I、全反交换） | ✅ |
| F5 | Cl(1,7) ≅ M₁₆(ℝ) | Γ⁰=σ₃⊗I₈（+I 时间）、Γⁱ=σ₁⊗γ'ᵢ（−I 空间），8 生成元 16×16 复反交换 + 平方正确，**256 个 Clifford 单词秩 = 256** ⟹ 复化 = M₁₆(ℂ)；16 实 Majorana 忠实模（Cl₀(1,7) = Cl(1,6) = M₂(ℍ)⊕M₂(ℍ) 不可约模 ℍ²⊕ℍ² = 16 实）⟹ 实代数唯一 = M₁₆(ℝ)（M₈(ℍ) 最小忠实实模 = ℍ⁸ = 32 实，被排除） | ✅ |
| F6 | 签名唯一性 + 时间维来源 | p+q = 8 中 (1,7) **唯一**洛伦兹签名（时间 = 1）；**时间方向 = c₃ 分支**（IFS 递归根基，静默因子 = 1 永不静默，谱流参数 t 沿此演化——`spectral_zero_parameter_derivation.md` §7.3，权重排序 S₃S₄ < S₄ < 1 机器证明）⟹ "1 时间"为框架内部结构非外部输入；Majorana 判据：Cl(1,7) ≅ Cl(7,1) ≅ M₁₆(ℝ)（16 实）；欧氏 (0,8) 未分次同构但 Z₂ 分次不同；(2,6)/(3,5) 等 = M₈(ℍ) | ✅ |
| F7 | 对偶网络复核 | 旋量 16 = 2·k_max、B = 15 = 2·8−1、d_H = ln15、D=10 衔接 N_tr = 8 = k_max、α₀ = 8/16 = 1/2（v0.17 复核） | ✅ |

**构造性同构链**：Cl(1,7) = Cl(1,1) ⊗̂ Cl(0,6) = M₂(ℝ) ⊗̂ ℝ(8) ⟹ M₁₆(ℝ)（超张量积矩阵放大）；Cl(0,6) = ℝ(8)（p−q ≡ 2 mod 8 → ℝ 类，铁证校准：Cl(0,2) = ℍ 为 ≡ 6 → ℍ 类）。旋量 16 = 2^⌊8/2⌋ = 2·k_max（D1 对偶构造性来源）。

**诚实边界**：
1. **时间方向已由 c₃ 分支确定**（2026-08-08 确认）：c₃ 静默因子 = 1（永不静默的递归根基分支），谱流参数 t 沿此演化 = 时间维度（`spectral_zero_parameter_derivation.md` §7.3；权重排序 S₃S₄ < S₄ < 1 机器证明）——"1 时间"不再依赖 A_GR 谱外部输入。剩余诠释层：c₃ 分支的"时间"是**动力学时间**（递归演化方向），其与 Cl(1,7) γ₀（平方 +I 生成元）的对应为物理论证；静默因子 S₃ = e⁻³（统一 3 定理）、S₄ = e⁻d_H（d_H = ln15 机器证明）均已推导
2. M₁₆(ℝ) 同构、旋量 16、B/d_H/对偶网络为**构造性数学事实**（脚本可复现），不再属"查表/勘误"；Cl(1,7) 代数选择由"k_max（已证）× 时间维（c₃ 分支，已证）"⟹ 结构唯一确定——**s = e⁻¹ 已获范畴层独立推导 + Lean 机器证明（2026-08-08）**：Moran 方程 15·s^{ln15} = 1 + d_H = ln15/B = 15（机器证明）⟹ s = e⁻¹（纯代数封闭，`paperX_s_categorical_time.py` 9/9；`CoherenceToBranching.lean §10a` moran_closed_*），信息论降级为佐证；**c₃ 时间方向 Lean 形式化完成（§10b）**：`c3_unique_silent_factor`/`c3_silent_factor_unique`/`c3_silent_factor_exists`（唯一永不静默分支 ⟹ 时间维数 = 1，lake build 2454 jobs 通过），T3–T5 对应论证为物理论证层。③ 剩余开放方向："递归演化 = 时间"的 $\mathbf{Sp}$ 纯范畴形式化（$\mathbf{Sp}$ 内时间方向定义）

**交叉引用**：假设-断言分类账（hNorm 状态更新）、paper33 Bott 塔、paper20 §5.4、k_max 对偶网络（§7.8）；roadmap phase61 v0.32/v0.33/v0.34/v0.35。
