...
**Step 4（三维观测的投影效应）**：三维观测者测量到的"处处指向球心"是 $W$ 方向引力线垂直穿透三维层时的投影（**诠释辅助：$W$ 的严格实现为谱模式正交（Paper XXXI §3.3 J2）与纤维丛层 $V\perp H$，见 5.7k.5 T1 登记**）——$W$ 方向与 $XYZ$ 完全正交，投影后表现为从各方向汇聚到球心 $O$。
...
- **T1 张力对齐（2026-08-12，paper35 v0.5 / paper44 v0.14）**：本节及 paper35 正文 §3.2 的 $W$ 轴语言为**诠释辅助**（几何直觉）；"正交"的严格实现为谱模式正交（Paper XXXI §3.3 J2，机器证明）与纤维丛层 $V\perp H$（Paper XLIV §7.2）——$W$ **非几何额外空间维度**（与 KK 的区分见 5.7k.5 上条）。paper35 正文 §3.2.1 Step 3/Step 4 已加"诠释辅助"注、§3.2.3 收敛表加诚实标注；paper44 §7.2 诚实边界第 1 条补"与 Paper XXXV §3.2 的一致性"交叉引用。完整对齐方案与实施记录见笔记方向 2 §3.6（T1 已对齐，v0.68）
