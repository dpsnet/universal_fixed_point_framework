...
2. **正常序常数（截距）**：Virasoro 正常序常数 $a = -\tfrac{D-2}{2}\cdot[\sum n - \sum(r+\tfrac{1}{2})]$——玻色 $a_B(D) = \tfrac{D-2}{24}$（D=26 → 1）、**超弦 NS $a_{NS}(D) = \tfrac{D-2}{16}$**（D=10 → 1/2）、R 扇区 $a_R = 0$。
3. **临界维数（量子自洽）**：中心荷消去固定超弦临界维数 $D = 10$ → $\alpha_0 = a_{NS}(10) = \tfrac{10-2}{16} = \tfrac{1}{2}$。
