...
**k_max = 8 的正确来源**：不是 Cl(1,7) 表示维数（16），而是 **Bott 塔截断 = 统一 3 定理**（paper33 机器证明）——严格 4-范畴的主动生成层数 N_active = 3（1/2/3-态射）经 Bott 塔翻倍指数给 $k_{\max} = 2^{N_{\text{active}}} = 2^3 = 8$（引理 3，`BottTower.lean`），并由对偶网络佐证（旋量 16 = 2·k_max、分支 B = 15 = 2·k_max−1、d_H = ln(2·k_max−1) = ln15，`paperX_kmax_duality.py` 10/10）——**结构确定量，非模型输入**（勘误 v0.21 升级；ρ_c 扫描 {4,6,8,16,100} 保留为交叉验证）。【2026-08-07 勘误：原"由于 Cl(1,7) 的表示维数为 8，有 k_max=8"错误——旋量 16 维，k_max=8 来自 Bott 塔翻倍（v0.21 起为结构确定，非模型选择）】
