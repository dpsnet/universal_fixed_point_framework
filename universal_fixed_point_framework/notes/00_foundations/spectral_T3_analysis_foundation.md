...
**A. 实质技术债（可闭合，未闭合）**：
1. **fc-poly-le-spec-int 构造化**（fc-integral 降定理唯一剩余登记项）——需 ∫f⁺−∫f⁻ 语义重构（**方案 A 已批准启动（2026-08-03，§5.16.8）**：正负分解重构 4 阶段路线；**阶段 1 ✅ v1.22**（max-ℝ 族 + f⁺/f⁻ 定义与逐点性质，零新增公理）；**阶段 2 第一部分 ✅ v1.23**（Op 减法 `_−ₒ_` + op-sub-comm + spec-int-general-decomp 桥接）；**阶段 2 第二部分第一步 ✅ v1.24**（·ₒ-zero-r + op-sub-zero-r + spec-int-nonneg + spec-int-general-zero + CrossLayer 14 组证书）；待阶段 2 第二部分第二步（spec-int-general 定义重构 + 下游适配））。
