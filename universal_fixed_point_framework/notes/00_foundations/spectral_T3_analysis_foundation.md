...
**关键设计决策（数值规模）**：65/24 与 ln15 的相对间隙 ~1e-4，任何有理比较需分母 ~1e5（对比 e<3 的 288 可手算）。`_*ℕ_` 定义性归一化成本 ~m·n，1e9-1e11 的交叉乘积使 Agda 检查超时（实测挂起），故纯有理比较 `ln15-arith-ax` 按 **账目开放项**（scoped 数值公理）登记——它不含 log 内容（纯有理算术），本质是**工程计算资源不足**（框架归一化能力不可达，非结构性；标准分析中可计算验证：4·0.69317 - 29/450 = 2.77268 - 0.064444 ≈ 2.7082356 < 65/24 ≈ 2.7083333），与 S0 表示静默（语义结构性不可闭合）不同。`ln2-lt` 为 log 级数内容（定义性公理，待阶段 3+ 级数机制实现为可证明定理），同为账目开放项；`ln1615-lb` **已于 2026-08-05 经 T3 级数机制（exp-partial-at + exp-tail-bound 桥接公理）闭合为可证明定理**。
...
**A. 实质技术债（可闭合，未闭合）**：
1. **fc-poly-le-spec-int 构造化**（fc-integral 降定理唯一剩余登记项）——需 ∫f⁺−∫f⁻ 语义重构（**方案 A 已批准启动（2026-08-03，§5.16.8）**：正负分解重构 4 阶段路线；**阶段 1 ✅ v1.22**（max-ℝ 族 + f⁺/f⁻ 定义与逐点性质，零新增公理）；**阶段 2 第一部分 ✅ v1.23**（Op 减法 `_−ₒ_` + op-sub-comm + spec-int-general-decomp 桥接）；**阶段 2 第二部分第一步 ✅ v1.24**（·ₒ-zero-r + op-sub-zero-r + spec-int-nonneg + spec-int-general-zero + CrossLayer 14 组证书）；**阶段 2 第二部分第二步 ✅ v1.25**（非负一致性：spec-int-general-ext-pt + pos-part-absorp/neg-part-zero-point + spec-int-nonneg-consistent——定义重构改走 decomp 显式化，见 §5.16.8）；**阶段 3 ✅ v1.26-27**（钉住解析：spec-int-nonneg-exp/-phi-t + id 完全解析 spec-int-general-id-pos/零贡献桥接——阶段 3 收官）；**阶段 4 第一/二步 ✅ v1.28**（fc 侧分解：fn-sub + fc-sub + fc-decomp-pos-neg）；**阶段 4 余项第一步 ✅ v1.29**（dyadic 网格 ℝ 基础：natℝ-zero/nonneg + div-zero-l/div-nonneg + grid-pt/grid-pt-nonneg）；**阶段 4 余项第二步第一部分 ✅ v1.30**（SimpleF 阶梯 disj 侧：grid-pt-suc（网格严格递增）+ dyadic-Ω（区间 [xⱼ,xⱼ₊₁)）+ ℕ 严格序工具（<-ℕ-trich/split/suc-split）+ fin-to-nat-trich（Fin 三分律）+ grid-pt-lt（网格严格单调）+ dyadic-disj-lt/dyadic-disj（区间不相交 Ωᵢ∩Ωⱼ=∅，SimpleF.disj 核心，零新增公理）；**阶段 4 余项第二步第二部分 ✅ v1.31**（SimpleF dyadic 阶梯实例组装：_≤ℕ_/≤-ℕ-suc-le/Fin-<ℕ（可证）+ grid-pt-last（x_{2^k}=c）+ grid-pt-upper（xⱼ≤c）+ dyadic-cover 桥接登记（实数划分定理，archimedean-ub 同层 ℝ 完备性族标准推论）+ dyadic-Ω3（三段式 Ω：负部/dyadic/正部）+ dyadic-disj3（三段式不相交）+ dyadic-cover3（全空间覆盖）+ dyadic-stair（SimpleF 实例，值函数参数化）——**阶段 4 余项第二步（SimpleF 阶梯构造）闭合**）；**阶段 4 余项第三步第一部分 ✅ v1.32**（上界 ∫sₖ ≤ₒ ∫p⁺：dyadic-vc（单项式阶梯值：负部/正部 0、dyadic xᵢⁿ）+ dyadic-vc-le（xᵢⁿ≤xⁿ，power-mono）+ dyadic-stair-below（s≤p⁺ 逐点）+ dyadic-below-member（spec-int-below 成员）+ dyadic-int-below（∫sₖ≤ₒ∫p⁺，sup-op-upper）——阶梯逼近侧基础闭合）；**阶段 4 余项第三步第二部分 ✅ v1.33**（MCT：stair-seq（dyadic 阶梯序列族 sₖ = stair k (k+1)）+ stair-seq-le（supₖ∫sₖ≤ₒ∫p⁺，可证）+ stair-MCT 桥接（∫p⁺≤ₒsupₖ∫sₖ，Lebesgue 单调收敛算子序形式）+ stair-int-full（∫p⁺=supₖ∫sₖ）——收敛机制闭合）；**阶段 4 收官 ✅ v1.34**（fc-poly-le-spec-int 依赖循环解决：≤ₒ-refl 登记 + fc-poly-le-spec-int 由 postulate 降为可证定理（经 fc-integral §5c + ≤ₒ-refl）；循环分析——fc(p⁺)≤ₒ∫p⁺ 经 fc-continuous 引用 fc-poly-le-spec-int 自循环（结构性，p⁺ 非多项式侧唯一工具是 fc-continuous/fc-integral），改用更基础 fc-integral（fc = ∫，谱定理函数演算）直接降——**方案 A 收官，技术债 A1 闭合**，fc 侧唯一剩余 D 类 = fc-integral（健全））；待 fc-integral 完整降为定理（测度论完整层，v1.29-1.33 基础设施备用）。
...
4. **跨层完整实例化**（谱对象映射 A/E/fc/exp-tA ↦ Hilbert 构造）——**谱对象映射完整 ✅（2026-08-03，v1.21 E/exp-tA + v1.36 A/fc）**：CrossLayer §2 谱对象映射证书 `SpectralObjPt`（E P ↦ E-hilb P、exp-tA t ↦ exp-hilb-tA t 的性质保持——10 字段：E-idem-pt/E-orth-ip/E-total-pt/E-self-adjoint/E-norm-le-one/E-union-pt + exp-tA-semigroup-pt/zero-pt/self-adjoint/contractive，E 族零新增公理、exp-tA 族 §12 桥接字段；**v1.36 扩展 A/fc 字段**：A-self-adjoint-hilb（A ↦ 自伴算子，HilbertSpace §12' A-hilb）+ A-comm-E-hilb-pt（A 与谱投影交换，M-Sp/M-σ 的 Hilbert 侧）+ fc-hilb-id-A-pt（恒等函数演算 = A）+ fc-hilb-exp-tA-pt（指数函数演算 = 半群）——**谱定理降定理链端点桥接**（A-hilb/fc-hilb 与 spectral-subspace/exp-hilb-tA 同层，链体 = 自伴算子谱定理证明文档化为降定理路径，方案② 用户决策）——**项 4 谱对象映射完整闭合**）。**剩余（结构性）**：算子层等式版公理 + 对象映射 op-lin 等式保结构（需 funext，P4 先例）。
...
- **阶段 2 第一部分 ✅（v1.23）**：SpectralTheory §1b'' Op 层减法 `_−ₒ_`（X −ₒ Y := X +ₒ ((−1)·ₒ Y)，定义性）+ **可证** `op-sub-comm`（减法保交换：X 与 Y、Z 交换 ⟹ X 与 Y−ₒZ 交换，distribₒ + ·ₒ-comm-l/·ₒ-comm + distribₒ-l 逐项——X-comm-spec-int-general 重构后重验核心）+ **桥接登记** `spec-int-general-decomp`（∫f dE ≡ ∫f⁺ dE −ₒ ∫f⁻ dE，模型必然性 = 测度论线性，降定理路径 = 定义重构后可证）。
- **阶段 2 第二部分第一步 ✅（v1.24，一致性组件）**：P1Spectral 补充公理 `·ₒ-zero-r`（a·ₒ𝟘ₒ = 𝟘ₒ，标量乘零算子，G 类算子代数补充）+ HilbertSpace **可证** `·ₗ-zero-r-pt`（(c·ₗ𝟘ₗ)v = 𝟘ₗ v，scalar-zero——CrossLayer 证书补第 14 组点态字段，OpAlgPt 14 组公理完整）+ SpectralTheory **可证** `op-sub-zero-r`（X −ₒ 𝟘ₒ = X，·ₒ-zero-r + +ₒ-ident）+ `spec-int-nonneg`（非负积分别名 = sup-op (spec-int-below)，重构定义避免递归）+ **桥接登记** `spec-int-general-zero`（∫0 dE = 𝟘ₒ，D 类，模型必然性 = 测度论零函数性，降定理路径 = Hilbert 层谱投影非负 + 标量保序 + sup-least）。
- **待（阶段 2 第二部分第二步）**：spec-int-general **定义重构**（∫f := spec-int-nonneg f⁺ −ₒ spec-int-nonneg f⁻）+ 下游全适配（X-comm-spec-int-general 用 op-sub-comm 重写、spec-int-R-trunc-conv/-ℕ-conv 陈述调整、fc-integral 系列重验）+ 非负一致性组装（spec-int-general-decomp + op-sub-zero-r + spec-int-general-zero）；阶段 3（钉住桥接转定理）。

**定义重构评估（2026-08-03，v1.25 决策）**：spec-int-general **定义重构破坏面确认过大**——下游 MCT 定理（spec-int-R-trunc-conv/-ℕ-conv、trunc-below-general）与 fc-integral 系列（-le/-ge/-full）全部依赖 `spec-int-general f = sup-op (spec-int-below f)` 定义性，重构需连带新增 ≤ₒ 减法单调性桥接 + 多个定理陈述改写（多轮工程）。**改走"decomp 显式化"路线（不重构定义）**：保留 `spec-int-general f = sup-op (spec-int-below f)` 定义，用 `spec-int-general-decomp` 桥接（∫f = ∫f⁺ −ₒ ∫f⁻，v1.23 已登记）+ 非负一致性把 ∫f 归约为非负 sup（f⁺/f⁻ 下界族非空，构造上有效）——"消除钉住"通过阶段 3（钉住桥接转定理）实现，破坏面为零。

- **阶段 2 第二部分第二步 ✅（v1.25，非负一致性闭合）**：`spec-int-general-ext-pt`（spec-int-general 逐点外延：f ≡ g 逐点 ⟹ ∫f = ∫g，spec-int-below-mono 双向 + sup-op-ext，**避开 funext**——sup-op-ext 自 §1c 移至 §1b 消解前向引用）+ `pos-part-absorp`（f x ≥ 0 ⟹ f⁺ x = f x，三分律）+ `neg-part-zero-point`（f x ≥ 0 ⟹ f⁻ x = 0，neg-≤-ℝ + neg-zero）⟹ **`spec-int-nonneg-consistent`**（**f ≥ 0 ⟹ ∫f ≡ 非负 sup**：decomp + 吸收引理（逐点外延）+ spec-int-general-zero + op-sub-zero-r）——**非负函数的 ∫f 即朴素下界族 sup，钉住语义与非负 sup 无分歧**（非负一致性闭合，为阶段 3 钉住桥接转定理铺路）。
...
- **阶段 4 余项第一步 ✅（v1.29，dyadic 网格 ℝ 基础）**：DHStructural `natℝ-zero`（**可证**：natℝ 0 = 0——natℝ-suc zero + natℝ-one + 加消去（add-neg-cancel + 交换））+ `natℝ-nonneg`（**可证**：0 ≤ natℝ j）+ `div-zero-l`（基础假设登记：0/d = 0，与 /-pos-ℝ 同层）+ `div-nonneg`（**可证**：0 ≤ a 且 0 < d ⟹ 0 ≤ a/d，三分律三分）⟹ SpectralTheory `grid-pt`（dyadic 网格点 xⱼ = (j·c)/2^k）+ `grid-pt-nonneg`（**可证**：0 ≤ c ⟹ 0 ≤ xⱼ，natℝ-nonneg + *-nonneg-ℝ + div-nonneg）——**dyadic 阶梯逼近的网格基础**（阶段 4 余项路线：网格 → SimpleF 阶梯构造 → 上界 ∫sₖ ≤ Aⁿ → MCT → fc-poly-le-spec-int 零登记项）。排坑：div-nonneg 的 subst 方向（0=a 非 sym）。
