...
> **论题 R1**（资源 = 谱结构）。每种量子资源（相干性、纠缠、魔力、失谐）对应 $\mathbf{Spec}$ 对象的一个**谱不变量**。资源测度是 $\mathbf{Spec}$ 上的函子 $R: \mathbf{Spec} \to \mathbb{R}_{\ge 0}$，在自由操作下非增。
> **自由操作** = $\mathbf{Spec}$ 中保持该谱不变量不变的态射。
...
| 资源理论要素 | 标准定义 | $\mathbf{Spec}$ 翻译 |
|------------|---------|-------------------|
| **资源态** | 具有非零资源量的量子态 $A$ | $\mathbf{Spec}$ 对象 $E = (\mathcal{H}, A, \sigma(A))$ |
| **自由操作** | 不增加资源的操作 $\Lambda$ | $\mathbf{Spec}$ 态射 $T: E \to E'$ 满足 $R(T(A)) \le R(A)$ |
| **资源测度** | 单调非增函数 $R(\rho)$ | 函子 $R: \mathbf{Spec} \to \mathbb{R}_{\ge 0}$ 满足 $R(T(A)) \le R(A)$ |

**定义 R1**（资源函子）。设 $\mathcal{C} \subseteq \mathbf{Spec}$ 为资源理论定义的全子范畴。**资源函子** $R: \mathcal{C} \to \mathbb{R}_{\ge 0}$ 是从 $\mathcal{C}$ 到偏序范畴 $(\mathbb{R}_{\ge 0}, \le)$ 的函子，满足：
...
在 $\mathbf{Spec}$ 中，自由操作 $T: E \to E'$ 必须满足谱交织条件 $T A \subseteq A' T$。不同的资源理论选择不同的 $\mathcal{C}$ 和不同的态射约束：
...
**定理 R2**（资源转化由谱流实现）。设 $A_1, A_2 \in \mathbf{Spec}$ 为两个资源态。存在谱流从 $A_1$ 到 $A_2$ 当且仅当存在生成元 $G$ 和 $t \ge 0$ 使得：
$$A_2 = e^{-tG} A_1 e^{tG}.$$
...
| 热平衡态 | 自由态 | $R(A) = 0$ 的 $\mathbf{Spec}$ 对象 |
| 熵增 | 资源衰减 | $R(A_t) \le R(A_0)$ (谱流单调性) |
| 卡诺效率 | 资源转化效率 | $\eta = (R(A_0)-R(A_t))/R(A_0)$ |

**论题 R2**（资源理论 = 谱热力学）。量子资源理论是谱热力学（Paper VII）在 $\mathbf{Spec}$ 范畴中的推广——不同的资源测度对应不同的"谱势函数"，自由操作对应保持该势函数的态射。
