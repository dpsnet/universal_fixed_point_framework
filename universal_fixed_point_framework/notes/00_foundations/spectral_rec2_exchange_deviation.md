...
> **状态**：研究笔记 v0.10（2026-08-05）。数学推导完成，数值验证 **18/18 检查通过** + 结构性诊断（D7/D8 结合律失败、D9 拉回 2-态射空间稀疏性、**T17 修正（原负结果为扫描设计缺陷假象）**、**D10 Fredholm 可解性刻画**），**选定路径 B（D-拉回）**，**Lean 实现完成**（`HigherRecCategory.lean` 重写，零 `sorry`，`lake build` 通过）；开放问题 7 **部分闭合**（§4.4 定理 12 时间无关子类）、开放问题 8 **完全闭合**（§4.4 定理 11 可对角化 + 命题 13 迹障碍 + 例 14 T19 正面例 + **命题 15 Fredholm 准则**，v0.10）。
...
> **数值（T17/T18/T19/D10，v0.9-v0.10）**：**T17 更正**——v0.8 的随机扫描用 `rand_map`（恒生成置换=双射），300 次尝试全部被"跳过双射"分支过滤，扫描实为空 ⟹ 原"负结果"是扫描设计缺陷的假象；修正为非双射随机函数后立即发现非平凡解（残差 $=0$，第 2 次尝试）。**T18** 确认迹障碍（$\#\operatorname{Fix}(s^2) = 2 \neq \#\operatorname{Fix}(s) = 0$ ⟹ $(s,s^2)$ 不可解）。**T19（正面例）**：等迹缺陷情形存在非平凡解——步进 $s: 0\mapsto1, 1\mapsto2, 2\mapsto2$（非双射、缺陷），$f = s$、$g = s^2$，$\#\operatorname{Fix}(s) = \#\operatorname{Fix}(s^2) = 1$（等迹，迹障碍不排除），显式 $H = \begin{bmatrix} 1 & 0 & -1 \\ 0 & 0 & 0 \\ 0 & 0 & 0 \end{bmatrix}$ 精确满足 $A H - H A = A^2 - A$（残差 $< 10^{-8}$）。**D10（全枚举 21 个非双射 $s$ × 6 幂对 = 126 对）**：78 对可解（全部 $f \neq g$ 非平凡）、24 对等迹但不可解（**迹条件不足**）、非等迹可解 $= 0$（迹障碍严格）、Fredholm 正交条件 $C \perp \ker(L^*)$ 与可解性**完全吻合**（违反数 0）。**结论**：可解 ⟺ $C \perp \ker(L^*)$（命题 15）；可对角化步进 ⟺ $f = g$（定理 11）；缺陷步进 + 非等迹 ⟹ 空（命题 13）；缺陷步进 + 等迹 ⟹ 可解**当且仅当**额外正交条件满足。

**例 14（T19，等迹缺陷非平凡解，v0.9）**。$X = Y$（$n = 3$），步进 $s: 0\mapsto1, 1\mapsto2, 2\mapsto2$（非双射 ⟹ $A = \mathrm{stepMatrix}(s)$ 缺陷），$f = s$、$g = s^2$。则
$$\#\operatorname{Fix}(s) = 1 = \#\operatorname{Fix}(s^2) \quad (\text{等迹，命题 13 不排除}),$$
且 $H = \begin{bmatrix} 1 & 0 & -1 \\ 0 & 0 & 0 \\ 0 & 0 & 0 \end{bmatrix}$ 满足 $A H - H A = A^2 - A = T_g - T_f$（直接代入，数值残差 $<10^{-8}$）。故 $\mathrm{Hom}^{PB}(s, s^2) \neq \varnothing$ 且 $f \neq g$——**非空性 $\iff f = g$ 在缺陷等迹情形下不成立**。

**命题 15（Fredholm 准则，Sylvester 可解性的完整刻画，v0.10）**。设 $L(H) := A_X H - H A_Y$（$\mathbb C^{n \times m} \to \mathbb C^{n \times m}$），$L^*$ 为其 Frobenius 伴随，$L^*(Y) = A_X^H Y - Y A_Y^H$。则
$$\mathrm{Hom}^{PB}(f,g) \neq \varnothing \iff T_g - T_f \in \mathrm{range}(L) \iff T_g - T_f \perp \ker(L^*).$$
当 $X = Y$（$A_X = A_Y =: A$）时，$\ker(L^*) = \{Y : A^H Y = Y A^H\}$ 即 $A^H$ 的**中央化子**，恒含单位阵 $I$（维数 $\geq 1$）；迹条件 $\operatorname{tr}(T_g - T_f) = 0$ 恰是 $C \perp I$ 这一个正交分量。**缺陷时 $\dim \ker(L^*) > 1$（D10 实测 $\in \{3, 5\}$），等迹不再充分**——还需 $C$ 与中央化子其余基元正交。

*证*。有限维 Fredholm 交替定理：$\mathrm{range}(L) = (\ker L^*)^{\perp}$；迹条件 $\operatorname{tr}(C) = \langle C, I \rangle$（Frobenius 内积），$I \in \ker(L^*)$。□

**例 16（D10 数值验证，v0.10）**。$n = 3$ 全枚举 21 个非双射 $s$ × 6 幂对 $(s^k, s^m)$（$k \neq m$，$f = s^k$、$g = s^m$）= 126 对：78 对可解（全部 $f \neq g$，非平凡率 $62\%$）、24 对等迹但不可解（迹条件满足、中央化子其余分量不满足）、非等迹可解 $= 0$（迹障碍严格）、Fredholm 正交判定与 lstsq 残差判定**零违反**。$\ker(L^*)$ 维数在 $\{3, 5\}$ 之间变化（随亏损块结构），证明"等迹 ⟹ 可解"在缺陷情形**一般失败**，需命题 15 的完整正交条件。

> **理论意义（v0.10 更新）**：T17 的 v0.8"负结果"是扫描设计缺陷的假象（`rand_map` 恒生成置换 ⟹ 全部被过滤）；真实图景：缺陷步进下非平凡 2-态射**普遍存在**（D10 62% 幂对可解），可解性由命题 15 的 Fredholm 正交条件**完全刻画**（含迹条件作为其一维分量）。这推翻了早前"非空性 $\iff f = g$ 数值稳健"的结论——该结论仅对**可对角化步进**（定理 11）成立。物理诠释：异源 2-态射在缺陷（不可约耗散）动力学下丰富存在，中央化子正交条件即"谱选择规则"；这为 §5 交换律偏差提供异源 2-态射上的非平凡载体族（78 个数值实例）。
...
8. **D-拉回 2-态射空间稀疏性（2026-08-05，✅ 完全闭合）**：转移矩阵恒有特征值 1（$\mathbf 1$ 为特征向量），故 Sylvester 方程 $A_X H - H A_Y = T_g - T_f$ 对一般 $f \neq g$ 罕见可解，但**精确判据已建立**：
   - **可对角化步进（定理 11）**：$\mathrm{Hom}^{PB}(f,g) \neq \varnothing \iff f = g$（$L$ 半单 ⟹ $\ker L \cap \mathrm{range}\,L = \{0\}$）；
   - **一般步进（命题 15，Fredholm）**：$\mathrm{Hom}^{PB}(f,g) \neq \varnothing \iff T_g - T_f \perp \ker(L^*)$——迹条件（命题 13）是其 $\dim \ker(L^*) - 1$ 个正交分量之一；缺陷时 $\dim \ker(L^*) > 1$，等迹不充分；
   - **数值全枚举（例 16，D10）**：$n=3$ 的 21 个非双射 $s$ × 6 幂对 = 126 对，78 对可解（全非平凡）、24 对等迹不可解（迹条件不足的证据）、非等迹可解 0（迹障碍严格）、Fredholm 判定零违反；
   - **T17 更正**：v0.8 的"300 次扫描负结果"为扫描设计缺陷假象（`rand_map` 恒生成置换，全被过滤）；修正后立即发现非平凡解。
   物理诠释更新：缺陷（不可约耗散）动力学下异源 2-态射丰富存在（62% 幂对），中央化子正交条件 = 谱选择规则；"Rec₂ 本质为同源自交结构"的表述仅对可对角化步进成立。
