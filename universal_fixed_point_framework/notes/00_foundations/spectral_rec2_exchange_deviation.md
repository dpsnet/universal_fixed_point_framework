...
> **与原 ℕ-指标结构的联系（待推导）**：原 flow-diagonal 条件 $\alpha(n{+}1)[x,g(x)] = \alpha(n)[x,f(x)]$ 可视作 homotopy $H$ 的"流分解"（$H$ 沿流动对角的谱流分辨），精确对应（$H$ ↔ α(n) 的特定极限/投影）为开放问题（§7 新增）。拉回定义的交换律偏差（引力）与 §5 的偏差分解在 Sp₂ 侧已统一。
