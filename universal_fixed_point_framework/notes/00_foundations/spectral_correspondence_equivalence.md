# 谱对应 $ \lambda_i = e^{-\mu_i}$ 的范畴自然同构表述

> 本文档目标：将分形谱化理论的核心等式 $ \lambda_i = e^{-\mu_i}$ 从数值等式升级为严格的范畴自然同构。这是「数学语义学转向」的关键一步，也是回应「拟合尝试」质疑的理论基础。
...
> $ \lambda_i = e^{-\mu_i}$ 不是人为构造的等式，而是两个范畴函子之间的**自然同构（natural equivalence）**。
...
若 $\eta$ 是每个分量上的同构（双射），则 $\eta$ 是**自然同构**，记作 $M \cong L$。
...
设 $f: R_1 \to R_2$。由谱化函子 $D(f): D(R_1) \to D(R_2)$ 满足谱交织条件
...
由 $M(f)$ 与 $L(f)$ 均由同一个谱化函子 $D(f)$ 诱导，且 $\eta$ 仅是指数重参数化，二者相容。形式化地：
...
由于每个 $\eta_R$ 都是双射，自然变换 $\eta: M \Longrightarrow L$ 是**自然同构**。记作
...
在 `src/spectral_correspondence.py` 中，上述自然同构被实现为可验证的数值工具：
...
自然同构 $\eta: M \cong L$ 表明：

> 压缩分形谱（递归描述）与算子半群谱（谱化描述）不是两套独立的数据，而是同一函子的两种等价表示。

这为「递归系统可通过算子半群实现谱化」提供了范畴论层面的正当性。

### 6.2 对 SM 实例的影响

在标准模型实例中，$ \lambda_i = e^{-\mu_i}$ 不再是一个需要数值拟合的等式，而是自然同构 $\eta$ 在 Cl(1,7) 低能实例下的分量。质量谱的指数形式 $m_{s,k} \propto e^{-(k-1)\beta_s z_s \eta_s}$ 是这一自然同构在离散谱上的具体显现。
...
若希望更抽象化，可将 $\mathbf{Rec}$ 与 $\mathbf{Sp}$ 本身视为等价范畴。即构造函子

$$D: \mathbf{Rec} \longrightarrow \mathbf{Sp}$$

并寻找其拟逆（quasi-inverse）$C: \mathbf{Sp} \to \mathbf{Rec}$，使得

$$C \circ D \cong \mathrm{id}_{\mathbf{Rec}}, \quad D \circ C \cong \mathrm{id}_{\mathbf{Sp}}.$$

此时 $D$ 是**范畴等价**，递归系统与谱空间在范畴论意义上是「同一结构」的两种表述。

### 7.2 当前可行性

完全证明 $D$ 是范畴等价可能过强。作为 Phase 1 的交付物，我们仅证明 $M \cong L$ 这一自然同构，作为更弱的但已足够用于理论建构的结论。
...
> 结论：连续谱情形不破坏 $M \cong L$，但需将 $M, L$ 理解为带测度的谱空间，自然同构 $\eta$ 为测度空间的 Borel 同构。
...
- **自然同构**：若限制到标量谱（即 $A_R$ 与 Clifford 作用可交换），$\eta_R(\mu_0) = e^{-\mu_0}$ 退化为普通指数。完整 Clifford 值情形的 $\eta_R$ 需要将 $M(R)$ 与 $L(R)$ 解释为 Clifford 值谱空间，并要求指数映射与 Clifford 模结构相容。

> 结论：在标准物理实例（SM、弦论）中，通常先取标量谱，Clifford 结构通过表示论进入；完整 Clifford 值谱的自然同构仍是开放研究方向。

---

## 10. 与伴随函子 $D \dashv R$ 的关系

自然同构 $M \cong L$ 与伴随函子 $D \dashv R$ 的关系可从以下两个角度理解：

1. **$M, L$ 由 $D$ 诱导**：$M$ 与 $L$ 本质上都是从 $D(R)$ 的谱数据读出的两种视角。$M(R) = \sigma(-\log \Phi_R^\ast)$ 与 $L(R) = \sigma(\Phi_R^\ast)$ 都通过 $D$ 定义，自然同构 $\eta$ 只是同一谱数据的不同参数化。

2. **$R$ 保持自然同构**：若右伴随 $R: \mathbf{Sp} \to \mathbf{Rec}$ 存在，则对任意 $E \in \mathbf{Sp}$，应有
...
3. **伴随的 unit/counit 与 $\eta$**：伴随的 unit $\eta: \mathrm{id}_{\mathbf{Sp}} \Rightarrow D \circ R$ 可视为自然同构 $M \cong L$ 在 $D \circ R$ 像上的特例；counit $\varepsilon: R \circ D \Rightarrow \mathrm{id}_{\mathbf{Rec}}$ 则保证递归系统的几何信息在 $R \circ D$ 中被适当恢复。
...
4. ~~**与伴随函子的关系**：自然同构 $M \cong L$ 是否与 $D \dashv R$ 的伴随结构相容？~~  已完成：见 §10，$M \cong L$ 是 $D$ 函子性的推论，伴随 $R$ 保持该等价。

---

## 12. 版本记录

- v0.1（2026-07-12）：初稿，将 $ \lambda_i = e^{-\mu_i}$ 表述为函子 $M$ 与 $L$ 之间的自然同构。
