# 噪声谱丛纤维化 $\pi_\eta: \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec}) \to \mathbf{Noise}$

**版本**：v0.1（2026-07-22）

**摘要**：本笔记将噪声强度参数 $\eta \in [0,\infty)$ 提升为 Grothendieck 纤维范畴的基空间 $\mathbf{Noise}$，构造噪声谱丛 $\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$ 及其投影 $\pi_\eta$。核心成果包括：(1) 验证 $\pi_\eta$ 是分裂 Grothendieck 纤维化——Cartan 提升由 Feynman-Hellmann 公式 $d\lambda/d\eta = \langle\psi_\lambda|\delta A_N|\psi_\lambda\rangle$ 给出；(2) 证明 $\tau(\eta) \propto 1/(\eta_c - \eta)$ 发散预言是截面在基边界 $\eta = \eta_c$ 处的奇异性定理；(3) 建立与 $\mathbf{Bun}(\mathbf{Temp}, \mathbf{Spec})$ 的丛态射（温度-噪声联合参数 $(\eta,T)$）。本形式化为 Paper XIX §11-13 和 Paper X §12.4 的预言提供了严格的纤维范畴基础。
...
## 2. 噪声谱丛 $\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$

### 2.1 总范畴的定义

**定义 2.1**（噪声谱丛总范畴）。$\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$ 是以下范畴：
...
**定义 2.2**（投影）。$\pi_\eta: \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec}) \to \mathbf{Noise}$ 定义为：
...
**定理 3.1**（$\pi_\eta$ 是 Grothendieck 纤维化）。投影 $\pi_\eta: \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec}) \to \mathbf{Noise}$ 是分裂 Grothendieck 纤维化。
...
**定义 3.1**。对每个 $\eta \in \text{Ob}(\mathbf{Noise})$，纤维范畴 $\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})_\eta$ 定义为：
- 对象：$(\eta, \{\lambda_i\})$，即 $\pi_\eta^{-1}(\eta)$ 中的对象
- 态射：$\text{Hom}_\eta((\eta, \{\lambda_i^{(1)}\}), (\eta, \{\lambda_i^{(2)}\})) = \{(\text{id}_\eta, \phi)\}$

**命题 3.1**。$\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})_\eta \cong \mathbf{Spec}_\eta$。$\square$
...
等价地，纤维截面 $\sigma_\Delta^{(\text{noise})}: \mathbf{Noise} \to \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$ 在 $\eta = \eta_c$ 处具有奇异性：截面在该点无法连续延拓到 $\eta > \eta_c$ 的区域。
...
**定理 4.2**（纤维类型跳变）。当 $\eta < \eta_c$ 时，纤维 $\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})_\eta$ 等价于 $\mathbf{Spec}$（离散谱）；当 $\eta > \eta_c$ 时，纤维等价于 $\Sigma\mathbf{-Spec}$（连续谱背景）。此跳变使得 $\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$ 是一个**非乘积丛**。

**证明**。由 `NoiseCategory.lean` 中的 Σ-Rec/Σ-Spec 构造，$\eta > \eta_c$ 时谱线展宽为连续谱带，对应 $\Sigma\text{-Spec}$ 对象。$\square$

---

## 5. 与 $\mathbf{Bun}(\mathbf{Temp}, \mathbf{Spec})$ 的丛态射

### 5.1 温度-噪声联合参数化

**定义 5.1**（联合参数丛）。定义乘积基 $\mathbf{Temp} \times \mathbf{Noise}$ 上的联合谱丛：

$$\mathbf{Bun}(\mathbf{Temp} \times \mathbf{Noise}, \mathbf{Spec})$$

其对象为 $((T, \eta), \{\lambda_i(T, \eta)\})$，其中 $\{\lambda_i(T, \eta)\}$ 是 $A(T, \eta) = A_R(T) + \eta \cdot \delta A_N(T)$ 的谱数据。

**命题 5.1**。限制函子 $\iota_T: \mathbf{Bun}(\mathbf{Temp} \times \mathbf{Noise}, \mathbf{Spec}) \to \mathbf{Bun}(\mathbf{Temp}, \mathbf{Spec})$（取 $\eta=0$）和 $\iota_\eta: \mathbf{Bun}(\mathbf{Temp} \times \mathbf{Noise}, \mathbf{Spec}) \to \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$（取 $T=T_0$）都是 Grothendieck 纤维化的态射。

### 5.2 丛态射

**定理 5.1**（温度-噪声丛态射）。存在纤维保持函子 $\hat{\mathcal{N}}: \mathbf{Bun}(\mathbf{Temp}, \mathbf{Spec}) \to \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$，其基函子为 $\Phi: \mathbf{Temp} \to \mathbf{Noise}$（$\Phi(T) = T$ 作为实数），使得以下图表交换：

$$
\begin{CD}
\mathbf{Bun}(\mathbf{Temp}, \mathbf{Spec}) @>{\hat{\mathcal{N}}}>> \mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec}) \\
...
| `NoiseFiber` | $\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$ + $\pi_\eta$ 纤维化 | 复用 `SpecFiberTemp` 模式 |
...
| 版本 | 日期 | 更新内容 |
|:----|:----|:--------|
| **v0.1** | **2026-07-22** | 初始版本：$\mathbf{Noise}$ 范畴定义；$\mathbf{Bun}(\mathbf{Noise}, \mathbf{Spec})$ 纤维化；Feynman-Hellmann Cartan 提升；$\eta_c$ 奇异性定理；纤维类型跳变；温度-噪声丛态射；Lean 形式化方案 |
