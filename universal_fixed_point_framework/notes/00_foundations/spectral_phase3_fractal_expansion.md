...
> **状态**：研究笔记 v0.1（2026-08-05）。阶段 3 **首个子任务完成**：数值验证 **7/7 检查通过**（`paperX_ifs_sigma_rec_spectral.py`，已注册 `run_all_tests.py`）。Lean 侧桥接（IFSFractal ↔ NoiseCategory）为后续子任务。
...
| Σ-Rec coproduct 谱保持 | `NoiseCategory.lean` §15.3（Σ-D 保持 coproduct） | ✅ 机器证明 |
| IFS 片 → RecObj 编码 | `IFSFractal.lean` `IFSToRecObj'`（当前 step := id 占位） | ⏳ 待升级（S2 的片级确定性步进） |
| 谱隙从压缩比导出 | 待（谱隙的 Lean 表述依赖有限维谱积分层） | ⏳ 阶段 3 依赖 |

**下一步候选**：
1. 升级 `IFSToRecObj'` 的占位步进为片级确定性转移（S2 的 Lean 化）；
2. 将 Weierstrass 图 IFS 的压缩比 → 谱隙关系整理为 Lean 命题（依赖有限维谱积分层，mathlib `ContinuousFunctionalCalculus` 桥接）；
3. 阶段 3 任务 3（D 在 Σ-Rec 上保持 coproduct 的数值全验证，§15.3 定理的直接引用）。
...
| `formal_proof/.../NoiseCategory.lean` | Σ-Rec / ι_Σ / §15.3 D 保持 coproduct |
