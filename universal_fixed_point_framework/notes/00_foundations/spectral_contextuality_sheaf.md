...
**摘要**：本笔记将 Kochen-Specker 语境性定理翻译为谱框架中的层论表述。基为 $\mathbf{Spec}$ 的交换子范畴覆盖 $\{\mathbf{Spec}_{\text{com}}^{(i)}\}$（每个覆盖对应一个测量语境），纤维为 $\{0,1\}$ 真值赋值。K-S 定理等价于该预层无全局截面。v0.2 新增 **Peres-Mermin 方具体构造**——9 可观测量（$\sigma_x\otimes I$, $I\otimes\sigma_x$, $\sigma_x\otimes\sigma_x$ 等）在 6 个语境（3 行 + 3 列）上的矛盾证明：行乘积 $=+1$ 而列乘积 $=-1$，同一连乘不能同时等于两个值。
...
**定义 1.1**（交换子范畴覆盖）。令 $\mathcal{C} = \{\mathbf{Spec}_{\text{com}}^{(1)}, \ldots, \mathbf{Spec}_{\text{com}}^{(k)}\}$ 是 $\mathbf{Spec}$ 的交换子范畴族，其中每个 $\mathbf{Spec}_{\text{com}}^{(i)}$ 由一组两两交换的谱对象构成。$\mathcal{C}$ 覆盖 $\mathbf{Spec}$ 当：
$$\bigcup_i \text{Obj}(\mathbf{Spec}_{\text{com}}^{(i)}) = \text{Obj}(\mathbf{Spec})$$

**注 1.1**。语境覆盖的存在性由 K-S 定理保证：如果存在一个交换子范畴覆盖整个 $\mathbf{Spec}$，则非语境隐变量模型存在。
...
- $\mathcal{F}(\mathbf{Spec}_{\text{com}}^{(i)}) = \{v: \text{Obj}(\mathbf{Spec}_{\text{com}}^{(i)}) \to \{0,1\} \mid v \text{ 是乘法同态}\}$
- 限制态射 $\mathcal{F}(\mathbf{Spec}_{\text{com}}^{(i)} \supseteq \mathbf{Spec}_{\text{com}}^{(j)})$：真值赋值的限制
...
$$\not\exists\, s \in \mathcal{F}_{\text{PM}}(\mathbf{Spec}) \quad \text{使得} \quad s|_{\text{RowA}}, s|_{\text{RowB}}, s|_{\text{RowC}}, s|_{\text{Col1}}, s|_{\text{Col2}}, s|_{\text{Col3}}$$
