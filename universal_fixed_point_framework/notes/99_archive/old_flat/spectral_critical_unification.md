...
**关联**：
- 流变-Lorentz 同构：`notes/spectral_rheology_lorentz_isomorphism.md`
- 流变谱边界严格化：`notes/spectral_rheo_boundary.md`（主定理 E3 三类临界现象统一）
...
- 与 DST 临界硬化实验（`notes/spectral_rheology_experiments.md` §1）共享分析方法
...
由主定理 E1（`notes/spectral_rheo_boundary.md` §4），流变硬化的临界指数 $-1/2$ 由 $\mathfrak{so}(1,1)$ Lie 代数唯一确定。量子相变在 $z\nu = 1/2$ 时共享同一 Lie 代数结构，故两者同构。$\square$
...
**谱间隙保持**：由主定理 E1-E3（`notes/spectral_rheo_boundary.md`）与主定理 F1-F4，所有七类临界现象都满足 $\Delta\lambda_{\min} \to 0$，故 $\mathcal{F}$ 保持谱间隙结构。
...
- `notes/spectral_rheology_lorentz_isomorphism.md` — 流变-Lorentz 同构
- `notes/spectral_rheo_boundary.md` — 流变谱边界严格化（主定理 E1-E3）
- `notes/spectral_rheology_experiments.md` — 流变实验设计
