...
**定理 1**（谱对应自然等价，Paper I 定理 3.7a）。存在 $\mathbf{Spec}$ 范畴中的自然等价：
$$M \cong L,$$
其中 $M$ 是谱去递归函子 $D$ 的像（压缩算子谱），$L$ 是生成元谱。
...
从谱对应自然等价 $M \cong L$，电磁谱算子 $A_{\text{EM}}$ 的特征值为：
...
谱对应自然等价 $M \cong L$ 同样适用于 SU(2) 和 SU(3) 规范耦合：
