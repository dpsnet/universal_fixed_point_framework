...
**开放项**：自伴性与 Mourre 估计的 **Lean 形式化**（Kato–Rellich/Nelson + 谱测度理论库依赖——推导级 + 数值佐证已闭合 2026-08-14）；耦合情形 a.c. 谱保持的独立证明（FGSS 线文献引用）；Friedrichs 模型严格化（共振极点，η(z) 下半平面零点 + spec(H)=[0,∞)_ac∪{阈值下束缚态}）；WW 复极点的 Lean 代数骨架；成熟后提炼 paper44 §2.4/§7.5。
