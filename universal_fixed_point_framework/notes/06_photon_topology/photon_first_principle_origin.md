...
**诚实边界**：层次 A（可数无穷离散能级，Rydberg 序列）已闭合（v0.62：数值 S6/S7/S8 8/8 注册 + Lean `rydberg_combination`/`multiObjInfinite`/`rydberg_band_edge` 机器证明）；范畴骨架已通用、新增为物理函数具体化与谱结构（数值/Lean 数据核对）；层次 B（连续谱 [0,∞)）为库依赖开放项，未闭合；组合原理/带边极限为标准量子物理事实（telescoping/极限，数据核对），非新预言。
