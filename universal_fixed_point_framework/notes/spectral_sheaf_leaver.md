...
**摘要**：本笔记将 Leaver 连续分数法对应的三对角矩阵族 $M(\omega)$ 翻译为谱丛语言，揭示其**细分纤维化（fine fibration）**结构。核心发现：(1) 三对角矩阵的 off-diagonal rank-1 性质天然定义了一个二叉树纤维化；(2) 去递归理论的 D 函子对应谱丛的全局截面；(3) 同伦延拓路径对应谱丛沿参数空间的平行移动（单值性）；(4) 非物理根吸引域对应谱丛分支点的叶间跳跃。这一视角统一了已有的数值技巧，并给出了严格的数学基础。
...
去递归理论的核心 $$D: \mathbf{Rec} \to \mathbf{Spec}$$
...
3. **剪枝优于两弦法的条件**：$C_{\text{prune}} \cdot \log^2 N < C_{\text{弦法}} \cdot N$。对典型参数 $N=100$，剪枝在 $\log^2 N \approx 45$ 次 Schur 补内展开即有优势。建议自适应混合策略：先试剪枝，若剪枝无效（$|q|$ 持续大于阈值）退化为两弦法。
...
3. **RN 的显式 $D_i$ 系数**：给出含电荷 $Q$ 的完整 $D_i(\omega)$ 表达式，三对角谱丛结构保持，无需修改两弦法求解器。
