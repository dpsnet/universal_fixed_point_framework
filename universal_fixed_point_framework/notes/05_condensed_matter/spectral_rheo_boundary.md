...
**关联**：
- 流变-Lorentz 同构（主定理 A-D）：`notes/05_condensed_matter/spectral_rheology_lorentz_isomorphism.md`
...
严格化遵循 MUFPF 公理层级非反馈原则：所有证明仅使用元公理 1-2（递归存在、谱化）与结构定理（Paper V 谱流方程、Paper VIII $\partial\mathbf{Rec}_D$），流变层不反馈到上层。
...
这些方向在 Phase 51F-F5 中进一步探索（见 `notes/05_condensed_matter/spectral_critical_unification.md`）。
...
- `notes/05_condensed_matter/spectral_rheology_lorentz_isomorphism.md` — 流变-Lorentz 同构（猜想 E/F 的原始提出）
- `notes/04_lorentz_gravity/spectral_lorentz_dynamics.md` — Lorentz 谱动力学核心
- `notes/04_lorentz_gravity/spectral_lorentz_causality.md` — 因果结构（主定理 8 光锥=∂Rec_D）
