...
1. **脚本 `paperX_shale_spectral.py`**（已注册 `run_all_tests.py`，18 项检查，17/18 通过 + 1 项负结果登记 + 1 子项根因已诊断，2026-08-08）：
...
|:--|:--|:--|:--|:--|
| P1 | 谱隙-门限压力双曲标度 | $\log P_t = C/(D-2)+B$，$D\to2^+$ 时 $P_t$ 对数发散 | Washburn 方程只给 $P_t$–孔径关系；$P_t$–$D$ 在文献中仅经验线性/幂律拟合，无第一性函数形式 | M11 实证：双曲 R²=0.578 > 线性 0.450；理论斜率 2.24 ≈ 实测 1.81；$D\to2$ 端待极端均匀样品（低复杂度页岩/粉砂质）检验 |
...
| P4 | 游离烃零注入阈值 | 线性注入 $S_1 = 0.57\cdot\text{TOC} - 0.24$ 外推：$\text{TOC}^* = 0.24/0.57 \approx 0.42$ wt% 以下 $S_1=0$（无游离烃） | 传统有"有效烃源岩 TOC 下限 ~0.5%"的判据性经验值，但无由生烃-注入标度直接外推的定量阈值 | M8 数据标定（长7段 10 样品 R²=0.994）；多盆地更多样品待检验；可与 0.5% 判据对比或证伪 |
