...
1. **脚本 `paperX_shale_spectral.py`**（已注册 `run_all_tests.py`，16 项检查，15/16 通过 + 1 项负结果登记 + 1 子项根因已诊断，2026-08-08）：
...
**三、负结果与诚实边界**：seal shale 上"结构复杂度→可动流体↓"直觉不成立（M2）；B1 幂律候选被量级验证否定（已修正）；**谱隙-毛管压力定量对应已闭合——经验形式（M10）与理论双曲形式（M11）**；**单井窗口效应已量化（M12：跨盆地双互补指标穿透窗口，窗内干酪根质量耦合 ρ=+0.868）**；产油页岩可动流体-分形成对数据待开放数据源。
