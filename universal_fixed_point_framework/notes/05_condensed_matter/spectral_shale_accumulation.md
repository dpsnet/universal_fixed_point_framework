...
1. **脚本 `paperX_shale_spectral.py`**（已注册 `run_all_tests.py`，14 项检查，13/14 通过 + 1 项负结果登记 + 1 子项根因已诊断，2026-08-08）：
   ...
   - **M10 谱隙-毛管压力定量对应（通过，首要开放问题闭合）**：Tuscaloosa 31 样品标定门限压力与分形维数经验对应 **log P_t = 1.81·D + 3.22**（ρ(D, logP_t)=**0.671**，R²=0.450）——**结构复杂度→门限压力显著正相关**（分形维数每 +1，封堵压力 e^1.81≈6.1 倍）——UFPF 谱隙（Δλ_mat）与毛细管门限压力（P_t）的定量对应首次建立（经验形式，物理机制=结构复杂度驱动孔喉减小→封堵增强）；
...
**三、负结果与诚实边界**：seal shale 上"结构复杂度→可动流体↓"直觉不成立（M2）；B1 幂律候选被量级验证否定（已修正）；**谱隙-毛管压力定量对应已建立经验形式（M10：log P_t=1.81·D+3.22），但 Δλ↔P_c 的理论函数形式仍待第一性推导**；单井成熟度窗口效应限制谱流方向性的观测；产油页岩可动流体-分形成对数据待开放数据源。
...
- **已知边界**：B 级预测为候选假设；**谱隙-毛管压力定量对应已建立经验形式（M10：log P_t=1.81·D+3.22，ρ=0.671），Δλ↔P_c 的理论函数形式（第一性推导）仍为下一阶段任务**；
