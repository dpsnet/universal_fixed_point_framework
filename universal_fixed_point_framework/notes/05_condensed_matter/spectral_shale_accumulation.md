...
1. **脚本 `paperX_shale_spectral.py`**（已注册 `run_all_tests.py`，15 项检查，14/15 通过 + 1 项负结果登记 + 1 子项根因已诊断，2026-08-08）：
...
**三、负结果与诚实边界**：seal shale 上"结构复杂度→可动流体↓"直觉不成立（M2）；B1 幂律候选被量级验证否定（已修正）；**谱隙-毛管压力定量对应已建立——经验形式（M10：log P_t=1.81·D+3.22）与理论双曲形式（M11：log P_t=−1.66/(D−2)+10.47，第一性推导自压汞分形+最小可测截止）**；单井成熟度窗口效应限制谱流方向性的观测；产油页岩可动流体-分形成对数据待开放数据源。
