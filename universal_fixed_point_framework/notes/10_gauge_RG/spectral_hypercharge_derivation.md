...
## 3. 从 $8_s$ 旋量表示提取超荷【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（Cl(1,7) ≅ M₁₆(ℝ)，标准旋量维数 16）】

### 3.1 Cl(1,7) Gamma 矩阵的显式构造

采用 `zero_parameter_derivation.md` §5.1 的 8×8 实表示：【2026-08-07 勘误：Cl(1,7) 标准矩阵代数系 M₁₆(ℝ)，对应 16×16 实矩阵表示，"8×8 实表示"系旧遗留表述】
...
其中 $\Sigma_{\mu\nu}$ 是 $8 \times 8$ 矩阵。【2026-08-07 勘误：Cl(1,7) 标准矩阵代数系 M₁₆(ℝ)，Σ_μν 应为 16×16 实矩阵，"8×8"系旧遗留】

### 3.3 $8_s$ 旋量空间的基【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（Cl(1,7) ≅ M₁₆(ℝ)，标准旋量维数 16）】

$8_s$ 旋量表示有 8 个基向量 $|s_1s_2s_3\rangle$，其中 $s_i \in \{\uparrow, \downarrow\}$，对应三个张量积因子 $\sigma_x, \sigma_y, \sigma_z$ 的本征态。【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）；"8 个基向量"系旧框架计数】

### 3.4 Cartan 子代数生成元的对角化

在 $8_s$ 表示中，我们选择 Cartan 子代数的生成元为四个互相反对易的对角化生成元。一个自然的选择是：【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）】
...
**定理 3.1**（$H_i$ 的本征值）。在 $8_s$ 旋量表示中，四个 Cartan 生成元 $H_1, H_2, H_3, H_4$ 的本征值均为 $\pm\frac12$，且 $8_s$ 的 8 个基向量一一对应于 $(\pm\frac12, \pm\frac12, \pm\frac12, \pm\frac12)$ 的 8 种符号组合。【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）；16 维旋量的符号组合计数为 2⁴ = 16，非 8】
...
计算得 $T^3$ 在 $8_s$ 上的本征值为 $\pm\frac12$，其中：【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）】
- $+\frac12$：左手中微子态
- $-\frac12$：左手电子态
- $0$：所有右手态
...
### 4.1 $8_s$ 基向量上的荷值【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（Cl(1,7) ≅ M₁₆(ℝ)，标准旋量维数 16）】

令 $8_s$ 的基向量为 $|s_1s_2s_3\rangle$，其中 $s_i = \pm 1$ 对应 $\sigma_i$ 的本征值。我们计算每个基向量上的 $T^3$ 和 $Y$。【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）】
...
- $Y$ 的本征态构成 $8_s$ 的完备正交基【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）】
...
Spec 4-范畴
     ↓
Cl(1,7) ≅ M₈(ℝ) × M₈(ℝ)【2026-08-07 勘误：Cl(1,7) 标准矩阵代数系 M₁₆(ℝ)（代数维数 256，旋量维数 16），并非 M₈(ℝ) × M₈(ℝ)】
     ↓
8_s 旋量表示 ──────→ 8 个基向量【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）】
     ↓                    ↓
...
1. Cl(1,7) 的 Gamma 矩阵张量积表示（固定 $8_s$ 的基）【2026-08-07 勘误：8_s 系旧遗留记号，应理解为 16 维旋量 S₁₆（标准旋量维数 16）】
