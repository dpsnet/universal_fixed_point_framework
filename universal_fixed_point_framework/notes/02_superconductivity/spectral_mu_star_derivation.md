...
| S6 | 数值验证 | 本工作 | Al, Sn, Pb, Hg, Nb 五类材料 |
...
> >
> > $$\text{偏差}_{\text{total}} = \text{偏差}_{\text{映射链}} + \text{偏差}_{\text{μ*}} = 10.8\% + 0.9\%$$
> >
> > 谱映射链贡献了 92% 的总偏差。□
...
**注**：McMillan 公式对所有材料的 $T_c$ 系统性高估，这是 McMillan 公式自身（两方阱近似）的精度限制，不是 $\mu^*_{\text{spec}}$ 的问题。使用 $\mu^*_{\text{spec}}$ 与使用经验 $\mu^*$ 的 $T_c$ 高度一致，说明 $\mu^*_{\text{spec}}$ 成功替代了经验参数。
...
**核心结论**：$\mu^*_{\text{spec}}$ 公式在全部六类材料上均验证有效。MgB$_2$ 的独立验证证明多带 μ*_spec 在 p-轨道体系中使用标准 D$_0$ = 0.122。
...
验证程序 `src/spectral_bcs_strong_coupling_closed.py` 包含以下模块：

| 模块 | 功能 |
|:----|:-----|
| `mu_star_spectral(eps_F, wD)` | μ*_spec 闭式公式 |
| `Tc_McMillan(lambda, mu_star, wD)` | McMillan T_c 公式 |
| `a_GeilikmanKresin(Tc, wD)` | GK 能隙比修正 |
| `a_spectral(r, Z)` | 谱框架比例因子 |
| `r_from_a(a_target, Z)` | 逆求解（二分法） |
...
3. **非常规超导**（铜氧化物、铁基）：d-波/p-波谱密度框架已设计（§7.4），但需实际材料数据验证
