...
这三个收缩因子**不依赖任何实验输入**（见 `notes/02_ckm_pmns_flavor/spectral_zero_parameter_derivation.md`）。
...
三个扇区的指数 $\alpha_u = 1.945$（上型夸克）、$\alpha_d = 1.229$（下型夸克）、$\alpha_l = 1.358$（带电轻子）来源于 **Spec** 的 Cl(1,7) 代数结构（Paper I §A.15.8）。标度 $M_{\text{sector}}$ 通过顶夸克、底夸克、$\tau$ 子的实验质量固定——这是**唯一的半经验输入**，但质量比完全由谱框架决定（8/9 在因子 2 内，见 Paper XI §8.4 及 `notes/02_ckm_pmns_flavor/spectral_full_19_parameters.md`）。
...
与实验完美匹配（偏差 < 0.1%）。PMNS 三个混合角由 $6\times6$ 轻子质量矩阵的谱对角化给出（Paper XI §8.6），预测值与实验偏差 ×1.00–×1.04（见 `notes/02_ckm_pmns_flavor/spectral_PMNS_theta13.md`）。
...
**汇总**：15/29（52%）严格零参数预测 ✅，14/29（48%）部分预测 🟡，0 参数未覆盖。谱框架已证明自身是一个有完整覆盖能力的物理参数推导体系——所有 29 个参数均至少有一条从 **Spec** 第一原理出发的推导路径（详见 Paper XI 附录 D 及 `notes/99_archive/spectral_parameter_audit.md`）。

---

> **关键文献索引**：Paper I（**Spec** 范畴基础）、Paper XI（谱 QFT 公理系统与 29 参数审计）、`notes/00_foundations/rec_spec_definitions.md`（范畴定义）、`notes/02_ckm_pmns_flavor/spectral_zero_parameter_derivation.md`（零参数推导）、`notes/10_gauge_RG/spectral_alpha_derivation.md`（谱间隙与规范耦合）、`notes/02_ckm_pmns_flavor/spectral_CKM.md`、`notes/02_ckm_pmns_flavor/spectral_PMNS_theta13.md`、`notes/01_qcd_higgs/spectral_strong_CP.md`。
