...
**状态**：🟡 直觉/诠释笔记 v0.9（2026-08-14；v0.1 登记直觉与精确形式；v0.2 新增"范畴层延伸"与"主观=观测时间"澄清；v0.3 新增"引力侧在谱流中的表达"；v0.4 新增 §5.1"等价性证明"（paper18 §11 谱交织条件）；v0.5 新增 §7"图像登记"（同心圆坐标系：径向=T-共轭方向、切向=空间、垂直=时间；T 藏在径向层间变换；弥漫性包覆修正）；v0.6 §7 补充"两种质量 = 同一几何图像"；v0.7 §7 收敛"核心纲领"（点增值以不同径向距离、切向等距铺满所有法向自由度，保证来自结构/对称性/强制平均；T = 结构涌现，CONJECTURE 级）；v0.8 核心纲领补 §7.1"本体论基础"（实在=范畴 ⟹ 范畴=关系（Yoneda）⟹ 关系从一点增值=关系网生成）与 §7.2"可证伪性分层判定"；v0.9 补 §7.3"闭合路径评估"（①谱交织前提涌现化 + ②缺陷来源=径向分布不均候选（ADM/位错先例，避开 Δ 结构常数陷阱）；闭合候选非闭合完成，T-共轭环节与三项形式化未决）——王斌基于 paper44 §5.3 双法向偏转统一提出的直觉：质量 = 偏转时间轴的难度；本笔记将其精确化并与框架内已有结构（paper44 时间耦合图像、paper16 质量=谱间隙、paper18 谱牛顿、paper31/35 引力起源、等效原理）挂钩）
...
**实例化出口**：光子展开（paper44 Φ，P1–P6 已盲登记，远期）；静默分层 S_k = s^{n_k}（层指数 n_k 已锚定：3/4.207/2π/α）；T = 弥漫结构强制平均（未实例化，CONJECTURE）。
...
- 归属：路径③（世界线/固有时）× 动力学列；亦可挂路径④（对称群）× 动力学列（经 m² = Δλ_M）。
- 等级：**表述层诠释**（与主流结论（$E=(\gamma-1)mc^2$、$d\tau/dt=\cos\theta_{\rm esc}$）无可观测分歧）——不计入检测力，不构成缺口登记。
- 有锚性：内容均可由已检验路径独立到达（SR 运动学、GR 引力红移），属 ✅/⚠️ 格。
...
- 或挂接融合路线路径③ 的融合点登记（光子 ⊥ 时间 ⟷ 零代价偏转），作为表述层内容补充；
- 无新预言，无盲登记需求。
