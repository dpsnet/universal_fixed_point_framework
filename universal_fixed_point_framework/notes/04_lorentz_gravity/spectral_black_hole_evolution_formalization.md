...
**超辐射谱 → 蒸发动力学衔接【谱新增，2026-08-08，`scripts/paperX_kerr_sr_evaporation.py` 5/5 注册 `run_all_tests.py`】**：完整超辐射谱定量接入蒸发动力学（E1–E5）：

| 检查项 | 数值 | 物理 |
|:--|:--|:--|
| 超辐射增强因子（E1） | η(a*) = 0.008 / 0.777 / 220（a* = 0.5/0.9/0.99） | 超辐射增强蒸发，随转动单调（a*→1 主导发射） |
| 角动量效率（E2） | dJ/dE = 4.15/M（⟨ω⟩_sr = 0.241） | 超辐射低频模每单位能量高角动量提取 |
| 多模求和（E3） | l=m=2 贡献 36.5%（窗口 ω < 2Ω_H） | 多模超辐射显著增强 |
| **简化模型双向偏差（E4）** | 低转动低估（比值 8.7×）、中等近似（0.52）、极端高估（0.02） | **简化 R_J = 2 仅在中转动（a* ≈ 0.9）有效；低转动低估、a*→1 因 f³→0 高估角动量提取** |
| 蒸发轨迹（E5） | a*: 0.9 → 0.744（简化）/ 0.723（完整谱）单调递减 | Kerr → Schwarzschild 方向一致，完整谱加速角动量提取 |

**关键结论**：完整谱显示简化模型（常数 R_J = 2）的**双向系统性偏差**——低转动（a* = 0.5）低估角动量提取 8.7×（完整谱低频模 m/ω 效率高）、极端转动（a* → 1）高估（简化 dJ/dM ∝ a*/f³ 在 f → 0 时暴增）——**简化模型有效范围 = 中等转动（a* ≈ 0.9 附近）**，此前"简化图像获完整谱支持"结论修正为"中等转动近似、极端转动需完整谱"。
