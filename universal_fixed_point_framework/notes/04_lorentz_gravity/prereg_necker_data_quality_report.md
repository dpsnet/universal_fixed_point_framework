...
4. **空间离群点**：少量 gaze 值被替换为大幅度随机数（目标率 **0.5%**，σ = **3.0°**，校准后）；
5. **眨眼/眼跳漏检误检**：计数 ±1 抖动；
...
**说明**：v0.26 校准后，将 `gaze_outlier_rate` 从 2% 降至 0.5%、离群点 σ 从 5.0° 降至 3.0°， gaze RMS 从约 1.23° 降至 **0.767°**，进入典型眼动仪精度范围（0.3°–1.0°）。离群点率（\|gaze\| > 3°）随之降至 0.17%，处于真实实验常见范围的下限。

### 4.3 瞳孔噪声分布

| 指标 | 数值 | 典型真实范围 | 评估 |
|:--|:--|:--|:--|
| 瞳孔基线均值 | 3.523 mm | 2–5 mm | 符合 |
| 瞳孔基线标准差 | 0.195 mm | 0.1–0.4 mm | 符合 |
| 瞳孔平均直径均值 | 3.558 mm | 2–5 mm | 符合 |
| 瞳孔峰值直径均值 | 3.717 mm | 2–6 mm | 符合 |
| 完全缺失率 | 14.07% | 5%–20% | 符合 |
| 部分缺失率 | 9.12% | 5%–15% | 符合 |
| 眨眼伪迹率 | 1.48% | 1%–5% | 符合 |
...
在清洗后的 101,113 试次上拟合 MUFPF toy 幂律模型：
...
**结论**：在由 MUFPF toy 生成的含噪声数据上，MUFPF toy 模型显著优于标准 DDM（ΔAIC > 10,000）。这一结果符合预期，因为数据本身由 MUFPF toy 生成；本报告目的是验证清洗/比较流程在 12.22% 排除率下仍能稳健恢复 γ，而非证明 MUFPF 优于 DDM。
...
3. **MUFPF toy 生成偏差**：C 与 t0 估计存在系统性偏移，提示真实实验需要试点阶段重新估计参数先验；
4. **DDM 仅使用标准形式**：未拟合非线性漂移、时变边界、有色噪声等扩展模型，因此 ΔAIC 可能夸大 MUFPF toy 的优势；
5. **子样本比较**：DDM 在 10,000 试次子样本上拟合，MUFPF toy 在 101,113 试次上估计参数但在同一子样本上评估似然，AIC/BIC 基于同一子样本量计算，具有可比性；
