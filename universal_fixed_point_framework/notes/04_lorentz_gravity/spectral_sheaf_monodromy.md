...
**摘要**：本文档基于 `notes/04_lorentz_gravity/spectral_sheaf_leaver.md` 建立的谱丛理论框架，针对 §6 开放问题1 设计完整的单值群计算方案。第一部分建立支点分类定理、单值群生成元理论和双重同伦延拓的单值群解释；第二部分给出数值实现的具体方案（伪代码级设计）。
