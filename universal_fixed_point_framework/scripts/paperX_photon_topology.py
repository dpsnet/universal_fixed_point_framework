...
验证项:
  ...
  S2 光速不变拓扑定理 (定理 3.1): c = 1/sqrt(mu0*eps0) + 洛伦兹速度加法不变 + 空间平移不变
  S3 c = lambda*nu 自洽定理 (定理 3.2): lambda*nu = c 恒等式 + 反比关系 + 红移保持
  S4 E = h*nu 拓扑释义 + 可拦截性 (命题 4.1 / 命题 2.3 / 定义 2.4): Bohr 匹配 + 吸收截面共振选择性
  S5 推论 2.1 时间解耦: 传播途中零时间耦合, 耦合仅在端点 (发射 D 转变 / 吸收 R 折叠)
  S6 零静质量 v < c 不自洽 (命题 3.1): m = 0 -> E = pc -> v_g = c (E^2 = p^2 c^2 + m^2 c^4)

诚实边界: 本脚本验证的是公理 A4 / 定理 3.1 / 定理 3.2 / 命题 2.3 / 推论 2.1 / 命题 3.1
...
# ============================================================
# S4 E = h*nu 拓扑释义 + 可拦截性 (命题 4.1 / 命题 2.3 / 定义 2.4)
# ============================================================
def s4_energy_and_interception():
    ...
    def sigma_abs(nu_v):
        # 定义 2.4: sigma_abs(nu) = (h*nu/c)*B12*g(nu), g 为洛伦兹线型
...
def s10_selection_rules():
    # 命题 2.3 取向门: 光子沿量子化轴的角动量 J_z 须与原子跃迁角动量变化匹配——
    #   已知物理 = 原子选择定则 (电偶极 E1: Δl=±1, Δm=0,±1; σ±↔Δm=±1, π↔Δm=0)
    #   拓扑重述: 选择定则 = 取向门控 (结合 = 能量门 ∧ 取向门); 注意 J_z 沿量子化轴,
    #   螺旋度 s 沿传播轴 k (拓扑表述 2.5.1), 二者仅当 k∥ẑ 时重合——线偏振光子
    #   螺旋度仍为 ±1, 其 Δm=0 来自几何 (ε∥ẑ) 而非 s=0
    # C37: 偏振-沿量子化轴角动量 J_z-Δm 匹配表 (σ±: J_z=±1; π: J_z=0)
    sel = {"sigma+": (+1, +1), "sigma-": (-1, -1), "pi": (0, 0)}  # 偏振 -> (J_z, Δm)
    match37 = all(j == dm for j, dm in sel.values()) and set(sel) == {"sigma+", "sigma-", "pi"}
    check("S10-C37 偏振↔J_z↔Δm 匹配表 (σ±↔±1, π↔0)",
          bool(match37),
          "σ+↔(+1,+1), σ-↔(-1,-1), π↔(0,0)")
