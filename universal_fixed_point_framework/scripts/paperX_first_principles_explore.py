...
def run():
    ...
    print(f"  → k_max=8 的'选择'依据 = 匹配 ρ_c = 0.335（循环）；非独立群论推导")
    check("P3a Cl(1,7) '代数维数'声称混淆：真实代数维数 2⁸=256，8 是底空间维数",
          True, "paper36 候选 A 标签错误（代数维数 ≠ 8）")
    check("P3b k_max=8 无非循环第一性来源（Bott 周期 8 与谱截断无直接推导；"
          "ρ_c 匹配为循环）",
          True, "k_max 为模型输入，Δλ_min 公式在给定 k_max 下严格")
    ...
    print("    P3 k_max=8 无第一性来源——'Cl(1,7) 代数维数'声称混淆（真代数维数 256）；")
    print("       实际选择依据 = 匹配 ρ_c（循环）；Δλ_min 公式在给定 k_max 下严格（Lean）")
    print("    ★ 第一性边界：比值（严格）→ α^bare（谱）→ [SM 跑动 + 方案转换] → α(M_Z)")
    print("      框架第一性 = 谱量（比值、Δλ_min 公式给定 k_max）；输入 = k_max、实验 α(M_Z)")
