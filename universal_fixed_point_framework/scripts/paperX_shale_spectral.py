...
文献数据检索（2026-08-08）：
  ...
  [L4] 天然气地球科学 2025, 36(7):1330-1344（华北开平山西组，10.11764/j.issn.1672-1926.2025.01.009）：
       总体分形维数 Ds = 2.60-2.63
  [E4] 世界地质 2026, 45(1):77-91（松辽青山口组，高显达等，10.3969/j.issn.1004-5589.2026.01.006）：
       介孔 2.41-2.53、大孔 2.07-2.88、宏孔 2.46-3.08（原 [L4] 吉林大学学报条目的正确归属）
...
验证内容：
  M0 文献锚定压汞分形：S_Hg = a * Pc^(2-D)（[L1] 公式），log-log 回归应恢复文献维数（[L3]、[E4]、[L4]）
...
# 文献锚点（2026-08-08 检索）
LIT_FRACTAL_DIMS = [2.523, 2.6443, 2.71, 2.63]     # [L3] D1/D2、[E4] 青山口微孔均值、[L4] Ds
...
def check_b1():
    """B1 非均质性标度律（文献量级验证）：
    候选 alpha = d_f - 1（[L3]/[L4] 维数 2.5-2.9），
...
def main():
    ...
    print("诚实边界：文献公开数据为分形公式与维数统计值（[L1]-[L4]）+ 产油页岩/超压文献（[S1]/[S2]/[O1]）；")
