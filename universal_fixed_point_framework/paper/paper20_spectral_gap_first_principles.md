...
Step 5: 统一 3 定理 + 对偶网络 → k_max = 2³ = 8（结构确定，§5.4）
    ↓   ρ_c 扫描 {4,6,8,16,100} 保留为交叉验证
...
### 5.4 k_max = 8 的结构确定（已修正）

**定理 5.3**（截断来源）。$k_{\max}=8$ 由**统一 3 定理机器证明**确定：$k_{\max} = 2^{N_{\text{active}}} = 2^3 = 8$（`Unified3Theorem.lean`，Paper XXXIII），指数 $N_{\text{active}}=3$ 为严格 4-范畴主动生成层数；且 $k_{\max}=8$ 处于底层结构对偶网络中心——旋量 $16 = 2k_{\max}$、分支 $B = 15 = 2k_{\max}-1$、维数 $d_H = \ln(2k_{\max}-1) = \ln 15$（`paperX_kmax_duality.py` 10/10）。**2026-08-07 修订**：此前"模型选择"结论（扫描 $\rho_c$ 匹配）已过时——扫描结果 $\Delta\lambda_{\min} = 0.122$（$k_{\max}=8$）与 paper20 谱间隙值精确一致，但 $k_{\max}$ 本身不依赖该扫描，由范畴结构确定。
...
*注*。$\mathrm{Cl}(1,7)$ 的同构类由 Bott 周期唯一确定（$M_{16}(\mathbb R)$）；$k_{\max}=8$ 由统一 3 定理（$2^{N_{active}}$）机器证明 + 对偶网络（$B = 2k_{\max}-1$）确定，不再属模型输入层。扫描表保留为交叉验证（结构确定值恰为匹配最优值）。
