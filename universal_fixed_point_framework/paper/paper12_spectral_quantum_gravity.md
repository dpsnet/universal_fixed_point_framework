...
谱动力学在 Page 1993 假设下复现的 Page 曲线给出：
...
谱框架复现了 Page 的幺正演化结果。注意：这是在外部输入 Page 1993 幺正性假设下的复现，不是从谱公理唯一导出的预测。
...
**方向 2：有限 $N$ 修正**。谱截断 $k_{\max}=8$ 为模型选择（原表述"来自 Cl(1,7) Bott 分类"已停用；见《RAP_勘误与立场声明.md》）决定边界 CFT 的有效秩 $N = (k_{\max}+1)(k_{\max}+2)/2 = 45$。$k_{\max}$ 有限产生谱传播子的 $1/N$ 修正 $\delta K_{\text{spec}}^{1/N} / K_{\text{spec}} = 2/(k_{\max}+3) = 2/11$，该修正在当前实验精度下不可观测（LIGO ringdown 修正 $<10^{-16}$），但在未来 Einstein Telescope 的 Planck 标度附近处于可探测边界。
...
- 谱截断阶数：$k_{\max} = 8$（模型选择；原"来自 Cl(1,7) Bott 分类"表述已停用）
...
|:-:|:----|:----|:---------------|:----------:|:---------:|
| **A1** | $M_Z$ 规范耦合：$\alpha_1^{-1}=59.2$（0.3%）、$\alpha_2^{-1}=30.1$（1.7%）、$\alpha_3^{-1}=8.7$（2.4%） | §8.3，Paper XI §9.7 | LEP/SLD 精确测量（PDG 2022） | ✅ 已确证 | **高**——在登记参数基线内给出规范耦合 |
| **A2** | QCD 禁闭参数谱推导：$\Lambda_{\text{QCD}}=76$ MeV（示意）、$\langle\bar{q}q\rangle=-(275\text{ MeV})^3$（2%）、$T_c=153$ MeV（1.1%，使用外部 210 MeV） | §8.3，Paper XVII §12 | 格点 QCD（HotQCD/BMW）、实验 PDG | ✅ 已确证 | **高**——$\partial\mathbf{Rec}_D$ 边界穿越统一框架 |
| **A3** | PGW 红外极限还原标准暴涨：$P_T(k)$ 在 CMB/LIGO/LISA 频段修正 $<10^{-100}$，$r=-8n_T$ | §12.2-12.4 | Planck 2018、BICEP/Keck 2021（$r<0.036$）、LIGO O3、NANOGrav 15yr | ✅ 已确证 | **高**——LQG/弦论预言可能可观测偏离，SQG 要求红外精确 GR |
| **A4** | BH 熵谱公式：$S_{\text{BH}}^{\text{spec}}/(A/4G)=0.999952$（$k_{\max}=32$） | §5.2，Paper VIII | EHT M87* 阴影（强引力检验）、LIGO ringdown 无亏损 | ✅ 已确证 | **中**——在 $k_{\max}$ 模型选择下的第一性原理推导 |
| **A5** | Page 曲线：$\tau_{\text{Page}}=0.5\tau_{\text{evap}}$，信息守恒 | §7.2，Paper VIII | 信息悖论理论约束（Page 1993 幺正演化） | ✅ 已确证 | **高**——复现 Page 1993 幺正演化结果 |
| **A6** | 弱等效原理谱表述：$m_{\text{inertial}}=m_{\text{gravitational}}$（谱交织条件导出，非假设） | Paper XVII §12.7 | MICROSCOPE（$\eta<10^{-15}$）、Eöt-Wash（$\eta<10^{-13}$） | ✅ 已确证 | **高**——结构一致性结果 |
| **A7** | 暗物质遗迹密度：$\Omega h^2=0.12$ | Paper XVII §9 | Planck 2018（$0.1199\pm0.0012$） | ✅ 已确证 | **高**——在登记参数基线内 |
| **A8** | CKM/PMNS 矩阵：$\|V_{us}\|=0.2239$（0.19%）、$\theta_{13}^{\text{PMNS}}=0.1505$ rad（0.3%）、$\delta_{\text{CP}}=1.180$ rad（1.6%） | Paper XVII §7-8 | PDG、T2K、NOvA、Daya Bay | ✅ 已确证 | **高**——含 $d_H$ 登记参数 |

**B. 需未来实验检验**

| # | 预言 | 来源 | 预期可检验时间尺度 | 可检验状态 | 理论区分力 |
|:-:|:----|:----|:-----------------|:----------:|:---------:|
| **B1** | PGW Planck 能标谱偏离：$k\sim\Delta\lambda_{\min}$ 处 $P_T(k)$ 偏离 $1/k^2$ 超过 $10\%$ | §12.5（P2） | 高频 GW 探测器（$>10$ yr） | ⬜ 待检验 | **高**——$k_{\max}=8$ 模型选择下的谱截断预言 |
...
《元通用不动点函子范畴框架》系列论文 XII（增强版 v1.8），谱量子引力——传播子、散射与黑洞——在 $\mathbf{Rec}/\mathbf{Sp}$ 范畴框架下建立谱量子引力（SQG）。v1.8 按 RAP v0.1 修复工程：将 $k_{\max}=8$ 从"Cl(1,7) Bott 分类唯一锁定"改为"模型选择"；将 Page 曲线从"谱动力学推导"改为"复现 Page 1993 结果"；将 A 类预言表中的"零参数"表述改为"登记参数基线内"或"$k_{\max}$ 模型选择下"；摘要中"无参数谱截断"改为"依赖 $k_{\max}=8$ 模型选择"。v1.7 将 §12.6 可检验预言扩展为 SQG 全体 14 项预言表。6 核心脚本 44/44 检查通过。

**变更记录**：
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.8 | 2026-07-27 | **RAP v0.1 修复工程**：（a）摘要停用"无参数谱截断"，Page 曲线改为"复现 Page 1993"；（b）§5.3/§11.7 将 $k_{\max}=8$ 从"Cl(1,7) Bott 分类"改为"模型选择"；（c）§7.2 Page 曲线标注为复现而非推导；（d）§12.6 A/B 预言表停用"零参数"表述，改为登记参数基线或 $k_{\max}$ 模型选择。 |
...
| v1.9 | 2026-08-24 | 更名：UFPF → MUFPF（1 处替换）|
