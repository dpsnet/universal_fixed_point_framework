...
**定义 6.1**（静默层级的范畴对应）。$\mathbf{Spec}$（及其拓展 $\mathbf{Spec}_2, \mathbf{Spec}_\infty$）的 4-层次范畴结构自然生成 4 层谱静默：
...
**定理 6.2**（4 层静默的必然性）。$\mathbf{Spec}$ 是一个**严格 4-范畴**（strict 4-category），其 4 层态射结构必然产生 4 层独立的谱静默机制。
...
**推论 6.2**（层数=4 的唯一性）。若 $\mathbf{Spec}$ 是严格 $n$-范畴，则静默层数等于 $n$。由于谱动力学中最高需要 $\infty$-范畴（Paper 35）但物理上有意义的静默在 $n=4$ 处饱和，静默层数为 4 是唯一可能。
...
**定理 6.4**（$\Lambda$ 的多重静默公式）。宇宙学常数 $\Lambda$ 由以下谱公式唯一确定：
