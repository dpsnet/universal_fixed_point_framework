...
**版本**：v0.6 草稿（2026-08-08，未纳入 RAP 登记）
...
*可证伪检验*：若极端均匀样品（$D\to2$）的门限压力不发散、偏离双曲，或 H2 破缺（$a$–$D$ 强相关使双曲拟合显著劣化），预测即被否定。当前 M11 已支持，$D\to2$ 端待低复杂度样品检验。
...
*可证伪检验*：五项中唯一纯数据外推、最易被证伪——多盆地回归若过原点（截距→0），或低 TOC 端偏离线性，预测即被否定。
...
- 本稿为 v0.6 草稿：验证通过后按流程纳入 RAP 论文状态表并提炼正式版本。
