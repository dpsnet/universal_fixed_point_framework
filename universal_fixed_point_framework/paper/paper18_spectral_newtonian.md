...
2. **$\hbar$ 的范畴起源【2026-08-15 分解判定】**：谱间隙 $\Delta\lambda_{\min}$ 到物理质量 $m = \hbar/\Delta\lambda_{\min}$ 的转换中，$\hbar$ 是否是 $\mathbf{Sp}$ 4-范畴的内蕴结构常数？能否从范畴结构导出 $\hbar$ 的数值？**分解为三部分判定**（见笔记 `notes/04_lorentz_gravity/natural_unit_gauge_fixing.md` §5a/§5b，推论 H1）：**① 数值部分——已判定闭合**：$\hbar$ 的量纲数值（$1.054\,571\,817...\times10^{-34}$ J·s）是单位制规范依赖的，经单位重标度 $\lambda_M = \hbar_0\lambda_T/\lambda_L^2$ 可规范化为 1（规范固定，非动力学定理），不可从范畴导出——与 $G_N$、$c$ 的 SI 数值同类（外部测量比值，paper44 §3.5 / paper35 §6.2）；**② 无量纲结构——已闭合**：$\dfrac{m}{M_{\mathrm{Pl}}} = \Delta\lambda_{\min} = \dfrac{\sqrt{6}-\sqrt{2}}{\sqrt{72}} \approx 0.122$（引理 5.1 + paper20 定理 6.1），完全范畴确定；**③ 角色部分——已定位推进（存在性仍公设）**：$\hbar$ 的量纲角色（作用量）由量子谱流方程的量纲一致性唯一确定（对易子能量² → 演化率能量/时间）；谱惯性中 $\hbar$ 的出现为 de Broglie 关系 $p=\hbar k_0$（$k_0=mv/\hbar$）的转写；量子化 = 谱流方程的对易子提升，非对易性为经典谱框架既有结构（paper10 推论 C1.1）。**剩余缺口**：量子化存在性（为何物理用非对易算子代数）——公设级，登记开放。**登记语义**：开放问题 2 的数值子问题闭合（量纲数值非推导对象）、角色子问题已定位推进（量纲唯一 + de Broglie 转写），量子化存在性保持公设。
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.4 | 2026-08-15 | 开放问题 2 角色部分（量子化起源）定位推进：ħ 量纲角色由量纲一致性唯一确定、谱惯性中 ħ = de Broglie 转写、量子化 = 对易子提升（note `natural_unit_gauge_fixing.md` §5b）；量子化存在性保持公设级 |
| v1.3 | 2026-08-15 | 开放问题 2（ħ 的范畴起源）分解判定：数值部分判定闭合（量纲数值为规范依赖，推论 H1 规范固定，note `natural_unit_gauge_fixing.md` §5a）、无量纲部分已闭合（m/M_Pl=Δλ_min）、角色部分（量子化起源）登记开放 |
