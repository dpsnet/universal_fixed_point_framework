# 通用不动点范畴框架 XXVII：Leaver 谱丛理论——三参数纤维化、奇异纤维分类与耗散范畴嵌入
...
本文提出谱丛理论作为理解 Leaver 三对角矩阵族几何本质的统一框架。该理论建立在以下三个核心洞见之上：
...
2. **同伦延拓是谱叶的平行移动**：Leaver 求解器中的 $a$-同伦延拓和 $m$-同伦延拓，在谱丛语言中分别对应谱叶沿 $a$ 方向和 $m$ 方向的平行移动。这种视角将 Newton 迭代的收敛性与谱丛的截面选择统一为几何问题。

3. **非物理根对应谱叶间在分支点的跳跃**：当同伦延拓路径穿过谱丛分支点时，连续截面可能跳跃到非物理叶上，这正是非物理根吸引域的几何起源。分支点密度越高的参数区域，非物理根出现的频率越高。

在此基础上，本文作出七大贡献：

**贡献 I——三参数谱丛的严格定义**（§2）。将 Kerr 三参数空间 $(a,m,\omega)$ 上的三对角矩阵族构造为三参数谱丛 $\mathfrak{S}$，给出三重纤维积定义，建立子谱丛族分类。
...
**贡献 IV——Rec_diss 范畴嵌入**（§5）。将谱丛理论的范畴基础从 $\mathbf{Rec}$ 扩展至 $\mathbf{Rec}_{\mathrm{diss}}$，引入 $D_{\mathrm{diss}}$ 函子，构造谱丛与耗散范畴的严格函子 $F_{\mathrm{diss}}: \mathfrak{S} \to \mathbf{Rec}_{\mathrm{diss}}$，使三类奇异纤维获得范畴论刻画。

**贡献 V——辫子交叉数作为 $D_{\mathrm{diss}}$ 拓扑不变量**（§6）。定义 $D_{\mathrm{diss}}$ 上的辫子交叉数 $\mathrm{Br}(\mathcal{L})$，证明其在谱丛同构变换下的不变性，并建立其与分支点分布密度的定量关系。

**贡献 VI——三层基准体系**（§7）。建立包含（a）三重交换关系数值验证、（b）奇异纤维检测算法、（c）跨系统谱丛同构数值确认的三层基准体系。

**贡献 VII——四个可证伪预言**（§8）。提出四个具有明确数值阈值的可证伪预言，包括超辐射边界 LACI 跃变、高 $l$ 极限谱间隙退化、极值 Kerr 混合奇异行为，以及非引力系统三参数单值群结构的存在性。

本文与 Paper I §7.11 的关系如下：Paper I §7.11 首次提出了谱丛概念和二叉树纤维化结构，给出了定性描述和数值验证。本文在此基础上做三方面的严格化：其一，将 Paper I 中的单参数 $\omega$-谱丛 $\mathcal{S}(M)$ 扩展为完整的三参数谱丛 $\mathfrak{S}$，建立三重纤维积结构；其二，将 Paper I 中定性的单值群讨论提升为严格的交换关系定理和群扩张分类；其三，将 Paper I 中对分支点的经验观察系统化为奇异纤维三分定理，给出完备的分类体系。因此，Paper I §7.11 是本文的**定性前驱**，本文是其在三参数空间上的**严格数学展开**。

---

## 2. 三参数谱丛

### 2.1 Leaver 三项递推的谱丛表示
...
谱丛 $\mathfrak{S}$ 在参数空间 $\mathcal{P} = \mathbb{C}_a \times \mathbb{C}_m \times \mathbb{C}_\omega$ 上的纤维化结构需考虑三个参数之间的耦合关系。

**定义 2.2**（子谱丛）。沿单一参数方向固定另两个参数，得到三类子谱丛：
...
其中 $\mathcal{S}_\omega(a; m)$ 是 Paper I 中定义的单参数 $\omega$-谱丛 $\mathcal{S}(M)$ 在三参数空间中的自然推广。

三个参数对谱丛结构的影响有着本质区别：

**(a) $a$ 依赖**。自旋参数 $a$ 通过三条路径影响谱丛结构。首先，视界位置 $r_\pm = M \pm \sqrt{M^2 - a^2}$ 依赖 $a$，通过视界表面引力 $\kappa = (r_+ - r_-)/(2(r_+^2 + a^2))$ 进入 $\gamma_n(\omega)$ 系数。其次，角度特征值 $\lambda_{slm}(a,m)$ 是 $a$ 的缓变函数。最后，自旋修正项 $D_i(\omega)$ 中的 $a$ 依赖通过 Kerr 度量的非对角项引入。

**(b) $m$ 依赖**。磁量子数 $m$ 主要出现在 $\beta_n(\omega)$ 中 $\sigma_+$ 的分子项 $(-am)$。$m$ 通过角度特征值 $\lambda_{slm}(a,m)$ 的 $m^2$ 依赖间接影响谱丛的对称性。特别的，$m$ 的符号变换 $\omega \to -\omega^*$ 对应谱丛的对偶对称性。

**(c) $\omega$ 依赖**。复频率 $\omega$ 的依赖最为复杂——所有三个系数 $\alpha_n,\beta_n,\gamma_n$ 都通过不同方式依赖 $\omega$：$\alpha_n$ 显式依赖于 $\omega$（通过 $\nu_0$），$\beta_n$ 包含 $\omega^2$ 项，$\gamma_n$ 包含 $-2i\omega\kappa$ 项。这种**全系数依赖**使得 $\omega$ 方向的谱丛结构最为丰富，也是 $\omega$ 作为内循环变量的几何原因。

**定义 2.3**（三重纤维积）。三个子谱丛沿恒等置放的纤维积定义为：
...
**定理 2.1**（三重纤维积 = 全空间单值群）。三参数谱丛 $\mathfrak{S}$ 在参数空间 $\mathcal{P} = \mathbb{C}_a \times \mathbb{C}_m \times \mathbb{C}_\omega$ 上的完全单值群等于三重纤维积：
...
三参数谱丛 $\mathfrak{S}$ 包含多个有物理意义的子谱丛族。

**定义 2.4**（定 $m$ 谱丛族）。对固定磁量子数 $m$，定义谱丛族：

$$\mathfrak{S}^{(m)} = \{(a,\omega,\lambda) : \det(M_{a,m}(\omega) - \lambda I) = 0\}$$

底空间为 $\mathbb{C}_a \times \mathbb{C}_\omega$。这是 Leaver 求解器中实际使用的谱丛结构——$a$ 和 $\omega$ 为变量，$m$ 为固定参数。

**定义 2.5**（物理根截面）。在谱丛 $\mathfrak{S}$ 上，物理根对应满足 $\lambda = 0$ 的截面：
...
**推论 2.1**（物理根截面与子谱丛的关系）。对固定 $(a,m)$，物理根截面与 $\omega$-子谱丛 $\mathcal{S}_\omega(a;m)$ 的交集 $\Sigma_{\mathrm{QNM}} \cap \pi_\omega^{-1}(a,m) = \{\omega : 0 \in \sigma(M_{a,m}(\omega))\}$ 正是 Leaver 连分数条件 $\det M_{a,m}(\omega) = 0$ 的解集。

**定理 2.2**（$m=0$ 约化）。当 $m=0$ 时，谱丛 $\mathfrak{S}$ 退化为 $a$-$\omega$ 双参数谱丛，$m$ 方向消失：
...
这是 Leaver 求解器中 $a$-homotopy 策略（先沿 $a$ 从 0 延拓到目标值，固定 $m=0$）的谱丛基础。

**证明**。$m=0$ 时角度特征值 $\lambda_{sl0}(a,0)$ 退化为关于 $a$ 偶函数，且递推系数 $M_{a,0}(\omega)$ 中的 $(-am)$ 项消失。因此 $\omega$ 截面关于 $m$ 的依赖消失，谱丛从三参数退化为双参数。$\square$
...
**定理 3.1**（三重单值群交换关系）。三参数谱丛 $\mathfrak{S}$ 的三个单值群满足以下交换关系：
...
**步骤 2**（超辐射边界）。超辐射放大的临界条件 $\omega = m\Omega_H$（其中 $\Omega_H = a/(2Mr_+)$ 为视界角速度）处：QNM 频率越过 $\mathrm{Im}(\omega) = 0$ 线，从阻尼振荡变为放大模式。在临界点处，谱丛的零点截面 $\Sigma_{\mathrm{QNM}}$ 与 $N$ 个谱叶中的两个发生"触碰"——$\lambda = 0$ 不再是孤立截面。这等价于谱静默条件激活。

**步骤 3**（包含关系）。因此所有超辐射临界点（$\mathrm{Re}(\omega) = m\Omega_H$）和 $\mathrm{Im}(\omega) \to 0$ 的准束缚态都属于 $\mathcal{P}_{\text{II}}$。$\square$

**推论 4.1**（II 型奇异点的物理意义）。II 型奇异纤维不是数值异常，而是物理相变的谱丛表现——对应黑洞从"稳定阻尼振荡"到"超辐射放大"的边界。在该边界处，物理根定义不再唯一，LACI 判据自然失效。
...
2. **高角量子数极限**（$l \gg 1$）：eikonal 近似下 QNM 频率退化为光线轨道频率，谱丛的 $N$ 个特征值趋于连续统，$\gamma \propto 1/l$。
...
**命题 4.2**（零谱间隙 = 二叉树根部合并）。在谱丛的二叉树纤维化（Paper I 定理 7.39）中，$\gamma = 0$ 等价于以下条件之一：
...
**定理 4.5**（奇异纤维三分定理）。三参数谱丛 $\mathfrak{S}$ 的奇异纤维严格分为三类——I 型（分支交叉）、II 型（谱静默边界）和 III 型（零谱间隙退化）——互斥且全覆盖。具体地，对任意参数点 $p = (a,m,\omega) \in \mathcal{P}$，$\mathfrak{S}_p$ 属于且仅属于以下四类之一：
...
且每个子范畴在谱丛同构 $\mathcal{S}_{\text{Teuk}} \cong \mathcal{S}_{\text{Rheo}} \cong \mathcal{S}_{\text{NRG}} \cong \mathcal{S}_{\text{Mem}}$（Paper I 定理 7.49）下保持不变。

**证明**。三类奇异纤维的定义仅依赖于特征值关系 $\lambda_i = \lambda_j$、末位条件 $\det M = 0$ 和谱间隙 $\gamma$，这些条件均在谱丛同构下保持不变。因此三分定理在跨系统同构下自动继承。$\square$

**物理对应汇总**。三类奇异纤维与 Kerr QNM 谱丛的对应关系为谱丛理论应用于实际物理计算提供了分类依据：正则纤维对应常规 QNM 求解，I 型对应双重同伦延拓中的根切换，II 型对应超辐射临界，III 型对应极值极限下的收敛退化。
...
*证明要点*。由 Phase 58F 双重同伦定理，分步延拓中每一步的 Newton 迭代保持物理截面连续性（Kantorovich 条件保证收敛半径有限）。在 Koopman 算子层面，$U(a)$ 随 $a$ 连续变化，此为谱丛代数曲线的标准性质。若步长 $\Delta a$ 足够小，$U(a_2)$ 是 $U(a_1)$ 的小扰动，伪谱的 Lipschitz 依赖性给出 $C_{\text{hom}}$ 的界。双重延拓（$a$ 和 $m$）的复合态射仍为 $\mathbf{Rec}_{\text{diss}}$ 中的态射。∎
...
- $\mathbf{Rec}_{\text{fib}}$（纤维化范畴）：用谱丛纤维代替单一 Koopman 算子，适用于分支点密集区的平均化处理。
...
1. **谱间隙** $\gamma$：
   ...
   其中 $\lambda_{\max} = \max_i |\lambda_i|$ 为最大模特征值，$\lambda_{(2)}$ 为第二大模特征值。谱间隙度量谱丛中主导特征模的隔离程度。
...
1. **$m=0$ 路径全部 $k=0$**：$m=0$ 的 QNM 频率随自旋变化较小（$\operatorname{Re}(\omega)$ 从 0.374 单调降至 0.310，仅约 17% 的变化），谱叶在参数变化中保持相对顺序，未出现置换交叉。零交叉本身携带有意义的物理信息——它表明 $m=0$ 的谱丛结构在自旋参数空间中是平坦的。
...
在产生非平凡辫子结构（$k>0$）的参数区，$k$ 与 $\gamma$ 的 Spearman 相关系数 $\rho_s = 0.9177$（$p = 0.028$），通过显著性水平 $\alpha = 0.05$ 的检验。$k=0$ 的参数区间（弱谱变化）不携带有意义拓扑信息，属于预期行为——这些路径对应的谱丛平凡丛没有置换结构。
...
1. **谱丛拓扑结构的诊断**：$k$ 值的变化揭示了谱丛拓扑结构随参数变化的转变。$m=0$ 路径 $k=0$ 对应平凡丛（无谱叶交叉），$m=2$ 路径 $k>0$ 对应非平凡丛（存在谱叶重排）。这种拓扑转变是 Kerr 参数空间中谱丛结构的本质特征，与 QNM 频率的物理行为（$m=0$ 频率变化小、$m=2$ 频率变化大）一致。

2. **临界现象的拓扑预警**：$k$ 值的突变与物理临界现象对应。路径 1 ($a=0.30\to0.55$，$m=2$) 中 $k=116$ 的急剧增加对应超辐射边界附近谱叶密度最大区的进入；路径 4 ($a=0.85\to0.95$，$m=2$) 中 $k=408$ 对应极端自旋极限下谱丛拓扑结构的剧烈重整化。这种对应关系表明辫子交叉数可能作为物理相变的拓扑预警信号。
...
Leaver 连分数求解器的数值误差来源于多个独立机制。为系统评估谱丛理论的数值预言，需首先建立误差的分解框架。
...
**定义 7.2**（L1 层解析基准）。在 Schwarzschild 极限 $a=0$ 下，Kerr QNM 退化为 Schwarzschild QNM，其频率在给定 $(l,m,s)$ 下存在高精度解析参考值（Berti et al. 2006, 2009）。这些参考值构成 L1 层基准，用于验证谱丛数值实现的正确性。
...
**推论 7.1**（L2 层作为跨实现验证）。L2 层基准不依赖于单一数值实现的精度，而是验证两种独立实现（多项式形式 vs. 原始 Leaver 形式）之间的自洽性。偏差 $< 10^{-8}$ 确认了谱丛理论的数值实现与现有标准的一致性。
...
**推论 7.2**（三层基准体系的层次关系）。L1 层（解析基准）提供极限下的精确验证，L2 层（数值基准）提供中等参数范围的跨实现一致性验证，L3 层（收敛自洽基准）提供无外部参考时的内禀精度验证。三层基准构成完备的数值验证体系：L1 验证正确性，L2 验证兼容性，L3 验证收敛性。谱丛理论的数值预言在三层基准全部通过后方可确认。
...
**定义 8.1**（谱间隙标度律）。Kerr QNM 谱丛的谱间隙 $\gamma(a)$ 定义为基模与第一泛音之间的最小特征值间距：
...
**证明思路**（谱丛分支点几何论证）。谱间隙由谱丛中最近的两个特征值决定。在 $a \to 1$ 极限下，Kerr 时空的视界表面引力 $\kappa = (1 - a^2)^{1/2} / (2(1 + \sqrt{1-a^2}))$ 以 $\kappa \propto (1-a)^{1/2}$ 趋零。三对角矩阵 $M_{a,m}(\omega)$ 的次对角元 $\gamma_n(\omega) \propto \kappa$，因此 $M$ 在 $a \to 1$ 时趋于对角占优。对角占优矩阵的特征值间距受 Weyl 不等式约束：
...
**定义 8.2**（LACI 三段演化）。Kerr ringdown 的 LACI（谱丛谱间隙倒数）在时间域中呈现三段式演化模式：
...
四个预言共同构成谱丛理论的实验检验方案：P1 检验谱间隙的全局标度行为，P2 检验 LACI 的时间域动力学，P3 检验 LACI 对超辐射临界点的响应，P4 检验 LACI 对参数扰动的稳定性。四者通过即确认谱丛理论在 Kerr QNM 问题中的有效性，任一被证伪则指向理论修正方向。

---

## 9. 多耦合谱丛推广

本文前八章专注于单自旋 $s=-2$（引力扰动）Teukolsky 方程的三项递推谱丛 $\mathfrak{S}^{(s=-2)}$。真实黑洞物理涉及多个自旋场的耦合系统：Kerr-Newman 黑洞中引力扰动（$s=\pm2$）与电磁扰动（$s=\pm1$）在背景电磁场下耦合，EMRI 辐射反作用问题需同时处理多极多自旋扰动模式。本章将谱丛理论从单自旋推广至多自旋耦合系统。

### 9.1 多自旋联合谱丛与块三对角构造

**定义 9.1**（多自旋联合谱丛：无耦合情形）。对自旋指标集合 $S = \{s_1, s_2, \dots, s_k\}$，无耦合时联合谱丛定义为各单自旋谱丛的**纤维积**（fibered product）：
...
**定义 9.2**（耦合修正）。当存在耦合时，联合谱丛需由**耦合参数族** $\{M^{(s_i,s_j)}_{a,m,\omega,Q}\}$ 构造：
...
**定义 9.3**（耦合项的纤维联络解释）。在谱丛几何中，耦合项自然地编码为**纤维之间的联络形式** $\omega^{(s_i,s_j)}$：
...
- **强耦合**（$|Q| \sim M$）：联络曲率 $R^{(s_i,s_j)} = d\omega^{(s_i,s_j)} + \omega \wedge \omega$ 不可忽略，谱丛退化为真正的**编织谱丛**

**命题 9.1**（平凡化准则）。多自旋联合谱丛可完全分离（退化为各单自旋谱丛的直积）当且仅当存在规范变换 $U$ 使得：
...
### 9.4 Chandrasekhar 变换的谱丛解释

**命题 9.3**（Chandrasekhar 变换的自旋规范解释）。Chandrasekhar（1975-1983）建立的变换理论是多自旋谱丛之间的**自旋规范变换**：
...
在谱丛语言中，Chandrasekhar 变换将一个自旋权重的谱叶映射到另一自旋权重的谱叶，**不改变特征值的谱集**。这暗示了多自旋谱丛存在自然的"水平连接"——联合谱丛 $\mathfrak{S}^{(S)}$ 的纤维通过 Chandrasekhar 变换态射形成非平凡的单值群结构。
...
**猜想 9.1**（耦合耗散函子稳定性）。对 Kerr-Newman 背景（$|Q| < M$），存在耦合系统上的 $D_{\mathrm{diss}}^{\text{(coupled)}}$ 函子，使得谱丛的耗散结构在 $Q$ 的连续形变下保持稳定，退化仅发生在临界电荷 $Q_c$ 处。
...
- **路径 P1——电磁谱丛**（Paper XXVII §12）：将谱丛框架从引力扰动（$s=-2$）扩展至电磁扰动（$s=\pm1$），包括递推系数参数化、电磁 QNM 精度验证、LACI 跨自旋对比和奇异纤维扫描。Teukolsky-Starobinsky 恒等式保证 $s=+1$ 与 $s=-1$ 同谱，仅需实现 $s=-1$ 即可覆盖全谱。
- **路径 P2——Kerr-Newman 耦合谱丛**（Paper XXVIII）：将引力（$s=\pm2$）与电磁（$s=\pm1$）的 Chandrasekhar 耦合方程离散化为块三对角谱丛，引入 $Q$ 参数纤维延拓，分类 IV 型（耦合融合）奇异纤维，证明四型互斥完备性。
- **路径 P3——Dirac 半整数自旋谱丛**（Paper XXIX）：推广至半整数自旋（$s=\pm1/2$），引入自旋结构（spin structure）和 $\mathbb{Z}_2$ 阻碍，构造 Dirac-引力张量积联合谱丛 $\mathfrak{S}^{(-2)\otimes(-1/2)}$。
...
其中：P1 为 P2 提供耦合递推所需电磁系数实现；P2 为 P3 提供耦合谱丛的块矩阵构造模板；三者的 LACI 跨自旋对比（$\gamma_{\mathrm{D}} > \gamma_{\mathrm{EM}} > \gamma_{\mathrm{G}}$ 预期排序）构成对谱丛理论普适性的统一检验。

---

## 10. 全局存在性定理

谱丛理论目前关注局部结构（单一参数点的纤维、单一方向的单值群）。Leaver 连分数法全局存在性的根本问题是：解析方程 $\det M_{a,m}(\omega) = 0$ 的解曲线在复 $\omega$ 平面上是否存在唯一的物理 Riemann 面结构？该问题源于三个相互交织的困难——连分数截断误差的极限过渡、分支割附近解析延拓的唯一性、以及无穷多高泛音模式的存在性证明。
...
### 10.2 谱丛视角的 Reinterpretation

在谱丛框架下，全局存在性问题获得了几何解释：
- 物理 QNM 根 = 谱丛截面 $\mathfrak{S}_{a,m} = \{(\omega,\lambda): \det(M_{a,m}(\omega) - \lambda I) = 0\}$ 在零特征值叶上满足 $\text{Im}\,\omega < 0$ 的离散零点集
- 分支点（I 型奇异纤维）$\partial\det M(\omega)/\partial\omega = 0$ = 连分数尾部发散条件，在分支点 $\omega_0$ 附近连分数收敛圆半径由 $|\omega - \omega_0|^{1/2}$ 控制
- 全局存在性 = 谱丛截面延拓的唯一性：只要路径不穿过 II 型奇异纤维（超辐射静默边界），截面的连续延拓唯一

**命题 10.1**（LACI-全局存在性对应）。谱丛概念、LACI 数值诊断与全局存在性含义的三层对应关系为：
...
**路径 3**（远期，6-12 月，高难度）——**谱丛截面全局延拓唯一性的泛函分析证明**。将连分数映射转换为 Hilbert 空间上的算子族谱问题，证明该族在物理参数区域内是解析 Fredholm 族，应用 Kato-Rellich 定理（解析 Fredholm 族特征值在紧算子扰动下解析），推论只要路径不经过非正则点，特征值的解析延拓唯一。为 LACI 判据和双重同伦延拓提供严格的数学基础。
...
**命题 11.1**（八大贡献总结）。本文在通用不动点范畴框架下，建立了 Leaver 谱丛理论的完整数学体系，作出以下八大贡献：

1. **三参数谱丛的严格定义**（定义 2.1–2.3）：将 Kerr 黑洞三参数空间 $(a,m,\omega)$ 上的三对角矩阵族构造为三参数谱丛 $\mathfrak{S}$，建立三重纤维积结构，给出子谱丛族分类和物理根截面 $\Sigma_{\mathrm{QNM}}$ 的几何刻画。
...
8. **电磁谱丛框架（§12）**：将谱丛理论扩展至电磁扰动 $s=\pm1$，建立电磁谱丛 $\mathfrak{S}^{(s=-1)}$ 的严格数学定义，证明电磁 Teukolsky-Starobinsky 同谱性定理（定理 12.1），推广奇异纤维三分法至电磁情形（命题 12.2），建立跨自旋 LACI 对比框架。

### 11.2 展望方向

基于本文建立的谱丛理论，以下方向具有明确的研究前景。

**$\infty$-范畴提升方向**。谱丛理论的 $\infty$-范畴化可使谱叶之间的高阶同伦信息获得完整的范畴论编码。已在预研中建立 $\infty$-层化路径的概念框架（将三类奇异纤维视为不同维数的层），待解决的核心问题是 $\infty$-层范畴 $\mathbf{Strat}_\infty$ 中粘合函子的严格构造。

**多耦合推广的数值实现方向**。第 9 章已建立多自旋联合谱丛的数学框架、IV 型奇异纤维的分类和 $D_{\mathrm{diss}}^{\text{(coupled)}}$ 函子的扩展方案。后续需推进的方向包括：耦合项 $C^{(s_i,s_j)}$ 的显式离散化与数值实现，四重参数 $(a,m,\omega,Q)$ 下单值群 $\mathcal{M}_Q$ 与已有单值群的换位关系计算，以及 $s=\pm1$ 电磁谱丛的参数化与 LACI 验证（近期）、$s=\pm2$ 与 $s=\pm1$ 联合谱丛构造（中期）、Dirac $s=\pm\frac12$ 半整数自旋谱丛（远期）的三阶段实施。
...
本章将谱丛框架从引力扰动（$s=-2$）扩展至电磁扰动（$s=\pm1$），建立电磁谱丛 $\mathfrak{S}^{(s=-1)}$ 的严格数学定义，证明其与 §2-8 的谱丛理论具有完全兼容的纤维化结构和奇异纤维分类。本章内容构成 §9.7 路径 P1 的理论基础，数值实施部分将在后续工作中完成。

### 12.1 电磁 Teukolsky 方程的谱丛表示
...
与引力扰动（$s=-2$）对比，三组递推系数的参数差异决定了谱丛几何的根本不同：
...
电磁谱丛的一个关键简化来自 Teukolsky-Starobinsky（TS）恒等式。

**定理 12.1**（电磁 TS 同谱性）。$s=+1$ 与 $s=-1$ 的电磁谱丛具有完全相同的 $\omega$-零点集：
...
**推论 12.1**。电磁谱丛的数值实现只需处理 $s=-1$ 的递推系数，$s=+1$ 的谱可通过 TS 恒等式直接获得。这使待实现的递推系数数量减半。

### 12.3 电磁谱丛的纤维化与子谱丛

电磁谱丛 $\mathfrak{S}^{(-1)}$ 继承 §2 中建立的所有纤维化结构：
...
- **$m=0$ 约化**（定理 2.2）：当 $m=0$ 时谱丛退化为 $a$-$\omega$ 双参数结构

**命题 12.1**（电磁谱丛的交换关系）。电磁谱丛 $\mathfrak{S}^{(-1)}$ 的三个单值群满足与定理 3.1 完全相同的交换关系：
...
电磁谱丛的奇异纤维分类完全继承定理 4.5 的三分法，但具体分布和定量标度因自旋不同而异。

**命题 12.2**（电磁奇异纤维的三分法推广）。电磁谱丛 $\mathfrak{S}^{(-1)}$ 的奇异纤维严格分为三类：
...
1. **分支点密度**：由于 $\nu_0^{(-1)} = -1$ 使递推迟收敛，电磁谱丛的截断 $N_{\text{max}}$ 更大（预期 $N_{\text{max}} \sim 80$ vs 引力的 $N_{\text{max}} \sim 50$），因此分支点密度高于引力
2. **超辐射阈值**：电磁 QNM 的超辐射更易发生——预期 $a_{\text{crit}}^{\text{(EM)}} < a_{\text{crit}}^{\text{(G)}}$，具体值需数值确定
3. **谱间隙标度**：在 $a \to 1$ 极限下，预期 $\gamma^{(-1)}(a) \propto (1-a)^{\beta_{\text{EM}}}$，标度指数 $\beta_{\text{EM}}$ 的数值确定是关键任务

### 12.5 电磁 LACI 参数与跨自旋对比框架

电磁谱丛的 LACI 参数定义与 §4 完全一致：
...
跨自旋对比是检验谱丛理论普适性的关键——如果谱丛理论对所有自旋成立，则 $\gamma$、$\Delta\lambda$、disp 的定性行为（正则纤维处 LACI < 1、分支点附近 LACI 尖峰、超辐射边界 LACI → ∞、高泛音极限 $\gamma \to 0$）应对所有 $s$ 一致，仅定量参数不同。

### 12.6 截断误差与收敛性

电磁谱丛的截断误差分析与 §7 一致。对 $N$ 截断，误差指数衰减：
...
**[备注：本章提供了电磁谱丛的完整数学框架。数值实施——包括 §12.4 中奇异纤维标度指数 $\beta_{\text{EM}}$ 的数值确定、§12.5 中 LACI 参数的系统计算、以及与 Berti (2006) 基准表的精度验证——将在论文后续版本中完成。]**

---

### 致谢

感谢通用不动点范畴框架项目组全体成员的持续讨论。感谢匿名审稿人对 Paper I §7.11 中谱丛定性描述的建议，促成了本文的严格数学展开。数值计算使用了 SciPy（`linear_sum_assignment`）、NumPy 和 SymPy 开源科学计算库。
...
[9] Paper I (UFPF I, RKHS 收敛率与谱丛基础).
...
*本文是通用不动点范畴框架（Universal Fixed Point Framework, UFPF）系列的第 XXVII 篇。数学公式使用 LaTeX 排版，定理编号与正文连续。全文自包含，不引用 notes/ 或 roadmap/ 等内部文档。电磁谱丛的数值实施见后续版本，Kerr-Newman 耦合谱丛和 Dirac 半整数自旋谱丛将分别作为 Paper XXVIII 和 Paper XXIX 独立发表。*
