...
跨自旋对比是检验谱丛理论普适性的关键——如果谱丛理论对所有自旋成立，则 $\gamma$、$\Delta\lambda$、disp 的定性行为（正则纤维处 LACI < 1、分支点附近 LACI 尖峰、超辐射边界 LACI → ∞、高泛音极限 $\gamma \to 0$）应对所有 $s$ 一致，仅定量参数不同。表 12.1 的标度指数结果直接验证了这一普适性：三个自旋的 III 型奇异纤维均满足幂律标度，仅指数 $\beta$ 因自旋权重不同而异。
