...
**版本**：v2.40（2026-08-16）——LACI 函数族注册同步（A4 注册为 $F_{\text{mt}}$、定理 7.47 分量归属明确化）+ 谱比型关联修补（定义 7.22 同名冲突注、line 180 方向注、定义 7.47 综合静默度 LACI 成员指定、定理 7.47 后语义反转修正"高→低 LACI"），见主文件 §3.6 定义 3.12a
...
- (S3) 局部吸引子捕获指数（Local Attractor Capture Index, LACI）高条件：UV/IR 能标比 $\Lambda_i/\Lambda_{i+1} \gg 1$ 导致谱间隙消失（此处 LACI 为判据族语义：高 = 谱间隙消失 = 静默；与定义 7.22 的"γ≥10 = 谱间隙足够大"方向相反——后者为谱间隙充分性指数、非判据族成员，见主文件 §3.6 定义 3.12a）；
...
**注（2026-08-16 同名冲突修补）**：此处 $\gamma$ 为**谱间隙充分性指数**（$\gamma \geq 10$，大值 = 谱间隙充分 = 可辨识，可随演化参数变化如定理 7.24 表），与定义 7.19 的谱比型 $\gamma = 1-\lambda_2/\lambda_1 \in [0,1]$ **同名"LACI 指数"但公式与量域互不相容**；二者均编码"可辨识度"（大值指示谱间隙充分/结构清晰）、**均非 LACI 判据族成员**（判据族语义为"大值指示静默/不可辨识"，见主文件 §3.6 定义 3.12a）。
...
A4. **LACI 测度论刻画**：$LACI(\mu_\sigma) = -\log(\min_gap)$ 是谱间隙的测度论描述（注册为 LACI 函数族成员 $F_{\text{mt}}$，见主文件 §3.6 定义 3.12a）。
...
**注（2026-08-16）**：此处 LACI 指定为测度论成员 $F_{\text{mt}} = -\log(\min\text{-gap})$（与 A4 定义一致）；若改用操作成员 $F_{\text{op}} = 1/\min\text{-gap}$，须先经 $F_{\text{mt}} = \ln F_{\text{op}}$ 换算（$\min\text{-gap}=0.122$ 时 LACI/20 = 2.1/20 ≈ 0.105，而非 8.2/20 ≈ 0.41；见主文件定义 3.12a）。
...
**定理 7.47**（LACI = 谱丛截面正则性度量）。主文件 §3.6 定义 3.11 的复合 LACI（$F_{\text{comp}}$）三个分量在谱丛语言中对应（分量归属与谱比型的辨析见主文件定义 3.12a）：
...
低 LACI 值意味着截面远离分支点区域，物理根的辨识可靠（复合型 LACI 小 = 残差/分散度小、谱间隙大 = 远离分支点；2026-08-16 语义反转修正，原"高 LACI"笔误）。
