...
## 8. Einstein 散度的显式修正公式（真值路径闭合）

§7.1 证明了恒等式形态 ∇^μ G_μν = 0 在点值骨架中不成立，并指出其真值路径：沿递归系统 step 的有限差分导数 ∂̃_ρ T(x) := T(step_ρ x) − T(x) 重建带 ∂ 项的离散第二 Bianchi。**本章给出该重建的最终闭合结果**（模块 45 定理零 sorry，全库 `lake build` 4078 jobs 通过）。
...
1. **守恒律的离散形态被精确定位**。经典 ∇^μ G_μν = 0 是恒等式型守恒表述，在离散层不成立；定理 8.1 表明离散层的真确表述是修正公式型——守恒律与经典形式的全部偏差被完全定位到移位差分 artifact 上，而这些 artifact 在连续极限下消失。离散理论并未放弃协变守恒，而是守恒律在离散层改变了数学形态。

2. **框架自洽性的闭环**。§7.1 的证伪曾留下"守恒律在骨架中无家可归"的表面缺口；定理 8.1 表明该缺口在表述层级（恒等式 vs 修正公式）而非物理内容——Einstein 方程的自动守恒结构在 MUFPF 离散层完整保留，连续极限回到经典形式。这为"广义相对论是离散范畴结构的连续极限"这一核心论点补上守恒律侧的一环。

3. **下游条件链的根基**。定理 7.1（条件式能动量守恒）的前提由此获得确定真值：恒等式形态为假，其守恒内容由修正公式形态承载——条件式推导链的成立条件被完全刻画，不再是悬置的开放命题。
...
**定理 9.1**（`vacuum_einstein_from_spectral_action`）。设 g 为 MetricTensor，Ric 为 RicciTensor，h_vacuum: scalarCurvature(g, Ric) = 0，h_ricci_zero: Ric.R = 0。则：
...
| **Einstein 散度显式修正公式** | **`einstein_divergence_explicit`（定理 8.1，§8）** | **✅** |
...
3. **量子修正**：Δ 的二阶修正 Δ₂ = δ·(A_Y·δ − δ·A_Y)·δ 已在 Paper XLIX §7.3 建立（‖Δ₂‖/‖Δ₁‖ ~ 10⁻¹²，远低于当前观测精度），对应引力的非线性效应。待建立的是从 Δ₂ 到后牛顿修正的映射。
