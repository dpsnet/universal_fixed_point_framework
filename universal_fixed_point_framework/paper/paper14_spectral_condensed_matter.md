...
**结论层级标注（v1.6）**。按文首三级体系：C1【谱公理】（BCS Hamiltonian 翻译，§2）；C2【谱公理】（TKNN/陈数部分）+【输入-现象学】（ν 过渡的 RG 常数 α、ε₀、γ₂、ζ₀ 为拟合，§3.3-3.5）；C3【内生-范畴】+【谱公理】（GP 生成元构造 $A_{\text{GP}} = -\log\rho$ 的谱对应求逆、谱映射、谱流闭合已机器证明，属内生-范畴层——paper5 §5 模板"子范畴约束 → 生成元"的凝聚态实例；GP-PDE 到谱流方程的翻译仍属谱公理层，ρ 的 PF 不动点存在性开放，§4 标注）；C4【谱公理】（统一论点，框架主张）；C5【谱公理】（谱丛结构翻译，§5.7）；C6【独立数值验证】——非推导，是框架对独立数据的实证应用（§5.8.5 已有诚实边界声明）。**本文由此首次出现【内生-范畴】成分**（C3 的生成元构造链，v1.6 第三轮 `GPEmergence` 闭合）：范畴 Δ 内生谱间隙资产（SU(2) Casimir、Cl(1,7) Bott）经 Paper XX 链进入本文预言（谱预言 6.1/6.4），G1 桥（Δ → 凝聚态生成元）的 GP 切入点已闭合，BCS/Hall 侧仍开放——v1.6 重建的名实结构由此从"全文最高【谱公理】"升级为"生成元构造层已内生"。
