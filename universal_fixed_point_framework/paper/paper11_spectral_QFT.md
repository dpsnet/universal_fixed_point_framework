...
Phase 44 路线图的目标是将已知物理方程用谱语言重写，补齐 UFPF 框架缺失的谱 QFT 数学工具箱。本文是这一路线的完整理论总结与数值验证。
...
| §7 | 谱手性理论与反常消去 |
| §8 | 谱标准模型完整翻译 |
| §9 | 谱 QFT 形式化（LSZ/Cutkosky/光学定理/KL /谱规范的 LSZ /幺正性证明） |
| §10 | 结论与展望 |
...
在谱语言中，轴子是 $\mathbf{Spec}$ 中的周期伪标量对象：$a(\lambda) \cong a(\lambda) + 2\pi f_a$。
...
顶 Yukawa 耦合 $y_t = \sqrt{2}m_t/v = 0.9945$（预期 $\sim 1.0$），轻重子 Yukawa 比 $y_e : y_\mu : y_\tau \propto m_e : m_\mu : m_\tau$。
