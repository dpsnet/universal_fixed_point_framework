...
**推论 5.10**（重味 Cornell 有效参数谱定替代，v0.11）。定理 5.4 的经验有效耦合 $\alpha_s = 0.39$ 由谱框架两圈跨味跑动谱定替代：$\alpha_s(m_c) = 0.419$（谱值 $\alpha_s(M_Z)^{-1} = 8.7$ 起步，61C 独立锚点 $\alpha_s(m_c) = 0.413$、PDG 0.40，偏差 1.5%/4.8%）。经验值 0.39 获谱框架来源——两圈反解其有效标度 $\mu_{\mathrm{eff}} = 1.39$ GeV $\approx m_c$（1.27–1.5 GeV 重味标度）。谱定替代后：

| 强子 | 谱定 $\alpha_s = 0.419$ | 经验 $0.39$ | PDG | 谱定偏差 |
|:--|:--:|:--:|:--:|:--:|
| $J/\psi$ | 3.302 GeV | 3.33 GeV | 3.097 GeV | 6.6%（↓0.9pp） |
| $\psi(2S)$ | 3.916 GeV | 3.93 GeV | 3.686 GeV | 6.2%（↓0.5pp） |
| $\Upsilon$ | 9.417 GeV | 9.476 GeV | 9.460 GeV | 0.5%（↑0.3pp） |
| $\Upsilon(2S)$ | 10.023 GeV | 10.050 GeV | 10.023 GeV | **0.0%**（↓0.3pp） |

**4 态平均偏差从 3.66% 降至 3.33%**；径向激发间距 614/606 MeV（PDG 589/563，偏差 4.3%/7.6%，< 20% 保持）。

*证明要点*。① 两圈跨味 RGE（$b_1(N_f)$，RK4 数值积分）给出 $\alpha_s(\mu)$ 表：$\alpha_s(1.27\ \text{GeV}) = 0.419$；经验值反解 $\alpha_s(\mu_{\mathrm{eff}}) = 0.39$ 得 $\mu_{\mathrm{eff}} = 1.39$ GeV。② 谱定值代入定理 5.4 的 Cornell 求解（$m_c = 1.5$、$m_b = 4.8$、$\kappa_{\mathrm{lin}} = 0.18$ GeV² 保持）——$\alpha_s$ 增强使色-Coulomb 吸引更强、能级更负，charmonium 偏差下降（J/ψ 7.5%→6.6%），bottomonium 对 $\alpha_s$ 不敏感（线性项主导）保持 < 1%。□

**数值**（`paperX_qcd_heavy_flavor_spectral.py`，6/6 检查通过，已注册 `run_all_tests.py`）。

*诚实边界*：谱定替代采用**统一标度 $m_c$**（保留 Cornell 单参数结构，最保守替代）；$\alpha_s(m_c)$ 相对 61C 的 0.413 有 1.5% 数值差异（积分实现细节）；$m_c/m_b = 1.5/4.8$ GeV 有效质量仍为输入（裸 MS-bar $1.27/4.18$ GeV 的 dressing 精确化登记为后续）。
...
v0.11 完成 61B 重味参数谱定：**重味 Cornell 有效耦合谱定替代**（推论 5.10，§5.6——经验 $\alpha_s = 0.39$ 由两圈跨味 $\alpha_s(m_c) = 0.419$ 替代，61C 锚点 0.413/PDG 0.40；经验值获谱框架来源：反解有效标度 $\mu_{\mathrm{eff}} = 1.39$ GeV $\approx m_c$；4 态平均偏差 3.66% → 3.33%，J/ψ 7.5%→6.6%、Υ' 0.0%；径向间距 4.3%/7.6% 保持）——重味 Cornell 消除经验耦合锚点，$m_c/m_b$ 有效质量 dressing 登记为开放问题 5。
...
5. **重味有效质量 $m_c/m_b$ 的 dressing 精确化**：定理 5.4 使用 dressing 后有效质量 $m_c/m_b = 1.5/4.8$ GeV（裸 MS-bar $1.27/4.18$ GeV）——$\alpha_s$ 已谱定（推论 5.10，§5.6），有效质量的谱框架 dressing 精确化登记为后续。
...
| v0.11 | 2026-08-05 | **重味 Cornell 有效参数谱定替代（推论 5.10，§5.6）**：经验 α_s = 0.39 由两圈跨味 α_s(m_c) = 0.419 谱定替代（61C 锚点 0.413、PDG 0.40）——经验值获谱框架来源（反解有效标度 μ_eff = 1.39 GeV ≈ m_c）；4 态平均偏差 3.66% → 3.33%（J/ψ 7.5%→6.6%、ψ' 6.7%→6.2%、Υ' 0.3%→0.0%），径向间距 4.3%/7.6% 保持；`paperX_qcd_heavy_flavor_spectral.py` 6/6 注册 `run_all_tests.py`；定理 5.4 诚实边界更新、§8.1 结论更新、§8.2 新增开放问题 5（m_c/m_b dressing 精确化）。 |
