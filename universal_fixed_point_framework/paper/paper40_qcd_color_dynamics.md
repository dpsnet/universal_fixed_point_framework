# 元通用不动点函子范畴框架 XL：SU(3) 色规范完整动力学——色丛、胶子顶点、禁闭渐近自由与强子谱
...
**系列定位**：元通用不动点函子范畴框架（MUFPF）强相互作用谱理论系列
...
2. 势垒/相空间/同位旋/$\alpha_s$ 均为标准强子物理，MUFPF 增量在"静默释放 = 强耦合释放"的定量归属（§5.11 机制 + 时间耦合解绑通道语言，笔记方向 6）；
...
| v0.50 | 2026-08-24 | 更名：UFPF → MUFPF（2 处替换）|
