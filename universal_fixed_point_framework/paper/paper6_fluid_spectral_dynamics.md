...
三者共享同一机制：**最小谱间隙坍缩** $\Delta\lambda_{\min} \to 0$。

**证明**。由 Paper XVI 主定理 8（光锥 = $\partial\mathbf{Rec}_D$）、Paper VIII（黑洞视界 = $\partial\mathbf{Rec}_D$）以及本节主定理 E1（流变硬化 = $\partial\mathbf{Rec}_D^{\text{rheo}}$），三类现象都满足 $\Delta\lambda_{\min} \to 0$。三者都是 $D$ 函子作用下 $\partial\mathbf{Rec}_D$ 边界附近的临界行为，区别仅在递归对象的物理身份与谱流生成元。$\square$
...
《通用不动点范畴框架》系列论文 VI（增强版 v2.2），谱流体动力学——从湍流谱到谱流几何。主要内容：
- 谱流体动力学公理 B1-B3（§2.2）
- N-S 谱流方程（定理 2.1）
