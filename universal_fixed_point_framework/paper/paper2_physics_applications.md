...
- [I] 配套论文 I：《通用不动点范畴框架 I：分形谱化理论》，**v2.46**，2026-07-27。数学基础：范畴论、谱化函子 $D$、$\mathbf{Rec}_D$ 宽子范畴、$D\dashv R$ 显式余伴随构造、四层静默体系、辫子自然同构、隔离约束条件、谱对应自然同构、连续谱测度理论、IFS 热力学形式、Clifford 值谱理论、RKHS 收敛率定理。重要修复：$\mathrm{Cl}(1,7) \cong M_{16}(\mathbb R)$ 修正原 $M_8(\mathbb R)$ 错误；$\rho=0$ 与三代结构均登记为模型输入，不从 $\mathrm{Cl}(1,7)$ 导出。详见《RAP_勘误与立场声明.md》。
