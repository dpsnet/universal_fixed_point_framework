...
**发布日期**：2026-07-30
**版本哈希**：`8c7a06048f41968a00be8d0042297568cacb12a4`（v0.1）→ `5d4bdc215ef422d68961f6605a437dbbefa16426`（v0.2）→ `772d2ef75b`（v0.3）→ `c1afedc9`（v0.4）→ `2bc2a109`（v0.4a）→ `7edd2df4`（v0.4b）→ `71bd2428`（v0.4c）
...
| 参数 | v0.1 性质 | v0.2 性质 | v0.4 性质 |
|:----|:----------|:----------|:----------|
| $d_H$ | 登记参数（1） | **结构约束**：≈ln15（类型计数 + IFS 构造，**机器证明**）+ δ ≈ 0.00145（RMS 定理 ε̄ = √N_total·ε₃ 约束） | **推导值**：ln15 机器证明；δ 由 RMS 定理约束（`DeviationBound.lean` §1.6 + Paper XXXI） |
| $\lambda_{\text{静默}}=-\ln s$ | 登记参数（1） | e⁻¹（信息论最优，定理 R1） | **推导值**（定理 R1：几何级数 + 生成元匹配 + 双重最优性） |
| $N_{\text{gen}}=3$ | 登记输入（1） | **机器证明**（`Unified3Theorem.lean` + `BottTower.lean`） | **推导值**，提炼为 Paper XXXIII |
| 扇区参数 | 拟合参数（6–8） | **Cl(1,7) 推导**：超荷赋值直接导出；α = Δλ_min/4π | **推导值**，提炼为 Paper XXXII（8 定理机器证明） |
| $G_N$ | — | **Phase C 闭式**：$G_N = 18(2+\sqrt{3})\cdot(\Delta\lambda_{\min})^2/M_{\text{Pl}}^2$ | **推导值**（仅含 $M_{\text{Pl}}$ 外部标度） |
| B2 连续极限 | — | — | **✅ 理论闭合**（Paper XXXIV）：分形→$\mathbb{R}^4$ 拟对称嵌入 |
| **合计** | **8–10 个自由度** | **2–3 个**（$M_{\text{Pl}}$ + δ 约束） | **0 个自由参数 + 1 个外部标度 $M_{\text{Pl}}$**（$c=1$ 单位制）。δ 为 RMS 受约束的唯象残差，非可调参数。其闭式推导方向已排除——ε̄/ε₃ = √5 是数值穿越点而非解析极限（参见 Paper §6.4 诚实标注），无法闭式证明 |
...
| **v0.4a** | **2026-07-30** | **开放研究线扩展**：新增 O7（spExchangeLaw `sorry`，🔴 L3 概念特征）和 O8（`DeviationBound` 2 `sorry`，🟡 L2 待 Mathlib）。**等级体系引入**：L1 🟢 自包含 / L2 🟡 待基础设施 / L3 🔴 概念特征。各等级在受影响的论文（Paper XXXI §8、Paper XXXV §7.2）中同步登记 |
| **v0.4b** | **2026-07-30** | **参数总账补充**：δ 参数注记——闭式推导方向已排除（ε̄/ε₃ = √5 是穿越点而非极限）。**Paper XXXVII 新增 §3.4 已排除方向**：X1（ε̄/ε₃ √5 闭式证明）正式登记为 ❌ 负结果闭合 |
| **v0.4c** | **2026-07-30** | **v0.3 残留引用清理**：4 处 RAP-Errata v0.3 → v0.4（Paper XXXV, XXXVII, 盲登记协议, framework README）；参数总账标题/表头 v0.3 → v0.4。盲登记协议同步更新至 v0.4（预言数值不变） |
