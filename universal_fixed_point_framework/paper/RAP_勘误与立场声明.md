# UFPF 勘误与立场声明（RAP-Errata v0.41）

**发布日期**：2026-08-14（v0.41）
**版本哈希**：`d74a10b777`（v0.41 纳入 commit）
...
| **v0.40** | **2026-08-14** | **Paper XLIV WW 复极点 Lean 代数骨架闭合（纯增量，预言数值不变，零声明变更；paper44 v0.31 → v0.32，commit `9190995a23`）**：**§7.3 新增 `PhotonTopologyResonance.lean`（零 sorry，`lake build` 2454 jobs）**——机器证明"下半平面极点 ⟹ 指数衰减"（锚点 1 ③ 的代数核心）：`resonancePoleDecay`（‖e^{−i z t}‖² = e^{2·Im z·t}）+ `resonancePoleDecay_coeff`（极点贡献 |R·e^{−i z t}|² = |R|²·e^{2·Im z·t}）+ `resonanceDecayRate_pos`（Im z < 0 ⟹ γ = −2·Im z > 0）+ `resonancePoleDecay_mono`（Im z < 0 ⟹ 模方随 t 严格递减，无回归）——**锚点 1 ③"不可逆与因果性是同一数学对象（下半平面极点）"获机器证明代数骨架**；诚实边界：极点位置 z_res（第二叶 η_II(z)=0，解析延拓）由 `paperX_friedrichs_resonance.py` 8/8 定量给出，自伴性/Mourre 完整 Lean（无界算子/谱测度库依赖）仍登记开放。同步：FPO v0.102、paper44 §7.3/§7.5 开放问题 7、版本行 v0.32。**Phase 62 预言 P1–P6 数值不变、仍为远期假说**；主框架 P1–P7 冻结预言数值全部不变，冻结不受影响 |
...
| **v0.41** | **2026-08-14** | **光子拓扑形式化推进包（纯增量，预言数值不变，零声明变更；paper44 v0.32 → v0.33，commit `d74a10b777`）**：① **内容域命名重构（tracker 组 H 登记）**——6 通用模块去光子前缀（SpectralSkeleton/CurvatureSkeleton/TwoCategoryLaws/LiftingOrthogonality/ExteriorFunctor/PoleDecay），曲率内容（skew/lie_bracket/curvature_antisymm）迁入 CurvatureSkeleton，纤维正交 12 符号迁出 FiberOrthogonalSkeleton——`lake build` 2454 jobs 零警告零 sorry；② **A4 Mourre 估计 Lean 代数骨架（MourreSkeleton）**——锚点 2 a.c. 谱前提代数核心闭合（mourre_estimate_of_lower_bound：i[H,A]=H + 谱投影像内能量下限 ⟹ Mourre 估计），**A4 锚点 2 三前提（自伴 Kato–Rellich + a.c. Mourre + 位置表示）Lean 代数核心全部闭合**；③ **矩阵层完整字典（DeviationBound §1.9）**——登记开放项 #4 推进：Hilbert–Schmidt 迹内积下对角矩阵空间正交补 = 零对角元矩阵空间（J2 定位从逐条方向提升为补空间表述）；④ **辐射/无辐射跳变通道判据定量化（笔记 §6.6.1）**——三通道（取向门关闭/静默屏障抑制/竞争弛豫）统一量子产率 Φ=A_eff/(A_eff+k_nr)（paperX_channel_quantum_yield.py 5/5 注册）；⑤ **B 类静默抑制预言 vs 标准 X 射线荧光产额对照（诚实张力登记/负结果，paperX_p6_fluorescence_yield.py 5/5 注册）**——框架预言 Φ 随壳层数单调降（K 2.96e-4 > Xe 1.97e-5 > Rn 1.3e-6）vs 标准 K 荧光产额 w_K 随 Z 单调增（K 0.12 < Xe 0.87 < Rn 0.96），**方向完全相反、低 2 个量级以上** ⟹ **盲登记排除线**（以 K 荧光产额检验则 B 类叠加抑制假设被排除）+ **适用域限定候选**（R_supp 作用域或限光学/价层跃迁，不适用内壳层 X 射线）——P6 叠加假设候选形式的方向性检验（诚实负结果登记）；⑥ **检测矩阵对接说明（paper/UFPF_检测矩阵对接对齐说明.md v0.2）**——外部锚点标准化接口（公共模板外部维护 + 项目内同步实例 + 四接口点 + 权限条款：公共层只裁决当前、对未来仅建议）；融合路线规划移至 roadmap/。同步：FPO v0.106、paper44 §7.3/§7.5/版本行 v0.33、tracker（MourreSkeleton 去重核查/矩阵层完整字典）。**Phase 62 预言 P1–P6 数值不变、仍为远期假说**；主框架 P1–P7 冻结预言数值全部不变，冻结不受影响 |
| **v0.42** | **2026-08-15** | **Paper XLIV §5.3 动力学补充（力 = 偏转时间轴的驱动，能量转移动力学）（纯增量，预言数值不变，零声明变更；paper44 v0.33 → v0.34；力笔记 v0.19 → v0.21，路线图 v0.5 → v0.6）**：① **§5.3 新增"动力学补充"小节**——双法向偏转统一的动力学侧（几何侧已登记 v0.38）：**四动量偏转形式** $P^\mu=(E,\mathbf p)=m(\sec\theta,\tan\theta\,\hat n)$（时间分量 E=m·secθ、空间分量 p=m·tanθ·n̂、质量壳 = $\sec^2\theta-\tan^2\theta=1$，与图 5 语言一致）、**三维力分解** $\mathbf F=m\sec\theta\,\dot\varphi\,\hat n+m\tan\theta\,\dot\Omega\,\hat\varphi_\perp$（F_∥=m·secθ·φ̇=γ³m·a_∥、F_⊥=m·tanθ·Ω̇=γm·a_⊥，与标准相对论分解精确一致）、**功率闭环** dE/dt = m·tanθ·φ̇ = F·v（力做功 = 时间轴偏转的能量变化率，"力是能量转移的动力学机制"）、**谱侧衔接** dλ/dt = F·v（[衔接] 建模：图 5 时间耦合 cosθ + paper16 定理 6.3 质量=谱间隙的组合建模，推导链严格但前提为建模，不升级为独立推导）；② **诚实边界与归属**——四动量/三维力分解/功率定理均为标准相对论结论的偏转语言重述（温和兼容，无新预言），谱侧衔接标注 [衔接] 不升级；§5.6/§7.1 归属表同步加"温和兼容"行；③ **防重复推导核查记录**——与 paper5 论题 1（力 = 谱流生成元）、paper18 定义 3.1/定理 3.2（F_spec = m_spec·a_spec）、paper16 定理 4.1（ω_lab = ω₀·sechφ，运动学观测者效应）均不重复：本补充为动力学能量特征值（E = m·secθ，偏转语言），paper16 为运动学观测者效应，互补；④ 同步——力笔记 v0.21（§12 登记 + 状态行）、路线图 v0.6（路径③ 增量提炼完成登记）、paper44 版本行 v0.34。**Phase 62 预言 P1–P6 数值不变、仍为远期假说**；主框架 P1–P7 冻结预言数值全部不变，冻结不受影响 |
