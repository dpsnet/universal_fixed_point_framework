# UFPF 勘误与立场声明（RAP-Errata v0.26）

**发布日期**：2026-08-08（v0.26）
**版本哈希**：`30f822bc08`（v0.26）
...
| **XLIII** | **页岩油气成藏的谱流机制与实证** | **`paper43_shale_accumulation.md`** | **✅** | 跨领域应用支线：李传亮"运移聚集模型"谱流重构 + 20 项检查（19/20 + M2 诚实负结果登记 + P1/P4 证伪边界测试）。核心贡献：谱隙-门限压力双曲标度第一性推导（M11）、五项独有理论预测（§4.2/§5.1）。**v2.0 正式化（v0.26，2026-08-08）：作者-年份引用（移除编号）、8 图 5 表（新增图 7/8 与表 4/5）、引言创新点三段式、应用路线图（§5.2）内联** |
