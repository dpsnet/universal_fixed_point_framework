...
**谱静默的物理基础**：谱静默不是独立假设，而是从已建立的物理原理出发的严格推论。(i) **起点**：Lorentz 不变性 + 零静质量（相对论基本原理，已获广泛实验支持）；(ii) **类光条件**：零质量粒子的4-速度满足 $g_{\mu\nu}u^\mu u^\nu=0$，沿传播方向 $k$：$dx=c\,dt$（光速方向⊥时间方向）；(iii) **时间静止**：类光条件严格推出固有时 $d\tau=0$——光子内部结构不耦合时间方向（Paper XLIV 推论 2.1）；(iv) **观测一致性**：光子在真空中任意时刻性质完全相同（时空均匀性，已确立的实验事实）与上述推论一致。谱静默机制（$P_{V_\Lambda}D(f)=0$）是这一推论的数学表述：光子的内部结构（形变循环）不耦合时间方向。
