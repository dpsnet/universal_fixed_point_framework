...
在 $4+4$ 分块（Weyl 上下半）下，$\Delta$ 的 Frobenius 范数分布（`scripts/paperX_delta_block_decomp.py`，2000 样本 Monte Carlo）：
...
`scripts/paperX_mass_delta_directionality.py`（已注册 `run_all_tests.py`）：
...
| 脚本 | 内容 | 状态 |
|:----|:-----|:----:|
| `scripts/paperX_mass_delta_directionality.py` | J1/J2 数值综合验证 | ✅ 2/2 检查通过 |
| `scripts/paperX_source_defect.py` | B1 ①环源定义与精确线性 | ✅ 已注册 |
| `scripts/paperX_delta_block_decomp.py` | B4 $\Delta$ 分块支撑分布 (~87% 混合块) | ✅ 已注册 |
