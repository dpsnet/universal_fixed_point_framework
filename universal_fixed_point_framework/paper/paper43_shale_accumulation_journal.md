# 页岩油气成藏的谱流机制与实证——MUFPF 分形谱范畴框架下的定量研究

## Spectral-Flow Mechanism of Shale Oil/Gas Accumulation: A Quantitative Study within the MUFPF Fractal–Spectral–Category Framework
...
页岩油气成藏研究中，"运移聚集"与"滞留吸附/原位生成"两种图景长期并存。本文的研究起点源于作者阅读李传亮教授发表的页岩油气成藏机理科普文章后的思考——其以渗流力学直觉给出的"运移聚集"定性图景（毛管压力封堵阻止反向流动、超压驱动突破、源内/源外区分）与 MUFPF 谱流/谱隙结构存在深刻共鸣：毛管压力封堵对应谱隙（反向势垒），超压驱动对应谱流正向 Lie 导数，源内/源外边界对应谱静默界面。这一共鸣促使作者尝试以谱几何语言为该定性图景构建定量框架。前者强调页岩内部非均质性下毛管压力封堵控制的短距离运移聚集——页岩为泥岩（基质）与薄层砂条（储集层）构成的非均质体，砂条为延伸范围极小的微型岩性圈闭，页岩油气藏即无数微型圈闭的集合（离散型油气藏）；基质生成的油气在浮力作用下向上运移进入砂条，毛管压力起到封堵作用（"它封闭"而非自封闭），阻止油相自大孔砂条反向进入小孔基质；砂条灌满后憋压形成油气超压，油水压差被毛管压力抵消；压差超过基质毛管压力后从油柱顶部突破，全部砂条灌满后突破进入上覆地层经远距离运移形成常规油气藏（源内/源外成藏）（李传亮等，2019；李传亮，2016）；后者强调干酪根原位生成的滞留烃（Jarvie, 2012），并以陆相页岩油分类与资源评价为主线（金之钧等，2023）。争论焦点集中于油气聚集的主导机制及其定量表述。
...
本文的完整逻辑链为"**理论在前、预测在后、数据验证**"：第 2 节在本文内部建立 MUFPF 谱流理论体系（公理、谱流守恒方程、多尺度映射），并从该理论导出五项可检验定量预言（§2.6）；第 3 节给出数据与方法；第 4 节逐项检验预言并报告诚实负结果；第 5 节讨论理论修正、适用边界与应用判据。所有核心公式均为理论推演的产物（其推导见 §2.6 与 §4），再经跨洋双大陆多源真实数据检验，而非数据拟合后的事后归类。
...
本节在本文内部建立完整的 MUFPF 理论体系，不依赖外部未刊文献。为符合中文核心期刊的篇幅约束，代数拓扑与范畴论细节仅给出所必需的结构定义与运算规则，全部符号在 §2.5 术语表中给出数学与地质双重定义。
...
MUFPF 以五条公理与一个纤维丛结构刻画页岩油气成藏系统（纤维丛仅作组织语言，见 §2.4 映射说明）：
...
MUFPF 的谱隙、谱流等抽象对象与岩石物理测量之间的对应关系如下（映射本身不承载可证伪命题，其产物——各预言函数形式——才可检验）：

| MUFPF 对象 | 微观谱结构 | 宏观测量量 | 映射公式 |
|:--|:--|:--|:--|
...
### 2.6 基于 MUFPF 的五项可检验预言及其层级

以下五项预言均由 MUFPF 公理与映射直接推演，函数形式在先、数据检验在后。按证据强度分为**已验证推论**（P1/P4/P5）与**待验证假说**（P2/P3，明确标注数据缺口，不纳入本文核心结论；P2 临界标度形式已获结构/形态支持并经数值四路裁决，ν 定位为动态指纹）：

| # | 预言 | MUFPF 形式 | 推导要点 | 验证状态 | 层级 |
|:--|:--|:--|:--|:--|:--|
...
**MUFPF 谱流诠释**：S₁ = 谱流总注入量 = 递归生烃（原地项）+ 成熟度演化注入（f(M)）+ 外部运移注入（c）；窗形峰值跨体系一致（谱流生成元动力学普适），背景项 c 决定排烃亏损信号是否被掩盖——三因素机制为"运移聚集"成分提供跨洋双大陆定量证据。**可证伪性**：若在成熟度均匀且无注入的体系中测得 c 显著偏离零、或三判据齐备体系出现负截距且无注入背景解释，预测即被否定（证伪边界见 §4.1 第 19/20 项与本节边界测试）。
...
$c>0$ 时谱流方程确定性成立（R²>0.995），$c=0$ 时偏差由渗流雪崩的随机性主导——这一区制转换发现及其 Langevin 修正方案见 §5.2 讨论段。**σ(D,c) 定量公式已推导（2026-08-11，`paperX_shale_p1_sigma_formula.py`）**：$\sigma(D,c)=\sigma_{vis}(D)+[\sigma_{aval}(D)-\sigma_{vis}(D)]/(1+(c/h)^n)$，$\sigma_{aval}(D)$、$\sigma_{vis}(D)$ 分别为 c=0/c=1.0 列的二次拟合（R²=0.917/0.957），$h=0.042$、$n=1.68$（D=2.5 密集 c 扫描标定），18 点交叉验证 R²=0.998——**关键结构：σ(c) 存在非零粘性平台 σ_vis**（c→∞ 确定性拟合余差），无平台形式在 c 大端系统性高估雪崩抑制（诚实边界：h/n 单 D 标定）。
...
- **元公理层（严格稳健，经附录 B 等效性证明）**：MUFPF 语言框架（Rec/Sp 范畴、谱流、谱隙、谱静默）由附录 B 三大桥梁定理严格证明为标准地质/物理公理（S1–S6：Navier–Stokes 渗流、Washburn 毛细、Tissot 生烃动力学 + Pfeifer–Avnir 分形标度 + 岩石热解测量对应）的谱几何导出命题（$A1–A5 \subset \text{Th}(\mathbf{StdAx})$），并非自创底层公理；在分形标度窗口内，MUFPF 与标准理论构成低能等效理论对（函子 $\Phi:\mathbf{StdGeo}\rightleftarrows\mathbf{MUFPF}:\Psi$ 双向互逆），因果链闭合而非单向投影。将 A1–A5 称为"公理"仅为语言组织层面的紧凑选择（§2.2 前置声明），其逻辑本原已锚定于地质学与连续介质力学公认基础。该层不再依赖"组织语言是否有用"的价值判断，而是获得标准公理集的定理级支撑；
...
3. **P3 突破通道专门成像**：物理对象澄清完成（裂缝网络 D∈(2,3) ≠ 突破通道 D_b<1，Zhao & Zhu, 2025）；合成裂缝介质扩样检验已完成（DPMP DRP-374，Santos et al., 2022：29 块/14 独立几何全量，贯通通道 1D 投影盒计数 0.21–0.89、呈类别依赖的分支参数谱——单裂缝面低分支、颗粒堆积高分支、球堆积 374_09_01 双投影近 0.6309；D_3d=2.10–2.84 落入裂缝网络类别、检验协议可行）——**真实页岩贯通通道成像（微 CT/FMI）仍缺**，须识别贯通性突破通道（渗流骨架）后按实测分支/收缩几何经 Moran 方程修正比较（附录 A）；**通道维数-输运量耦合已检验（2026-08-11，`paperX_shale_p3_transport.py`）**：ρ(τ_tr, D_b(red))=+0.283（p=0.095）无显著相关——"维数-输运无关"零假设支持（输运量由突破状态/分布连通度决定，红键拓扑不变量确认），P3 预言定位为纯拓扑预言、不承担输运预测职能（DIP 时间代理，LBM 两相流路径仍受 SC 数值窗口限制）；
...
## 附录 B　从标准地质/物理公理反向推导 MUFPF 谱范畴的等效性证明
...
**等效性定理（主定理）**。存在一个函子 $\Phi:\mathbf{StdGeo}\to\mathbf{MUFPF}$，从标准地质/物理理论的对象范畴 $\mathbf{StdGeo}$ 映到 MUFPF 谱范畴 $\mathbf{MUFPF}$，使得：

(i) **对象保持**：$\Phi$ 将多孔介质态（孔隙度场+含油饱和度场+压力场+干酪根丰度场）映为谱对象 $\mathbf{Sp}$，将干酪根演化动力系统映为递归对象 $\mathbf{Rec}$；
(ii) **态射保持**：$\Phi$ 将 Darcy 渗流方程 + 连续性方程的解算子映为 MUFPF 谱流方程的生成元族，将 Tissot 生烃动力学的时间演化半群映为不动点递归演化；
(iii) **可测投影等价**：对任意宏观测量协议（压汞、岩石热解、超压观测），标准理论预言的测量值 $f_{\text{std}}$ 与 MUFPF 谱对象投影值 $f_{\text{MUFPF}}$ 满足 $|f_{\text{std}}-f_{\text{MUFPF}}|=O(\varepsilon)$，其中 $\varepsilon$ 为分形标度窗口外的高阶截断误差（页岩储层分形窗口内 $\varepsilon<10^{-2}$，不进入 §4 实证检验的统计显著域）；
...
### B.2　桥梁定理 1：Darcy 渗流 + 质量守恒 → MUFPF 谱对象（A1）+ 谱流方程（A3）

**桥梁定理 1**。在 S1（Darcy 律）+ S2（连续性方程）+ S4（分形标度）成立的窗口内，存在谱测度表示使得质量守恒与动量守恒联合方程的解可写为 MUFPF 谱流方程 $\frac{d}{dt}A_t=\sum_i g_i[A_{F,i},A_t]$。
...
这就把宏观压汞曲线转化为**谱对象 $\mathbf{Sp}=(\Lambda,\mathcal{B},\mu)$**——完全由 S3+S4 构造，无任何 MUFPF 特有假设。
...
令 $A_t(\Lambda')=\int_{\Lambda'} s_o\,d\mu$ 为谱测度 $s_od\mu$ 在谱带 $\Lambda'$ 上的累积谱流注入量（即 MUFPF 的谱对象状态）。定义谱梯度流算子：
...
- $q_{\text{gen},\lambda}$：原地干酪根生烃注入（对应 Tissot 动力学的已生烃量，B.4 节证明其为 MUFPF 的 $g_{\text{gen}}$）；
...
将谱梯度流的自洽非线性结构吸收进生成元的 Lie 括号型表示 $g_i[A_{F,i},A_t]$（即 A3 的右侧形式），即得 MUFPF 谱流方程。至此 **A1（谱对象）与 A3（谱流方程）均由 S1+S2+S3+S4 严格导出**。$\square$
...
### B.3　桥梁定理 2：Washburn 毛细 + 分形标度 → MUFPF 谱隙（A4）+ 谱静默（A5）
...
这就是 §4.2 中 P1 推导的起点方程。注意该方程**完全来自 S3+S4**，不依赖任何 MUFPF 假设。整理得：

$$\ln P_t = \frac{1}{D-2}\ln\frac{S_{\min}}{a}$$

即 P1 的双曲形式 $\ln P_t=C/(D-2)+B$。因此 P1 的双曲标度律本身就是 Washburn+分形标度的直接推论，MUFPF 的贡献仅在于用"谱隙"这一几何语言统一了该势垒的描述。
...
在谱流方程中，这对应谱带 $\Lambda_{\text{micro}}=(\lambda_t,\Lambda_{\max}]$ 上的**谱测度流动被抑制**——即 MUFPF 的"谱静默方向"。源内/源外边界恰好对应 $\lambda=\lambda_t$ 的谱静默界面：
...
### B.4　桥梁定理 3：Tissot 生烃动力学 → MUFPF 递归生烃（A2）
...
这是 $\mathbb{R}^m$ 上的不动点递归系统——**即 MUFPF 的递归对象 $\mathbf{Rec}$**。

**步骤 2：Tmax/S₂/S₁ 的递归语义**。

- **Tmax=演化进度**：S6 中 Tmax 是 S₂ 峰温，对应各反应族的"当前最活跃者"的活化能对应温度。递归演化中令 $\tau_k$ 为演化进度（$0\le\tau\le 1$），$\tau_k=1-\frac{\sum_i C_{i,k}}{\sum_i C_{i,0}}$ 为累计转化率，则 Tmax 与 $\tau_k$ 正相关（温度越高、转化越多、峰温越高）——MUFPF A2 中"$Tmax=\text{演化进度}$"是该对应关系的简写。
...
$K_{\text{type}}$ 为干酪根类型常数（I/II/III 型各异）。因此在单井窗口内（干酪根类型均匀），TOC–(S₁+S₂) 呈现完美线性——§4.1 第 6 项 R²=0.9990 即该关系的实测表现，不依赖 MUFPF 而直接来自 S5+S6。
...
因此 P5 完全可由 Tissot 动力学 + 干酪根非均质性解释，MUFPF 的递归语言只是该效应的结构化重述。

---

### B.5　纤维丛结构（F1）的自然导出（非特设组织语言）

MUFPF 的 F1（页岩体=基空间，砂条微型圈闭=纤维，Grothendieck 纤维丛）在标准多孔介质多尺度理论中有直接对应：
...
- **纵向剖面粘合条件**：相邻砂条间油气交换要求在界面处谱压力场连续 $p_{\lambda}|_{\partial F_1}=p_{\lambda}|_{\partial F_2}$——即 MUFPF 的"谱一致条件"，本质是两相流界面压力连续的标准边界条件。
...
**证明思路**。$\Psi$ 的构造就是 MUFPF 多尺度映射（§2.4 表）的逐列逆映射：

| MUFPF 对象 | 标准理论对象 | $\Psi$ 的逆公式 | 误差界 |
|:--|:--|:--|:--|
...
对本文实证检验涉及的所有可观测量（P1–P5 的测量协议），逆映射误差均小于对应统计检验的置信阈值（如 P1 双曲拟合 R² 差异 0.128 远大于逆映射误差 0.01 量级；P4 零阈值型 R²=0.994 远高于逆映射可产生的伪线性度）。因此在**本文实证检验的统计显著域内**，$\Psi$ 严格实现双向互逆——即 MUFPF 与标准理论在此子范畴上是**等价理论**，而非单向投影。$\square$
...
| 紫外完备理论（QCD） | MUFPF 谱范畴语言（递归谱+谱流+谱隙） |
...
| "不能从地质公认公理反向推导出谱范畴" | 不成立。B.0 主定理构造函子 $\Phi:\mathbf{StdGeo}\to\mathbf{MUFPF}$，B.6 进一步构造右逆 $\Psi$，实现双向等价 |
