...
*图 12　P1 双曲标度的正向仿真涌现。分形孔隙网络（Pfeifer-Avnir 采样 + Washburn 阈值）驱动 DIP 两相流，突破压力 $P_t$ 与分形维数 $D$ 呈双曲关系 $\ln P_t = 0.183/(D-2) - 4.367$（R²=0.942）——双曲函数形式与 §4.2 真实数据拟合 $\ln P_t = -1.66/(D-2) + 10.47$（R²=0.578）一致；系数符号差异（$C\approx+0.18$ vs $-1.66$，无量纲 vs psi 单位）的物理对应登记为开放问题。*
