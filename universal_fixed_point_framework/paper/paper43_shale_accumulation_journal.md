...
拟合 $\ln P_t = 0.183/(D-2) - 4.367$，**R² = 0.942**（图 12）。P1 双曲标度律从分形驱动 DIP 自然涌现——不依赖经验拟合，仅从分形孔径分布 + Washburn 阈值 + DIP 动力学导出。对比真实数据 R²=0.578（§4.2），合成数据无测量噪声故拟合更优，但二者函数形式一致（双曲优于线性），交叉验证 P1 的理论预言。

![图12：P1 双曲标度的正向仿真涌现——ln P_t vs 1/(D-2)（64³，c=0，N_CFG=2），拟合 R²=0.942](../figs/shale_fig14_p1_simulation.png)

*图 12　P1 双曲标度的正向仿真涌现。分形孔隙网络（Pfeifer-Avnir 采样 + Washburn 阈值）驱动 DIP 两相流，突破压力 $P_t$ 与分形维数 $D$ 呈双曲关系 $\ln P_t = 0.183/(D-2) - 4.367$（R²=0.942），与 §4.2 真实数据拟合 $\ln P_t = -1.66/(D-2) + 10.47$（R²=0.578）函数形式一致。*
...
**核心结论层级**：已验证推论（P1/P4/P5）构成 UFPF 框架有效性的核心证据；P1 另获正向仿真涌现验证（R²=0.942，§4.6），第一性推导闭合。P2 为假说层级——临界标度形式经数值四路裁决（ν 系动态指纹）获机制支持但地层成对数据仍缺。P3 经正向仿真红键提取从纯假说升级为**部分数值验证**（$D_b(\text{red})\approx 0.84\pm 0.06$，L=16→256，理论公式 $D_{\text{red}}=d_{\min}-\beta_{\text{eff}}\approx 0.854$），但与 P3 预言 0.631 仍有 ~21% 差距，真实页岩成像仍待检验。P2/P3 均不作为支撑论文结论的核心证据（遵循"层级 3"规范）。
