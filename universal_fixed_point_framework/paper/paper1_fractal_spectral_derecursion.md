...
**注 5.23**（形式化验证）。四层静默体系的严格层次包含定理（定理 5.18）已在 Lean 4 中完成形式化，代码位于 `SilenceHierarchy.lean`，包含谱静默→态射静默、态射静默→对象静默的包含证明以及辫子静默与态射静默的独立性证明。有限维离散原型中所有层次关系空性成立（所有对象均满足正性条件），非平凡实例需连续谱基础设施。
...
《通用不动点范畴框架》系列论文 I（增强版 v2.35），分形谱去递归理论——建立递归系统（IFS、Koopman 动态、RG 流）的统一谱理论框架。完整变更记录已移至独立文件 `paper1_appendix.md` §版本信息与变更记录。
