...
## 11. 噪声谱丛的 Grothendieck 纤维化

### 11.1 噪声范畴 $\mathbf{Noise}$

**定义 11.1**（噪声范畴 $\mathbf{Noise}$）。$\mathbf{Noise}$ 是以下范畴：
- **对象**：$\eta \in [0,\infty)$，表示噪声强度
- **态射** $\eta_1 \to \eta_2$：正膨胀比 $r > 0$ 使得 $\eta_2 = r \cdot \eta_1$
...
**定理 11.2**（$\eta_c$ 的解析形式）。在 Cl(1,7) 框架下（$k_{\max}=8$），临界噪声强度有闭式表达式：
$$\eta_c = \frac{k_{\max}}{2} \cdot \Delta\lambda_{\min} = 4 \cdot \frac{\sqrt{6} - \sqrt{2}}{\sqrt{72}} = \frac{2(\sqrt{3} - 1)}{3} \approx 0.488$$

**证明**。混合算子 $A_\eta$ 在 2×2 子空间（最低两个本征态）中，$A_R$ 的谱间隙为 $\Delta\lambda_{\min} = (\sqrt{6}-\sqrt{2})/\sqrt{72}$，噪声算符限制为 $\delta A_N|_{2\times2} = \sigma_z/k_{\max}$。谱间隙闭合条件 $\lambda_1(\eta_c) = \lambda_2(\eta_c)$ 给出：
$$\lambda_1(0) + \eta_c\langle\psi_1|\delta A_N|\psi_1\rangle = \lambda_2(0) + \eta_c\langle\psi_2|\delta A_N|\psi_2\rangle$$
代入 $\langle\psi_1|\delta A_N|\psi_1\rangle = +1/k_{\max}$，$\langle\psi_2|\delta A_N|\psi_2\rangle = -1/k_{\max}$，得 $\eta_c = \Delta\lambda_{\min}/(2/k_{\max})$。$\square$
...
Paper I §5.2 的谱静默判据 S1–S4 是对象层面的不可见性判据（作用于谱子集 $\Sigma_{\text{silent}} \subseteq \sigma_E$）。本节将 S1–S4 推广到态射层面，建立 M1–M4 判据，使态射静默（Paper I §5.7.1 定义 5.11）具有与谱静默平行的判据系统。核心定义与定理已整合至 Paper I §5.7.7（定义 5.24–5.27，定理 5.26、5.28），本节给出完整证明与相图分析。
...
Paper I §5.4 的谱静默-紧致化对比局限于"属性层面"（KK 模式不可观测）。本节将对比拓展到态射与辫子层面，建立紧致化机制与四层静默的完整翻译字典。核心定理已整合至 Paper I §5.7.8（定理 5.29–5.30，命题 5.31），本节给出完整证明与物理验证。
...
### 15.6 B1–B3 数值验证与 $K_{\text{crit}}$ 的确定
...
Lean 4 形式化（`SpectralFlowHomotopy.lean` + `SpectralGap.lean` + `SpectralDynamics.lean`）揭示了谱流 $\infty$-端射的静默边界闭合条件：

| 形式化项 | 对应内容 | 验证状态 |
|:-------:|:--------|:--------:|
| `spectralFlowInfEndo` $(\text{h\_silence})$ | 谱流 $\infty$-端射在 $[A,G]=0$ 下的定义与交换性证明 | ✅ |
| `spectral_flow_at_zero` + `spectral_flow_ode` | t=0 恒等性与谱流方程有限和恒等式 | ✅ |
| `spectralGap_formula` + `spectralGap_at_kmax8` | $\Delta\lambda_{\min} = (\sqrt{6}-\sqrt{2})/\sqrt{k_{\max}(k_{\max}+1)}$，$\mathrm{Cl}(1,7)\to k_{\max}=8$ | ✅ |
| `bareCoupling` + `R2_coefficient` + `criticalEnergyDensity` | $\alpha_i^{(0)} = \Delta\lambda_i/(4\pi)$ 至 $\rho_c = 8\pi/(3c_1)$ 的完整导出链 | ✅ |
| `spectral_invariance` | 谱流 $A_t = U\!A_0U^{-1}$ 的行列式特征值相似不变性 | ✅ |
| `noether_conservation` | $[A_S,A_F]=0$ 时 $\operatorname{Tr}(A_S A_t)$ 守恒 | ✅ |

**谱流静默的层次定位**。谱流静默 $[A,G]=0$ 不构成第五层静默，而是贯穿四层的**桥接原理**（bridging principle）：

| 穿越的层 | 谱流静默的表现 | 对应判据 |
|---------|---------------|---------|
| 谱静默 S3 | 谱间隙 $\gamma=0$，$dA/dt=[G,A]=0$ | LACI$\to\infty$ |
| 态射静默 | 谱流族 $\{F_t\}$ 退化为恒等态射 $\text{id}_A$ | 命题 5.13 恒等态射机制 |
| 对象静默 | $F_t(A)\equiv A$，时间演化在对象层面无效应 | $k_{\max}=8$ 截断 |
| 辫子静默 | $[A,G]=0$ 时 $k=0$（零缠绕），退化到谱静默 | B3 判据扁平化 |
...
**方向三：连续静默度 $\delta_{\text{silence}}$**——将静默从离散二值分类扩展为连续参数：

$$\delta_{\text{silence}}(A,G) = \|[A,G]\|, \qquad \delta_{\text{silence}}\leq 2\|A\|\cdot\|G\|$$

当 $\delta_{\text{silence}}\to0$ 时 $F_t(A)\cdot A - A\cdot F_t(A)\to0$，提供谱流 $\infty$-端射的**渐近闭合条件**。$\delta_{\text{silence}}$ 与 §15.3 统一静默度 $\mathcal{S}$ 形成互补：
...
| $S_{\text{bra}}$ | 连续 | $[0,1]$ | 辫子交叉数归一化度量 |
| $\delta_{\text{silence}}$ | **连续** | $\mathbb{R}_{\ge0}$ | **谱流生成元交换性破缺程度** |

**Lean 形式化状态**：`Silence.lean` 已完成定义及核心性质证明：
- `frobeniusNorm`（Frobenius 范数）与 `deltaSilence` 定义 ✅
- `frobeniusNorm_eq_zero_iff`：$\|A\|_F=0\iff A=0$ ✅
- `deltaSilence_eq_zero_iff`：$\delta_{\text{silence}}=0\iff[A,G]=0$ ✅
- `deltaSilence_bound`：$\delta_{\text{silence}}\leq 2\|A\|_F\|G\|_F$ 📝（需 Frobenius 范数次乘性，推迟至 Phase 36）

### 15.8 与 Paper I §5.7 的对应总结

| 深化方向 | Paper XIX 推进位置 | 主要结果 | 数值验证 | 形式化 |
|:----------:|:----------------:|:--------|:--------:|:-----:|
| **M1–M4 判据** | §15.2 | 定义 15.1 + 命题 15.1 + 定理 15.1 | ✓（§13.2 四区域）| `M1_M4_Criteria` ✅ |
| **统一静默度** | §15.3 | 定义 15.2 + 定理 15.2 + 推论 15.1 | ✓（相图四区域）| `silence_measure_monotonicity` ✅ |
| **紧致化对比** | §15.4 + §15.4.1 | 定理 15.3 + 定理 15.4 + 命题 15.3 + **定理 15.7**（Fibonacci）| ✓（Fibonacci 任意子 5 点）| `compactification_translation_dict`, `fibonacci_wilson_braid_correspondence` ✅ |
| **$C$ 与 $C_{\text{crit}}$** | §15.5 + §15.5.1 | 定义 15.3 + 命题 15.4 + 定理 15.5 + 推论 15.2 + **定理 15.8**（BTZ）| ✓（BTZ QNM 6 点）| `pseudospectral_bound_C`, `braid_degeneration_C_crit`, `BTZ_Ccrit_stability` ✅ |
| **B1–B3 与 $K_{\text{crit}}$** | §15.6 + §15.6.1 | 算法 15.1 + 定理 15.6 + **定理 15.9**（Tangherlini 维度标定）+ 数值表 | ✓（Kerr 7 点 + Tangherlini D=4,5,6,7）| `braid_silence_B1_B3_algorithm`, `K_crit_kerr_calibration`, `K_crit_tangherlini_dimension` ✅ |
| **谱流静默** | §15.7.1 | 谱流 $\infty$-端射静默边界 $[A,G]=0$、解析公式、层次定位（开放） | ✓（`paper36` 7/7 数值验证）| `SpectralFlowHomotopy.lean` + `SpectralGap.lean` ✅ |
| **连续静默度 $\delta_{\text{silence}}$** | §16 方向三 | 定义 $\delta_{\text{silence}}=\|[A,G]\|$；$\delta_{\text{silence}}=0\iff[A,G]=0$ 已证；$\delta_{\text{silence}}\leq 2\|A\|\cdot\|G\|$ 待证 | —（理论度量）| `Silence.lean`: `deltaSilence` ✅, `frobeniusNorm_eq_zero_iff` ✅, `deltaSilence_bound` 📝 |
...
## 17. Temp/RG 参数谱丛 Grothendieck 纤维化

### 17.1 动机：从"系统分类"到"系统族参数化"

§13 建立的二维相图以 $(G, \eta)$ 为坐标，覆盖了 $\mathbf{Rec}$、$\mathbf{Rec}_{\text{id}}$、$\Sigma$-$\mathbf{Rec}$ 三类范畴对象之间的转化。然而，物理系统中存在另一类普遍但未被纳入的"维度"：**参数空间**。最具代表性的两个参数空间是：
...
**定义 17.1**（温度参数范畴 $\mathbf{Temp}$）。对象为 $T \in (0,\infty)$，态射 $T_1 \to T_2$ 为 $T_2 = r T_1$ 的正实数比例因子 $r > 0$。$\mathbf{Temp}$ 中的恒等态射对应 $r=1$，复合 $T_1 \xrightarrow{r} T_2 \xrightarrow{s} T_3$ 对应 $sr$。
...
**定理 17.3**（编织自然变换 $\theta$）。在对角线上，存在比较同构 $\theta_X$：
...
参数化截面 $\sigma_{n,A}$ 对任意 $n \in \mathbb{N}$ 和幂等矩阵 $A$（$A^2 = A$）可构造，允许扩展到非 Cl(1,7) 系统。
...
### 17.6 总参数丛 $(G, \eta, T, \mu, \ldots)$

**定义 17.12**（总参数范畴 $\mathbf{Param}$）。$\mathbf{Param}$ 是乘积范畴：
...
| 丛态射 | 源 → 目标 | 物理意义 |
|:------|:---------|:--------|
| $\hat{\mathcal{T}}$ | $\mathbf{Bun}(\mathbf{Temp}) \to \mathbf{Bun}(\mathbf{RG})$ | 温标对偶 |
| $\hat{\mathcal{N}}$ | $\mathbf{Bun}(\mathbf{Temp}) \to \mathbf{Bun}(\mathbf{Noise})$ | 温度-噪声对偶 |
| $\hat{\mathcal{H}}$ | $\mathbf{Bun}(\mathbf{Kerr}) \to \mathbf{Bun}(\mathbf{Temp})$ | Hawking 温度 |
| $\hat{D}$ | $\mathbf{EFT}/\Lambda \to \mathbf{Bun}(\mathbf{RG})$ | 谱退归 |
...
- **Kerr 截面**：$\sigma_{\text{Kerr}}(\mathbf{p}) = (\mathbf{p}, I_2)$（$2\times2$ 单位矩阵）
- **Cuprate 分布截面**：$\sigma_{\text{cuprate}}(\mathbf{p}) = (\mathbf{p}, [w_g(T)\mu_T])$（温度依赖）
...
Temp/RG 纤维范畴为 $(G, \eta)$ 二维相图引入参数空间维度：
...
QCD 是完整验证系统：谱丛截面 $\sigma_\Delta^{(T)}(T) = (T, \Delta\lambda_{\min}^{(0)}\sqrt{1-T^2/T_c^2})$，函子 $\mathcal{T}(T) = \Lambda_{\text{QCD}} \cdot (T_c/T)^\gamma$（$\gamma=2$），比例因子 $a = T_c/\Lambda_{\text{QCD}} = 0.729$。
...
- **Cuprate 分布截面**：将 $\partial\mathbf{Rec}_D$ 从单点 $T_c$ 扩展为赝能隙区间 $[T_c, T^*]$，对应的谱丛截面从单值谱间隙升级为分布谱间隙截面 $\sigma_{\Delta}^{\text{(c)}}(T)$，采用双组分高斯混合模型：
  $$\varphi_T(\Delta\lambda) = w_{\text{n}}(T)\delta(\Delta\lambda) + w_{\text{g}}(T)\mathcal{G}(\mu_T, \sigma_T)$$
  其中 $w_{\text{n}}(T)$（正常组分权重）、$w_{\text{g}}(T)$（赝能隙组分权重）、均值 $\mu_T$ 和标准差 $\sigma_T$ 由谱流方程确定。分布截面与 $\hat{\mathcal{T}}_{\text{Riem}}$ 的兼容性通过推前 (pushforward) 条件 $(\hat{\mathcal{T}}_{\text{Riem}})_*(\varphi_T) = \varphi_{\mathcal{T}(T)}$ 保证。

- **Hawking-Page 相变**：$T_H = 1/(8\pi GM)$，验证跨领域普适性。
