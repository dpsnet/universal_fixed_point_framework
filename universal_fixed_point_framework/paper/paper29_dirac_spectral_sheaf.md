...
6. **三维纤维化的数值构造**：定理 7.10 的三维纤维化 $\pi: \mathcal{E} \to \mathcal{B}_{\text{sys}} \times \mathcal{B}_{\text{level}} \times \mathcal{P}$ 目前为理论构造，其纤维 $\mathcal{E}_{(\text{sys}, s, p)}$ 的显式数值表示和 Cartesian 提升的数值实现方案尚未建立。
