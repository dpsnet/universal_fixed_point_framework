...
>
> **本章定位**：本章不引入新的范畴论定义，而是展示同一套“纯接口 / 契约”如何在 UFPF 这一具体理论中兑现为定理、构造与代码。读者应把这里的每一行映射理解为“抽象接口的某个实例方法”。
...
Part 6 的表格已经把论文、范畴概念与 `.lean` 文件名对应起来。本节进一步从程序员视角说明：这些 `.lean` 文件内部通常长什么样、如何快速读懂一个 UFPF 形式化证明、以及怎样把论文中的定理名与 Lean 中的定义/定理名对齐。

### UFPF 形式化仓库的常见文件结构

打开 `UFPFormalization` 仓库的 `paper/` 目录后，你会看到类似下面的文件命名规律：
...
### 读一个 UFPF Lean 文件的建议顺序

1. **先看 `import` 段**。UFPF 形式化大量依赖 `Mathlib.CategoryTheory.*`。通过 import 列表可以立刻知道：这个文件在 Mathlib 的哪一层面上工作（普通范畴、伴随、层、纤维化、2-范畴等）。
2. **再看 `namespace` 与 `open`**。UFPF 通常会用 `namespace UFPF` 或 `namespace PaperX` 包裹，避免与 Mathlib 同名概念冲突。
...
5. **关注 `simp` 与 `rw` 模式**。UFPF 证明中常出现 `rw [Adjunction.unit]`、`simp [Functor.map_comp]` 等，这些正是把数学等式翻译为代码改写规则的直接证据。
