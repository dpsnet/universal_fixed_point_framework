...
> 目标：理解 MUFPF 中谱丛、谱预层、Grothendieck 纤维化的构造，能够复述 Paper XXI 中 Temp/RG/Noise 等参数谱丛的定义。

## 问题动机

MUFPF 中的物理参数（温度、能标、噪声强度、黑洞参数）会变化，因此我们需要研究“一族谱对象随参数变化”的结构。同时，谱数据往往先在局部开集上定义，再被粘合为全局对象。层的语言让我们把局部信息粘合为全局信息；Grothendieck 纤维化则让我们把一族范畴整体地参数化在一个基范畴上。
...
Paper XVI 定义 10.10：奇点 = 层公理被破坏的位置。在切触条件下，谱预层自动满足层公理（命题 10.15）。这为 MUFPF 中的时空奇点提供了层论刻画。
...
Paper XXI 定义 2.1-2.2：Grothendieck 纤维化是"一族对象随参数变化"的严格数学语言。MUFPF 中所有物理实例均为分裂纤维化：
...
下面给出一个与 MUFPF 谱丛直接对应的、可手算验证的 Cartesian 提升例子。它既展示 Grothendieck 纤维化的核心机制，也解释为什么 Paper XXI 中的 Temp/RG/Noise 等谱丛都是**分裂**的。
...
因此这个 Grothendieck 纤维化是**分裂的**，符合 Paper XXI 中“MUFPF 物理实例均为分裂纤维化”的断言。
...
下表把 MUFPF 中的层实例与程序员熟悉的类型/结构对应起来。这不是严格定义，而是帮助你快速找到“这些 MUFPF 层构造在代码里长什么样”的直觉：
...
下表把 MUFPF 中的谱丛实例与程序员熟悉的类型/结构对应起来。这不是严格定义，而是帮助你快速找到“这些 MUFPF 谱丛在代码里长什么样”的直觉：
...
> **学习技巧**：把层想象成“带粘贴规则的依赖映射”，把 Grothendieck 纤维化想象成“依赖类型族 $b \mapsto \mathcal{E}_b$”，Cartesian 提升就是沿基映射的 coercion/reindexing。MUFPF 的 Temp/RG/Noise 等谱丛都是最简单的情形：纤维不随参数变化，限制函子是恒等，因此分裂性自动满足。Kerr 谱覆盖则是非平凡例子，分支点处需要仔细处理茎与单值群。
...
5. 为什么 MUFPF 中所有物理纤维化都是**分裂**的？非分裂纤维化可能出现在什么场景？

## 4.8 关键要点

- **预层**是反变函子，**层**额外满足局域性与粘合性。
- **Grothendieck 纤维化**是 MUFPF 上层建筑的核心，统一了温度、能标、噪声、签名等参数化谱族。
