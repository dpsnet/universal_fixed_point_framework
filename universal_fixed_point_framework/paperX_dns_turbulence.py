...
class DNSConfig:
    ...
    force_type: str = "deterministic"  # 'deterministic' | 'stochastic' | 'energy_controlled' | 'energy_injection'
    target_energy: float = 0.5  # energy_controlled / energy_injection 模式目标能量
...
class PseudoSpectralDNS3D:
    ...
    def _init_forcing_modes(self):
        """初始化 forcing 模式（固定波数壳，相位按 force_type 处理）"""
    ...
    def _randomize_forcing_phases(self):
        """为 stochastic 模式生成新的随机相位"""
        for i in range(3):
            phase = 2 * np.pi * np.random.rand(self.N, self.N, self.N)
            self.forcing_amp[i] = np.exp(1j * phase) * self.forcing_mask

    def _forcing(self):
        """
        大尺度 forcing。
        支持四种模式：
        - deterministic: 固定相位（可重复）
        - stochastic: 每步随机相位，破坏相干性以激发湍流级串
        - energy_controlled: 根据当前能量与目标能量的差距动态调整振幅
        - energy_injection: 维持近似恒定的能量注入率 ε_target = force_amp
        """
        ...
        if self.cfg.force_type == "stochastic":
            ...
            # 能量越低，forcing 越强；接近目标时减弱
            scale = max(0.0, self.cfg.target_energy - E_current) / max(self.cfg.target_energy, 1e-10)
            amp = self.cfg.force_amp * scale * self.N**3 / n_modes
        elif self.cfg.force_type == "energy_injection":
            # 维持恒定能量注入率 ε_target = force_amp
            # f · u ≈ ε, 取 u_rms = sqrt(2E/3), 则 |f| ≈ ε / u_rms
            E_current = self._compute_energy()
            u_rms = np.sqrt(2 * E_current / 3 + 1e-20)
            # 避免 u_rms 过小导致 f 过大，设下限
            u_rms = max(u_rms, 0.01)
            epsilon_target = self.cfg.force_amp
            amp = epsilon_target / u_rms * self.N**3 / n_modes
            self._randomize_forcing_phases()
        else:  # deterministic
            amp = self.cfg.force_amp * self.N**3 / n_modes
