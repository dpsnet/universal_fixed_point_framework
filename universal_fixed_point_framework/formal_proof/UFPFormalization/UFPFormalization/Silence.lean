...
/-- Frobenius norm (Hilbert-Schmidt norm) of a finite complex matrix.
    ...
    注（2026-08-13 去重核查）：此为本文件自建 ℂ 版（早期实现）；
    RAP4_silence_strictification.lean 另有 ℝ 桥接版（`Matrix.frobeniusNorm := ‖A‖`，
    mathlib 类型类）。两者语义一致（mathlib `‖·‖` 即 Frobenius 范数），
    新增使用优先采用 mathlib `‖A‖`（泛化标量域），勿新增第三处定义。 -/
...
/-- Continuous silence degree (连续静默度): δ_silence(A, G) = ‖[A, G]‖_F.
    ...
    注（2026-08-13 去重核查）：静默度结构同族——本 `deltaSilence`（对易子范数）、
    RAP4 `silenceDegree`（= 1 − ‖P·Df‖/‖Df‖，投影剩余）、paper1 §5.7.9 `S_D`（D-静默度，
    投影到 Im(D)）三者共享"范数比定义静默度"的同一数学形式，语义各自限定（对易子/投影/表示层）。 -/
