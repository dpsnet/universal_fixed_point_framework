...
import MUFPFormalization.BranchCounting
import MUFPFormalization.Unified3Theorem
import MUFPFormalization.BottTower
import MUFPFormalization.DHStructuralAnalysis
...
open MUFPF.Unified3
open MUFPF.BranchCounting

namespace MUFPF.CoherenceToBranching
...
theorem branch_dual_eq_kmax :
    MUFPF.BranchCounting.B = 2 * MUFPF.BottTower.k_max - 1 := by
  norm_num [MUFPF.BranchCounting.B_eq_15, MUFPF.BottTower.k_max_value]

theorem spinor_dual_eq_kmax : 16 = 2 * MUFPF.BottTower.k_max := by
  norm_num [MUFPF.BottTower.k_max_value]

theorem dH_dual_eq_ln15 :
    Real.log (2 * (MUFPF.BottTower.k_max : ℝ) - 1) = Real.log 15 := by
  norm_num [MUFPF.BottTower.k_max_value]

/-- 对偶网络综合定理：分支对偶 + 旋量对偶 + 维数对偶（v0.21）。 -/
theorem kmax_duality_network :
    MUFPF.BranchCounting.B = 2 * MUFPF.BottTower.k_max - 1 ∧
      16 = 2 * MUFPF.BottTower.k_max ∧
      Real.log (2 * (MUFPF.BottTower.k_max : ℝ) - 1) = Real.log 15 := by
  constructor
...
end MUFPF.CoherenceToBranching
