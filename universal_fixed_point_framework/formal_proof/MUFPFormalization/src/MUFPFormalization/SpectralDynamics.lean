...
/-- Pauli matrix σ_x（阶段 4 Fin 类型统一：`{n : ℕ} + hn : n = 2`）。
    当 n=2 时退化为标准 [[0,1],[1,0]]（SU(2) 生成元）。 -/
noncomputable def pauliX {n : ℕ} (hn : n = 2) : Matrix (Fin n) (Fin n) ℂ := by
  subst hn; exact !![0, 1; 1, 0]

/-- Pauli matrix σ_y（通用 Fin n + hn:n=2）。
    当 n=2 时退化为 [[0,-i],[i,0]]。 -/
noncomputable def pauliY {n : ℕ} (hn : n = 2) : Matrix (Fin n) (Fin n) ℂ := by
  subst hn; exact !![0, -Complex.I; Complex.I, 0]

/-- Pauli matrix σ_z（通用 Fin n + hn:n=2）。
    当 n=2 时退化为 [[1,0],[0,-1]]。 -/
noncomputable def pauliZ {n : ℕ} (hn : n = 2) : Matrix (Fin n) (Fin n) ℂ := by
...
noncomputable def A_weak_default (g₁ g₂ g₃ : ℂ) : Matrix (Fin 2) (Fin 2) ℂ :=
  A_weak (pauliX rfl) (pauliY rfl) (pauliZ rfl) g₁ g₂ g₃
