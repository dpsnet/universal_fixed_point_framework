...
SCRIPTS = [
    ...
    ("scripts/paperX_photon_topology.py", "光子拓扑 62B：方向性阶跃(A4) + 光速不变(定理2.1) + λν 一致(定理3.1) + Bohr 匹配/吸收截面(命题1.4/定义1.4) + 时间解耦(推论4) + 零质量不自洽(命题2.2) + 捕获-再转变(命题3.2, 开放#3) + 自由传播模方守恒一致性(树级, 开放#6) + 静默-跃迁门控(开放#8) (36/36)"),
