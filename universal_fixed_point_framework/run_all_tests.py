...
SCRIPTS = [
    ...
    ("scripts/paperX_shale_spectral.py", "页岩油气成藏谱流应用推演 (10 项，9/10 通过 + 2 项诚实负结果登记)：M0 文献锚定压汞分形（[L1] 公式，[L3]-[L5] 维数恢复 4/4）+ M1 真实数据多段分形（USGS Tuscaloosa 31 样品 [L6]，分段 R² 0.894→0.971）+ M2 真实数据深化（可动-分形 ρ=+0.214 弱正不显著，负结果登记）+ M3 产油页岩文献锚定（长7段 [L2] 排序 ρ_s=-1.00）+ M4 生烃谱流检查（Rock-Eval 示例 5 样品：TOC-S₁+S₂ ρ=0.997）+ M5 长7段 TOC-生烃潜量线性正相关（10 样品，R²=0.9990，斜率 4.90，夹层 CY-04/CY-07 自动识别）+ M6 跨盆地干酪根降解谱流（18 样品：青山口高成熟 HI 中位 349 < 长7段 410）+ B1 文献量级验证（α=d_f-1 量级偏低，负结果登记）+ B2 超压临界（ν=0.5021）+ B3 突破通道盒计数——真实数据已入库 scripts/data/（诚实边界）"),
]
