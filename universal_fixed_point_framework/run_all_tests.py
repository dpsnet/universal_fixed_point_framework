...
SCRIPTS = [
    ...
    ("scripts/paperX_cl17_gamma_spectrum.py", "Cl(1,7) 生成元谱类型分层 (18/18)：α/β 单生成元谱类型=平方 ±I 代数推论（约定内：B 时间实±1/空间虚±i，A 对偶）；γ/δ 双线性对谱类型约定无关（同类对 (ΓᵢΓⱼ)²=−I⟹纯虚±i、异类对²=+I⟹实±1）；ε 静默子代数 SO(4) 旋转生成元（同类对）²=−I/4⟹纯虚谱 ±i/2（两约定均成立）；T5 ζ 约定登记（GA=i·GB Wick 型翻转酉等价，2026-08-16 已统一为数学标准时间²=+1）；T6 静默 SO(4) ≅ SU(2)×SU(2)（6=3+3 直和、[S⁺,S⁻]=0、各封闭、谱纯虚）；T7 B2 谱值嵌入 A_R=γ·I+S（12 个静默旋转生成元全部 σ(A_R) 实部=γ、虚部=σ(S)、压缩 e^{−γ}<1；单 bivector 精确 {γ±i/2}；虚部↔谱值一一对应）；T8 γ 标度第一性候选（γ=Δλ_min=0.122008 谱间隙，paper20 定理 6.1 + paper27 定义 4.3 谱静默阈值判据，压缩度 e^−γ=0.8851 vs QNM |λ|=0.9148 差 −3.2% 同量级；γ=1/k_max 近等同差 2.4%；γ=1 被排除差 −59.8%）；T9 γ=Δλ_min 下谱值嵌入全通过（12 生成元）+ 1/Δλ_min=3(√3+1)=8.196152 精确闭式（vs k_max=8 差 +2.45% 数值关系）；T10 γ 临界论证框架先例：S₄=e^{−d_H}=1/15 精确、c₂=S₄ 恰在观测窗口阈值（paper32 §4.4 可见性判据含等号）、Rec_diss 压缩度 ρ=e^−Δλ_min=0.8851>S₄ 可见；T11 Δλ_min=(√3−1)/6 简洁闭式（恒等）+ Δλ_min·15=Δλ_min/c₂=(5/2)(√3−1)=1.830127 精确（γ-c₂ 关系精确化）；T12 γ 取临界必要性：γ≥Δλ_min 可辨识下界（paper27 定义 4.3 判据：γ<Δλ_min⟹LACI>2.0 不可辨识，非类比）+ γ=Δλ_min 可辨识区最小阻尼下确界（电磁长程 paper32 §5.2 U(1)~1 支持）——B1/B2 翻译缺口代数骨架 + 连接构造 + γ 判据量级锚定 + 临界必要性（笔记 silence_direction_allocation.md §4.7-§4.14）"),
    ...
    ("scripts/paperX_photon_time_coupling.py", "光子拓扑 63 开放问题#1 时间耦合线数值验证（S11-S15，6/6）：γ→∞ 极限收敛（v→c 时 1/γ 单调递减至 <1e-6，渐近零不可达）；cosθ(arcsin(v/c))≡1/γ（max err 1.8e-16）；boost 三角参数化 secθ/tanθ≡γ/γβ（5 β 值 <1e-12）；牛顿斜线穿过光速 vs 相对论渐近贴近 1（θ→90° 不可达）；光速锁定 E=pc 复核（v_g=c 精确）"),
    ("scripts/paperX_hbar_gauge_fixing.py", "自然单位制 ħ=1 规范固定推导数值验证（note natural_unit_gauge_fixing.md 推论 H1，H1-H4 全过）：λ_M=ħ₀·λ_T/λ_L² 规范固定条件在 6 种重标度选择（SI/光秒/极端尺度）下全部精确给出 ħ̃=1；c=1×ħ=1 双规范固定联合（c̃=1 ∧ ħ̃=1 同时成立）；量纲消去 3−2=1 个外部标度（M_Pl，与 RAP 基线一致）；Π 定理无量纲组合 β=v/c 单位变换下不变——与 paper44 推论 3.1（c=1）对称补全的纯单位变换规范固定，非新物理，不回答 paper18 开放问题 2"),
    ("scripts/paperX_spin_topology_z2.py", "光子拓扑 62B 方向5 §6.5 自旋拓扑化数值验证（S1-S7，7/7）：旋量 2π 变号（U(2π)=-I vs SO(3) R(2π)=I）/覆盖提升（2π 开路径、4π 闭环）/双覆盖群性质（Φ 群同态 + 核 Z₂ + 保向 det=1）/任意轴变号（20 随机方向）/SU(2)≅S³ 单连通参数化（a²+|b|²=1）/Uhlenbeck–Goudsmit 自转超光速（68.5c，自旋非运动）/Berry 相位=-Ω/2（θ0=60° 几何相位）——「自旋不是运动，是拓扑结构」数值层"),
    ("scripts/paperX_spin_zeeman_coupling.py", "光子拓扑 62B 方向5 §6.7 自旋作用角度量化候选验证（S1-S5，5/5）：塞曼能量投影 ΔE(ϑ)=2μ_B B|cosϑ|（90° 归零、B=1T 分裂 1.158e-4 eV 锚定）/结构同构（时间耦合 cosθ 与塞曼 cosϑ 同为单位矢量点积投影，100 随机对）/Larmor 进动 ω=2μ_B B/ħ（1.759e11 rad/s，自旋动力学含时）/90° 归零同构（光子零耦合极限 ↔ 垂直取向无响应）——作用角度=cos 投影量化候选成立"),
    ("scripts/paperX_z2_topological_charge.py", "光子拓扑 62B 方向5 §6.9 Z₂ 值拓扑荷公理化代数结构验证（S1-S5，5/5）：Z₂ 群结构（乘法≅模2加法，(-1)²=+1 费米子×费米子=玻色子）/符号同态 sign: S_N→Z₂（S₄ 全体 24² 组合）/N 粒子交换符号 (-1)^{N(N-1)/2}/Levi-Civita ε 偶奇置换 ±1/张量性候选 σ(X⊗Y)=σ(X)σ(Y)（光子 s₁s₂ 封闭、费米子⊗费米子=玻色子型）——公理化 A1-A4 的代数骨架自洽"),
    ("scripts/paperX_z2_winding_unification.py", "光子拓扑 62B 方向5 §6.10 Z₂ 值拓扑荷统一验证（S1-S5，5/5）：环绕数模 2 约化（π₁(SO(2))=Z→π₁(SO(3))=Z₂，U(2πn)=(-1)^n I，n∈[-3,3]）/光子两螺旋度模 2 同值（s=±1 → 同非平凡类，n≡-n mod 2）/A2 张量性由可加性推出（σ(n₁+n₂)=σ(n₁)σ(n₂) 全组合）/A4 量化嵌入（外显能量=σ·(2μ_B B cosϑ)，Z₂ 符号×强度投影）/三层次区分（投影值 s=±1 ≠ 环绕模 2 类 ≠ 统计类，光子玻色子+1 独立于非平凡环绕类）——光子环绕定向与旋量 Z₂ 结构统一为同一 Z₂ 值拓扑荷"),
    ("scripts/paperX_z2_characteristic_class.py", "光子拓扑 62B 方向5 §6.11 Z₂ 值拓扑荷层级核对（S1-S5，5/5，2026-08-11 修正 RP³ 数据）：π₁ 级阻碍（RP³ 非平凡、S²/S³/CP² 单连通）/w₂ 值（仅 CP² 非零；RP³ 切丛平凡 w(RP³)=(1+x)⁴≡1 mod 2 故 w₂=0）/反例 CP²（单连通但 w₂≠0 ⟹ σ≠w₂ 层级不同）/自旋结构判据（w₂=0 ⟺ 存在：RP³/S²/S³ 有、CP² 无）/双向判别（RP³ π₁ 非平凡但 w₂=0 有自旋结构 vs CP² π₁ 平凡但 w₂≠0 无自旋结构——σ 与 w₂ 相互独立无单调蕴含）——σ（π₁ 级环绕模 2）与 w₂（H² 级特征类）层级独立，正式对偶论证开放"),
    ("scripts/paperX_postnikov_homology.py", "光子拓扑 62B 方向5 §6.13 Postnikov 塔/同调对偶代数骨架核对（S1-S5，5/5）：RP³ 胞腔同调（H₀=Z/H₁=Z₂/H₂=0/H₃=Z，边界 d₃=0/d₂=2/d₁=0）/RP³ 的 Z₂ 上同调万有系数（H⁰=H¹=H²=H³=Z₂ 但 w₂=0——同调类非平凡≠特征类非零，w₂ 为切丛阻碍类非单纯同调）/CP² 同调（H₀=H₂=H₄=Z、H²=Z₂ 且 w₂=c₁ mod 2≠0）/Postnikov 塔前两层（RP³ π₁=Z₂、π₂=0；CP² π₁=0、π₂=Z）/可定向性-自旋结构核对（RP³ 可定向 w₂=0 有自旋结构 vs CP² w₂≠0 无）——w₂ 正式对偶论证的代数骨架，正式论证仍开放"),
    ("scripts/paperX_mass_time_window.py", "光子拓扑 62B 方向5 §6.14 时间=质量外显窗口层次二数值验证（S1-S4，4/4）：物质波频率 ω=mc²/ħ（电子 7.76e20/μ 1.61e23/质子 1.43e24/τ 2.70e24 Hz，ω∝m 严格线性）/寿命标度 τ∝1/m⁵（μ vs τ 子量级差 0.8，CKM/相空间修正内）/双尺度互补（m↑⟹ω↑且τ↓——质量经时间窗口两个外显方向）/质量门参考（有质量静止时间耦合=1 vs 光子零耦合 dτ=0）——§6.12 层次二（质量大小→时间尺度）数值支撑"),
    ("scripts/paperX_classifying_space.py", "光子拓扑 62B 方向5 §6.15 w₂ 正式对偶论证分类空间层骨架（S1-S5，5/5）：H*(BSO(3);Z₂)=Z₂[w₁,w₂,w₃] Poincaré 级数 1/((1-t)(1-t²)(1-t³))→1,1,2,3,4,5/H*(BSU(2);Z₂)=Z₂[v₄]（Spin(3)=SU(2) 分类空间单生成元 4 维）/诱导拉回 BSU(2)→BSO(3) w₁↦0、w₂↦0（σ 双覆盖分类空间层消除）/提升判据 w₂=0⟺Spin 提升可存在（RP³/S²/S³ 可、CP² 不可）/对偶关联（σ 结构群级 π₁ 双覆盖 ↔ w₂ 主丛级阻碍经分类空间）——σ 与 w₂ 分层关联骨架，正式论证仍开放"),
    ("scripts/paperX_postnikov_k_invariant.py", "光子拓扑 62B 方向5 §6.16 Postnikov 塔 k-不变量骨架核对（S1-S5，5/5）：K(Z₂,1)=RP^∞ 整上同调（H⁰=Z、H^{偶>0}=Z₂、H^{奇}=0）/RP^∞ Z₂ 上同调（H^k=Z₂ 全 k）/RP³ Postnikov 塔三层（π₁=Z₂、π₂=0、π₃=Z，S³ 覆盖）/k-不变量载体 k⁴∈H⁴(K(Z₂,1);Z)=Z₂ 非平凡（π₃ 层扭合）/w₂（切丛阻碍，RP³ 为零）与 k-不变量（同伦阻碍，载体非平凡）不同载体相互独立——w₂ 正式对偶论证第三层（Postnikov 塔），正式论证仍开放"),
    ("scripts/paperX_jacobi_bianchi.py", "光子拓扑 62B §8 曲率层 雅可比恒等式与 Bianchi 代数前提验证（S1-S5，5/5）：su(2) 基雅可比恒等式（3³ 组合残差 0）/随机 su(2) 值矩阵雅可比（100 组残差 7.6e-15）/常系数联络 Bianchi=雅可比（[ω,Ω] 3-形式分量≡雅可比组合）/变系数 su(2) 值联络完整 Bianchi dΩ+[ω,Ω]=0（3 维网格数值差分残差 1e-13）/代数前提总结（d²=0 §8.5 + 雅可比本节 ⟹ Bianchi，§8.2 第 3 项代数部分闭合）——§8 曲率层代数骨架补完整"),
    ("scripts/paperX_hydrogen_spectral_gap.py", "光子拓扑 62B 方向3 §4.4 谱带参数第一性标定：氢原子锚定（S1-S5，5/5）：氢原子能级 E_n=-13.6/n² eV（E₁=-13.6/E₂=-3.4，Rydberg 锚定）/Bohr 谱表示 hν=ΔE（Lyman α 121.6 nm、Balmer α 656.4 nm 标准值）/谱间隙=电离阈 13.6 eV（束缚带顶→自由带底离散跳变）/Rydberg 公式 1/λ=R_H(1/n₁²-1/n₂²) 重现/与定理 T3 衔接（束缚带={E_n}、自由带=[0,∞)、谱间隙第一性标定）——§10 第 3 项开放子项闭合（谱带参数从原子物理第一性标定，非工程参数化）"),
    ("scripts/paperX_functor_extended.py", "光子拓扑 62B 方向2 §3.3/§3.4 扩展子范畴函子律验证（S1-S8，8/8）：对象映射良定义（hν=ΔE=E_i-E_j>0，全 6 个发射跃迁）/保恒等（fold∘unfold=id_A，发射+吸收闭合）/保复合（核心：ν_ik=ν_im+ν_mk——直接 vs 分步跃迁频率一致性，ΔE 可加⟹频率可加=能量守恒）/全组合复合（4 能级所有多步路径，共 5 条）/扩展子范畴函子律成立——Φ=D|_Rec 多能级特例/S6 无穷维（ι=ℕ）组合原理大 n 验证（Rydberg-Ritz ν_ik=ν_im+ν_mk，n 至 10⁵ 随机采样残差 <1e-12）/S7 无穷维带边极限（固定 k、i→∞ 时 ν→R_H/k² 单调收敛，k=1 极限 hν=R_H=13.6 eV=电离阈，§4.4 锚定）+束缚带离散 vs 自由带连续/S8 无穷维函子律成立总结——Φ=D|_Rec 无穷维特例（§3.4 层次 A 闭合，层次 B 连续谱 [0,∞) 为库依赖开放项）"),
    ("scripts/paperX_closure_winding.py", "光子拓扑 62B 方向4 §5.5 闭合结构方向转变：环绕闭环数值验证（S1-S5，5/5）：圆偏振环绕 Im(ε±*×ε±)·k̂=±1（螺旋度定量，拓扑表述 2.5.1）/相位环绕轨迹（E(t) 法向平面圆轨迹，|E|² 恒定模守恒）/线偏振=ε±等权叠加（无净环绕 Im=0，非 s=0 本征态）/直线传播=环绕投影（螺旋线轴向投影为直线）/与拓扑表述 2.5.1 一致（螺旋度 s=±1=环绕定向）——命题 2.6 行波环绕轴闭合定量内容"),
    ("scripts/paperX_theta_esc_gravity.py", "光子拓扑 62B 方向6 §7.25 等效速度角 θ_esc 量化统一验证（S1-S5，5/5）：cosθ_esc ≡ √(1-2GM/rc²)（地球/GPS/太阳偏差 <1e-15）/逃逸速度对应统一形式 √(1-v_esc²/c²)=√(1-2GM/rc²)（运动学钟慢同构）/水星进动 42.99\"/世纪 分解（1/6 g₀₀+2/3 g_rr+1/6 测地线）/光偏折 1.750\"（时间/空间各半）/黑洞视界 θ_esc=90° 时间冻结（与光子⊥时间 θ=90° 零耦合极限呼应）——等效速度角与运动学速度角同为\"向法向自由度偏转\"量化统一"),
    ("scripts/paperX_exterior_derivative_nilpotent.py", "光子拓扑 62B §8 曲率层 外微分幂零性 d²=0 验证（S1-S5，5/5）：0-形式 d(df)=0（混合偏导差，解析精确）/1-形式 d(dω)=0/2-形式（4 维）d(dη)=0/su(2) 值形式逐分量 d²=0/无源衔接 F=dA⟹dF=d²A=0（无源 Maxwell；非阿贝尔 Bianchi 的 d²=0 前提，Bianchi 残差见 paperX_photon_curvature.py）——结构方程/Bianchi 的代数前提闭合（§8.4 开放子项数值层）"),
    ...
    ("scripts/paperX_epsilon_delta_derivation.py", "光子拓扑 62#5 深化：ε_Δ 严格定义（4-范畴 Δ 结构路径）——从 paper35 Δ 结构常数推导：‖Δ‖_F²=r_cat·Δλ_min²≈6.01e-4（传播子修正权重, A4 闭合）= 第一性候选 C1 + r_NLO≈8.06e-4（NLO 相对强度, A1 闭合）= 独立验证 C2（同量级互证, C1/C2=0.75）；量级判别：C1 与路径 A(S4³=2.96e-4) 同量级(差 2.03 倍)、路径 B(S4²=4.44e-3) 偏大 7.4 倍——结构推导支持路径 A 量级排除路径 B；κ_Δ 交叉约束重算(ε_Δ=C1 时 K_a/K_c 仍排除, 收窄带 [1e-4,6.01e-4], 盲登记主候选 S4³ 不受影响) + 数值巧合观察如实登记(r_NLO≈e·S4³ 差 0.07%, ‖Δ‖_F²≈2·S4³ 差 1.5%, 非推导依据) (14/14)"),
    ("scripts/paperX_rcat_analytic.py", "光子拓扑 62#5 深化：r_cat 解析化（路径 1）+ Δ 代数强度结构（路径 2）——r_LO 精确解析闭式 = 5/24 − S²/9216（S=Σ√(k(k+1)) k=1..8, Tr(A²)=10/3 解析精确, 完全解析占 r_cat 92%）+ NLO Wigner 平均解析近似（与采样模型系统性偏差 0.4 倍, 登记开放）+ r_cat 分解（LO 91.8% + 归一化重标度 6% + NLO 2%）+ ‖Δ‖_F² 与路径 A 关系（‖Δ‖_F²(MC)/(2·S4³)=1.015, LO=0.932）+ 数值巧合观察如实登记（r_cat/Δλ²≈e 差 0.15%, r_NLO≈e·S4³ 差 0.07%, 非推导依据）——r_cat 完全闭式化登记开放（含代数数和 S + 归一化效应） (9/9)"),
    ("scripts/paperX_nlo_analytic.py", "光子拓扑 62#5 深化：r_NLO 精确解析闭式（重大突破）——NLO=[A,δb]·δa+δb·[δa,A] 不含 f,g（只含随机扰动）⟹ 可精确解析：项1=2Δλ²[Tr(A²)/n−(TrA)²/n²]/n（解析 vs 场景1 MC 差 0.006%）+ 交叉=项1 恒等式（3M 样本 0.99994）+ r_NLO=3·项1=((2−√3)/18)·(5/16−S²/6144)≈8.281e-4（场景1 MC 差 0.007%）；全模型含 Nb,Na 缩放因子 ~1.028（登记）；r_cat 解析分解 = r_LO（随机 f,g 归一化无闭式, 登记开放）+ r_NLO（精确闭式）；r_cat 预测（理想归一化）=0.037916 vs MC 0.040404（差 6.2%, f,g 归一化效应开放）——ε_Δ=‖Δ‖_F² 解析结构获 NLO 部分精确闭合 (9/9)"),
    ("scripts/paperX_fg_normalization_analysis.py", "光子拓扑 62#5 深化：f,g 归一化效应定量关联（E_g[g²] 偏差分解）——δ（E_g[g²] vs 理论 (I+A²+A⁴)/Tr 偏差, 先降后升模式）与谱参数强相关：Δλ r=0.94（前7）、1/a r=0.97（前7）；δ 通过谱权重 Σδ·S 定量传递到 r_LO（f,g 随机化效应完全分解：r_LO 偏差 0.003621 vs 实际 0.003624, 差 <0.03%）；r_cat 完整分解 = r_LO_formula(0.037088)+δ 贡献(+0.003621)−缩放修正(0.001129)+r_NLO(0.000828) = 0.040409 ≈ MC 0.040404（差 0.01%）；δ 来源 = 随机归一化分式期望 Jensen 效应（delta 方法复现, 差 0.0001）——f,g 归一化效应定量闭合（除缩放修正解析, ~3%, 登记开放） (6/6)"),
    ("scripts/paperX_rcat_closed_form.py", "光子拓扑 62#5 深化：r_cat 完全解析闭式（里程碑）——r_cat = [r_LO_formula + δ贡献]·(E[1/Nb²])² + r_NLO·(E[1/Nb²])² = 0.040401 ≈ MC 0.040404（差 0.007%）；四项全解析：r_LO_formula=5/24−S²/9216（均匀化 92%）+ δ 贡献=Σδ·S（f,g 随机化）+ E[1/Nb²]=1/(1+Δλ²)+4Δλ²/(n²(1+Δλ²)³)（缩放修正微扰闭式, 差 0.001%）+ r_NLO=((2−√3)/18)(5/16−S²/6144)（精确闭式）——ε_Δ=‖Δ‖_F²=r_cat·Δλ²=6.0141e-4 完全解析（非 MC, 与路径 A 差 2.03 倍未闭合, 与 2·S4³ 差 1.5%） (7/7)"),
    ...
    ("scripts/paperX_ww_decay.py", "光子拓扑 A4 涌现候选数值层闭合（§3.7 登记项, 回应 CNF 评价 (a)A4 断言无动力学, 8/8）：S1 WW 积分微分方程非 Markov vs Markov（洛伦兹谱 ODE 化, 偏差 0.52% @γτ_c=0.01, 量级一致）+ S2 氢 2p→1s A 系数复现 6.260e8 s⁻¹ vs 文献 6.27e8（偶极矩阵元径向积分, 差 0.15%）+ S3 光子波包外向传播（原子处 P_e→0 τ=1.6ns, 波前 r=c·t 无回归）+ S4 Lindblad 可逆性破坏（混合度单调上升, 耗散子非幺正）——A4 获机制来源（因果性+谱逃逸）与失效条件（闭合系统⟹可逆）的数值支撑；诚实边界：WW 开放系统近似（Markov 偏差 ~γτ_c）、A 系数为已知原子物理（温和兼容非新预言）、自伴性 Kato–Rellich 为库依赖开放项"),
    ("scripts/paperX_kato_rellich_selfadjoint.py", "光子拓扑 A4 锚点 2 前提：Kato–Rellich 自伴性推导级验证（§3.7 自伴性闭合方案 (ii) 登记项推进, 5/5）：S1 Hardy 不等式 3D 常数 4（高斯波包 ‖ψ/r‖²/‖∇ψ‖²=4/3 ≤ 4）+ S2 库仑势 V=-Z/r 相对 -Δ（原子单位 1s 态 ‖Vψ‖/‖Δψ‖=0.632 复现精确 √(2/5), 氢 1s 能量 -13.61 eV；Hardy+Young ⟹ 相对界 0, Kato–Rellich 适用）+ S3 WW 耦合 V 预解判据（‖V(H₀+i)^{-1}‖=g/√2 精确奇异值复现, g=0.5 ⟹ 0.354<1, 相对界<1）+ S4 谱 ⊆ ℝ（自伴性直接推论）——方案 (ii) 从登记推进为推导级+数值佐证；诚实边界：不替代 Lean 形式化（谱理论库依赖）、Mourre 估计（a.c. 谱确认, RAGE 全条件）仍登记开放"),
    ("scripts/paperX_mourre_ac_spectrum.py", "光子拓扑 A4 锚点 2 前提：Mourre 估计（a.c. 谱确认, RAGE 全条件收尾, §3.7 登记开放项推进, 5/5）：S1 对易子恒等式 i[H₀,A]=H₀（解析精确: 标度齐次性 U_sH₀U_s†=e^{-s}H₀ 或动量空间直接计算; |k| 尖点使有限格点谱导数非求导, 格点对易子有 O(1) 混叠——恒等式为解析声称, 数值内容以精确对角给出）+ S2 Mourre 估计 E_I i[H₀,A]E_I = E_I H₀ E_I = diag(|k_j|) ≥ a E_I（5 窗口精确对角 min eig ≥ a + 随机平滑态二次型 min ≥ a）+ S3 无嵌入 L² 本征值（窗口波包关联衰减 1→0 vs 单动量本征态对照=1）+ S4 RAGE 谱逃逸定量（P_K: 0.91→0, >100 倍, 波包 v_g=1 外向传播）+ S5 RAGE 三前提清单闭合（自伴 Kato–Rellich + 自由带 a.c. Mourre + 位置表示）——A4 锚点 2 数学前提齐备（推导级+数值佐证）；诚实边界：Lean 形式化（谱测度库依赖）、耦合情形 a.c. 保持（FGSS 线文献）、Friedrichs 模型严格化（共振极点）仍登记开放"),
    ("scripts/paperX_friedrichs_resonance.py", "光子拓扑 A4 锚点 1 ③ 模型实例：Friedrichs 模型共振极点（下半平面极点 = 不可逆, §3.7 登记开放项推进, 8/8）：S1 束缚态判据（η(E) 实零点: 弱耦合 λ=0.05 有效无束缚态 E_b~Λe^{-ω₀/λ²} 指数贴近阈值, 强耦合 λ=1.0 显式 E_b=-0.712 阈值下孤立本征值 + 完备性 ∫ρ+|⟨e|b⟩|²=0.997）+ S2 共振极点（第二叶 η_II(z)=0: z_res=1.9965−i0.00787 下半平面, γ=0.01573≈2πλ² 差 0.16%, λ∈{0.02,0.05,0.1} 扫描 γ/2πλ²=1.0003/1.0016/1.0063 收敛到黄金规则）+ S3 衰变动力学（谱公式 P_e(t)/e^{-γt}=1.003 恒定, 指数衰减匹配极点率; ∫ρ=1.000）+ S4 谱结构（ρ 峰 FWHM=γ×0.996 可分辨洛伦兹非 δ 嵌入尖峰 + 强耦合阈值下束缚态 + 极点 Im≠0 不在谱）+ S5 A4 衔接（下半平面极点 = 因果性/推迟选择, 锚点 1 ③ 模型实现; γ 与 WW 衰减率同率）——不可逆性 = 谱外复极点获可解模型定量实现；诚实边界：Friedrichs 平坦谱可解理想化（ω³ 偶极谱密度给能量依赖修正, 已量化）、非 Lean 形式化（谱测度/复分析库依赖）、完整谱等式（耦合 a.c. 保持, FGSS 线）仍登记开放"),
    ("scripts/paperX_silence_vertex_beta.py", "光子拓扑第一性起源 方向1 S3 谱静默互补推进（§10 第 1 项数值层，6/6）：顶点数调控静默的场论（单圈）定量化——β 函数自相互作用项 = 谱静默维持源：U(1) b₀=-4n_f/3 无自相互作用项（N_vert=0）⟹ 红外自由 α→0 ⟹ 无谱间隙闭合 ⟹ 静默解除（光子可传播）；SU(3) b₀=11-2n_f/3 自相互作用项 11=三胶子 10+四胶子 1（N_vert>0，顶点谱封闭）⟹ 红外 Landau 极点 ⟹ 谱间隙闭合 Δλ_min→0（paper40 定理 4.2）=禁闭=静默驻留；'顶点数调控静默'从机制论证获单圈场论定量支撑，完整非微扰推导仍开放"),
    ("scripts/paperX_tensor_whitney_z2.py", "光子拓扑第一性起源 方向5 A2 张量性推进（§6.10①/§6.11① 共同开放项，8/8）：⊗ 结构定义候选的代数骨架——S1 Whitney 求和公式（S² 霍普夫 η⊕η stably trivial、RP² (1+x)³、Lucas 定理核对）+ S2 线丛张量积公式（w₁ 加法 4 组合、CP² 霍普夫 H⊗H 张量平方 Z₂ 消失）+ S3 σ 幺半群同态候选 (Sp,⊗)→(Z₂,·)（双覆盖⊗双覆盖=+1、结合/交换/单位元保持）+ S4 复合可加性（n∈[-5,5] 全 121 组合）+ S5 w₁⊕加性 ≅ σ⊗乘性（Z₂ 同构）——⊗ 定义候选 A/B 代数骨架闭合，正式范畴证明仍开放（同时为 §6.12 双窗口、§7.27② g_rr 清障）"),
    ("scripts/paperX_duality_discriminant.py", "光子拓扑第一性起源 方向5 w₂ 正式对偶论证实例判别矩阵收口（§6.16 结尾开放项，5/5）：三层骨架（流形 §6.13/分类空间 §6.15/Postnikov §6.16）整合——S1 σ（π₁ 级）判别（7 实例：S¹/S²/S³/T²/RP²/RP³/CP²）+ S2 w₂（H² 级）判别（自旋结构 ⟺ w₂=0）+ S3 层级独立四象限全覆盖 + 双向反例（RP³：σ≠0 但 w₂=0；CP²：σ=0 但 w₂≠0）⟹ 无单调蕴含 + S4 三层骨架一致性（H² 非平凡≠w₂、拉回=流形 w₂、π₁ 层=σ）+ S5 收口：普适对偶定理不成立（CP² 反例），条件性对偶登记开放"),
    ("scripts/paperX_dual_window_sigma.py", "光子拓扑第一性起源 方向5 双窗口统一范畴表述推进（§6.12 开放项，§6.17 ⊗ 清障后，6/6）：σ 外显乘积结构的代数骨架——S1 双窗口 σ 核同一性（时间/力窗口拓扑内核均含 σ，σ²=1 跨通道自洽）+ S2 通道分离定量对照（时间通道 ω=mc²/ħ 单调 + τ∝1/m⁵ 子量级符合；力通道塞曼/Larmor 标准值）+ S3 乘积结构统一（外显=σ×通道强度：时间 σ_time×ω、力 σ×2μ_B B cosϑ）+ S4 σ 幺半群同态兼容（⊗ 下 σ(X⊗Y)=σ(X)σ(Y) + 双窗口乘积 σ² 消去）+ S5 总结——双窗口同源为 σ 外显的代数骨架闭合，正式范畴定义（Obs 范畴/外显函子）仍开放"),
    ("scripts/paperX_grr_categorical.py", "光子拓扑第一性起源 方向6 g_rr 框架内范畴表述推进（§7.27② 开放项，⊗ 清障后，5/5）：双通道引力调制的代数骨架——S1 g_rr=1/g₀₀ 度规双通道互逆恒等式（g₀₀·g_rr=1 全采样 <1e-12）+ S2 双法向响应权重（水星进动 42.99\"=1/6+2/3+1/6，g_rr 空间响应 2/3、4:1 空间/时间比；光偏折各半）+ S3 双通道调制分离（√g₀₀ 时间通道 θ_esc、g_rr 空间通道，命题 2.1 双法向对称衔接）+ S4 ⊗ 自逆结构类比（g₀₀·g_rr=1 互逆 ↔ σ²=1 自逆，§6.17）+ S5 总结——g_rr 范畴表述候选代数骨架闭合，正式 Rec/Sp 表述仍开放"),
    ("scripts/paperX_deflection_externalization.py", "光子拓扑第一性起源 方向6 双法向偏转↔双窗口外显衔接推进（§7.27③ 开放项，5/5）：观测面代数骨架——S1 时间通道衔接（cosθ_esc≡√g₀₀=1/γ(v_esc)，地球/GPS/太阳偏差<1e-9）+ S2 双通道×双层矩阵（机制层 √g₀₀/g_rr ↔ 外显层 时间/力窗口 一一对应）+ S3 共享乘积结构（机制层 σ_grav×通道因子、外显层 σ×通道强度同一代数形式）+ S4 观测面同一性（GPS 引力时间膨胀 45.7 μs/日——机制层与外显层同一可观测量，标准值 +45.9）+ S5 总结——'外显窗口 = 偏转机制的观测面'获代数骨架（外显 = P_obs ∘ D），正式函子构造仍开放"),
    ("scripts/paperX_exterior_functor_formal.py", "光子拓扑第一性起源 正式范畴构造定义候选（§6.19/§7.28/§7.29 开放项严格定义层面，5/5）：Obs 范畴 + 外显函子 E: Sp→Obs——S1 Obs 范畴定义候选（对象=观测通道、态射=投影/恒等/复合）+ S2 外显函子函子律（E(id)=id 保恒等 fold∘unfold=id + E 在 ⊗ 下保复合 σ(X⊗Y)=σ(X)σ(Y)，§6.17）+ S3 P_obs∘D 复合（GPS 45.7 μs/日时间通道 + 进动 28.66\"空间通道外显值一致）+ S4 g_rr Rec/Sp 表述候选（g₀₀·g_rr=1 双通道互逆）+ ⊗ 全体证明范畴前提（σ 函子性框架）+ S5 总结——正式定义候选代数骨架闭合，严格证明（全体谱对象 + Lean 形式化）登记开放"),
    ("scripts/paperX_ww_decay.py", "光子拓扑 §3.7 A4 涌现不可逆推导候选数值验证（Wigner–Weisskopf，4/4）：S1 WW 非 Markov 精确解 vs Markov 指数解（γ₀/Γ 扫描，非 Markov 修正 ~O(γ₀τ_c) 幂律核对——Markov 极限 dev <1e-3、偏差随 γ₀/Γ 近线性（幂律指数 0.97）、Euler 数值核对 <1e-2）+ S2 氢 2p→1s 爱因斯坦 A 系数 6.27e8 s⁻¹ 复现（径向偶极积分 I_rad=1.2903a₀ + 角向因子 1/√3 ⟹ |⟨1s|z|2p,0⟩|²=(2^15/3^10)a₀² 锚点 <1e-4、A=6.260e8）+ S3 光子因果外向波包（洛伦兹谱⟹因果指数 e^{-γ₀t} 数值傅里叶核对 rel<1e-2 + 前沿峰值以光速外移 x_peak=ct + 原子处概率 e^{-γ₀t}→0 时间尺度 1/γ₀）+ S4 A4 成立域对照（开放连续谱：P_e 单调→0 无回波 e^{-25}≈1e-11 vs 闭合单模 JC：P_e=cos²(t) 周期回波 ~1——A4 失效条件=闭合系统⟹可逆的数值演示）"),
    ("scripts/paperX_p6_atomic_silence.py", "光子拓扑 §3.8 P6 原子实例验证（静默叠加定理的递归层实例化，5/5）：回应 CNF 评价'P6 跨尺度挪用'——P6 非跨尺度移植而是框架根基（静默叠加定理）的原子尺度实例——S1 壳层 = 嵌套 Rec 对象 + 逐层谱隙 Rydberg（Lyman α 10.2 eV/Balmer α 1.89 eV 核对）+ S2 R_supp(N)=(1/15)^N 抑制曲线（N=3 达 3e-4、N=6 达 8.8e-8 阈值）+ S3 N_crit(θ)=ceil(lnθ/lnσ)：θ=1e-3→3 层、θ=1e-6→6 层（与 cross_effects E6 一致，N_crit-1 严格未达阈值）+ S4 候选体系映射（n_max≥4 原子 K/Kr/Xe/Rn 达 N_crit(1e-3)；诚实负结果：现行周期表 N_max=5 < N_crit(1e-6)=6，θ=1e-6 全抑制区为超重元素可证伪预言）+ S5 单层锚点（R_supp(0)=1 允许跃迁无抑制、2s→1s 禁戒 B12=0 为单层静默抑制辐射框架重述）"),
    ("scripts/paperX_channel_quantum_yield.py", "光子拓扑 §6.6.1 通道判据定量化（量子产率框架，5/5）：辐射/无辐射跳变双通道统一为 Phi=A_eff/(A_eff+k_nr)——S1 基本框架（Phi(0)=1 全辐射、Phi(1)=0.5 产率 50%、r=100→0.0099 无辐射主导）+ S2 通道 A 取向门关闭（电偶极禁戒 A_elec=0→Phi=0；双光子弱通道 2s→1s Phi=5e-9≈0，paper44 定义 2.4）+ S3 通道 B 静默屏障抑制（A_eff=A*(1/15)^N，Phi(N) 单调递减：N=3 达 2.95e-3（r=0.1）、N=6 达 8.8e-7，P6 R_supp 定量实现）+ S4 通道 C 竞争性弛豫（r=k_nr/A 扫描：1e-3→Phi≈1、1e3→<1e-3 碰撞淬灭占优）+ S5 与 P6 阈值对照（R_supp(3)=2.96e-4<θ=1e-3；静默抑制 N=3 时 Phi<1%（r=1）——B 类无辐射定量实现）——A/C 类标准物理温和兼容、B 类为框架候选（P6 叠加形式）"),
    ("scripts/paperX_p6_fluorescence_yield.py", "光子拓扑 §6.6.1 B 类静默抑制预言 vs 标准 X 射线荧光产额对照（诚实张力登记/负结果，5/5）：P6 候选体系（K/Kr/Xe/Rn K 壳层空穴 N=n_max-1）的 B 类量子产率预言 Phi=R_supp/(R_supp+r) vs 标准 K 荧光产额 w_K（文献量级 Krause 1979 类）——S1 框架预言 Phi 随 N 单调降（K 2.96e-4 > Xe 1.97e-5 > Rn 1.3e-6）+ S2 标准 w_K 随 Z 单调增（K 0.12 < Kr 0.64 < Xe 0.87 < Rn 0.96）+ S3 方向性对照（端点 K vs Rn：预言降、标准升——方向完全相反）+ S4 张力量化（Phi/w_K 全部低 2 个量级以上：K 低 400 倍至 Rn 低 7e5 倍）+ S5 盲登记排除线（以 K 荧光产额检验则 B 类被排除）+ 适用域限定候选（R_supp 作用域或限光学/价层跃迁，不适用内壳层 X 射线——高能光子穿透外层电子云无阻碍）——负结果/登记项，诚实张力登记"),
    ("scripts/paperX_p2_eta_s3.py", "光子拓扑 §3.9 P2 残差预言与 η_S3 候选族（Z² 扣除，5/5）：回应 CNF 评价'P2 未扣 Z² 类氢标度'——P2 改写为残差预言 ν(Z)=ν_Z²(Z)·[1+η_S3·g(Z)]——S1 Z² 基线（类氢 2p→1s：E=10.2Z² eV、λ·Z² 守恒）+ S2 η_S3 候选族两档（自然档 S4/S4²/S4-d_H/S4-N_Weyl-d_H/S4²-N_Weyl/2 为 O(1e-2~1e-3)；可行档 S4⁴/S4⁵=P6 R_supp(4/5) 为 O(1e-4~1e-6)，同一静默机制衔接）+ S3 选择原理（MDL/结构匹配 → η_a，可测性约束收窄）+ S4 诚实负结果（O(1e-2~1e-3) 自然档残差 ≫ 等电子序列精度 1e-5，已被既有数据排除；存活档 S4⁴/S4⁵）+ S5 盲登记（主候选 η=S4⁵=1.32e-6 五层静默、备选 η=S4⁴=1.98e-5 + 双向排除线）"),
    ("scripts/paperX_p4_fractal_oscillation.py", "光子拓扑 §3.11 P4 参数锁定（分形红移震荡，5/5）：回应 CNF 评价'P4 三参数'——参数结构分析（δz_osc = z_Friedman·[1+A_P4·sin(2π(z−z₀)/Δz_P4+φ)]，三参数→一参数+相位边际化）+ 候选族（A_P4：S4³=2.96e-4 主候选/S4⁴ 备选/S4² 上限——与 η_S3 同源同一静默机制；δ：S4 MDL 最简；Δz_P4=1/d_H≈0.369）+ 可测性检查（巡天统计阈值 σ_z/√N~1e-6：主候选 3e2x 余量可测；相干性假设=最脆弱点，有效振幅=A·f）+ 盲登记（主候选 + 双向排除线）"),
]
