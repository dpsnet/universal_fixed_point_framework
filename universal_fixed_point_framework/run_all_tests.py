...
SCRIPTS = [
    ...
    ("scripts/paperX_shale_p1_lowD_anchor.py", "P1 谱隙-门限压力双曲标度·D→2 低端锚点（4/4 登记确认型）：H1 真实页岩压汞 D 低端存在（青山口组大孔段 D 低至 2.07、均 2.26，世界地质 2026，10.3969/j.issn.1004-5589.2026.01.006——D→2 端非理论空想，P1 发散预言可检验性前提成立）+ H2 Tuscaloosa 样品级 D∈[2.53,3.87] 无 D<2.5 样品——样品级'低 D+对应 P_t'成对数据仍缺 + H3 物理衔接（大孔段 D→2=P1 基准态：均匀孔隙、无分形封堵增强、P_t 发散）+ H4 检验方法学就绪（合成边界 F1a 证实'D→2 发散被抑制'可检出）"),
    ...
    ("scripts/paperX_photon_topology.py", "光子拓扑 62B：方向性阶跃(A4) + 光速不变(定理2.1) + λν 一致(定理3.1) + Bohr 匹配/吸收截面(命题1.4/定义1.4) + 时间解耦(推论4) + 零质量不自洽(命题2.2) + 捕获-再分岔(命题3.2, 开放#3) + 自由传播模方守恒一致性(树级, 开放#6) + 静默-跃迁门控(开放#8) (36/36)"),
