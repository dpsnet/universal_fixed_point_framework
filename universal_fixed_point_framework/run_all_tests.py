...
SCRIPTS = [
    ...
    ("scripts/paperX_photon_first_principle.py", "光子拓扑分岔第一性起源三方向验证（开放问题 §7.5 #1 推进，10/10）：S1 S3 谱静默互补对应（光子 U(1) 零自相互作用顶点 ⟹ σ_S3=0 静默解除可传播 vs 胶子 SU(3) 54 个三顶点谱封闭雅可比 5.6e-17 ⟹ σ_S3=1 静默驻留禁闭——规范玻色子传播性 = S3 静默状态，阿贝尔-非阿贝尔判据）+ S2 Φ⊆D 函子特例（fold∘unfold=id_A 能量守恒 + D 函子律保复合 + Φ=D|_Rec_photon 静默解除对象层一致——拓扑分岔从公理降为 D 函子定理）+ S3 分岔定量化（谱间隙 Δλ_gap(E) 单调减 + 单次离散阶跃无中间拓扑 + Bohr 条件 hν=Δλ_gap=ΔE 谱表示解析精确）——光子拓扑分岔第一性起源获机制验证；诚实边界：顶点调控为机制对照、函子律为有限子范畴验证、谱带为概念模型"),
]
