...
SCRIPTS = [
    ...
    ("scripts/paperX_shale_spectral.py", "页岩油气成藏谱流应用推演 (6 项，5/6 通过 + 2 项诚实负结果登记)：M0 文献锚定压汞分形（[L1] 公式 S_Hg=a·Pc^(2-D)，[L3]-[L5] 维数恢复 4/4）+ M1 真实数据分形分析（USGS Tuscaloosa 31 样品 MICP [L6]，D 中位数 2.862、R² 0.894）+ M2 真实数据深化（可动饱和度-分形维数 ρ=+0.214 弱正相关不显著，负结果登记；多段分形 R² 提升 +0.062，正发现）+ B1 文献量级验证（候选 α=d_f-1 量级偏低，负结果登记）+ B2 超压临界（ν=0.5021）+ B3 突破通道盒计数维数——真实数据已入库 scripts/data/tuscaloosa_micp/（诚实边界）"),
]
