...
SCRIPTS = [
    ...
    ("scripts/paperX_shale_spectral.py", "页岩油气成藏谱流应用推演 (13 项，12/13 通过 + 1 项负结果登记 + 1 子项根因已诊断)：M0 文献锚定压汞分形（[L1] 公式，[L3]-[L5] 维数恢复 4/4）+ M1 真实数据多段分形（USGS Tuscaloosa 31 样品 [L6]，分段 R² 0.894→0.971）+ M2 真实数据深化（可动-分形 ρ=+0.214 弱正不显著，负结果）+ M3 产油页岩文献锚定（长7段 [L2] 排序 ρ_s=-1.00）+ M4 生烃谱流检查（Rock-Eval 示例 5 样品）+ M5 长7段 TOC-生烃潜量线性正相关（10 样品，R²=0.9990，夹层自动识别）+ M6 跨盆地干酪根降解谱流（18 样品：青山口 HI 349<长7段 410）+ M7 Thomeer 双孔隙 HPMI 分形（整体 R²=0.655→两段 0.962）+ M8 B1 修正标定（长7段线性注入 S₁=0.57·TOC-0.24，S₁/TOC∈[0.212,0.583] 与文献锚点重叠）+ M9 长7段生烃谱流诊断（HI-Tmax +0.873，M4 子项3 根因=干酪根类型/质量效应主导单井窗口）+ B1 文献量级验证（α=d_f-1 量级偏低，负结果登记，已被 M8 修正）+ B2 超压临界（ν=0.5021）+ B3 突破通道盒计数——真实数据已入库 scripts/data/（诚实边界）"),
]
