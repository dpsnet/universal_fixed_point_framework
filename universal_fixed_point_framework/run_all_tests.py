...
SCRIPTS = [
    ...
    ("scripts/paperX_functor_extended.py", "光子拓扑 62B 方向2 §3.3 扩展子范畴函子律验证（S1-S5，5/5）：对象映射良定义（hν=ΔE=E_i-E_j>0，全 6 个发射跃迁）/保恒等（fold∘unfold=id_A，发射+吸收闭合）/保复合（核心：ν_ik=ν_im+ν_mk——直接 vs 分步跃迁频率一致性，ΔE 可加⟹频率可加=能量守恒）/全组合复合（4 能级所有多步路径，共 5 条）/扩展子范畴函子律成立——Φ=D|_Rec 多能级特例（§10 第 2 项推进；完整 Rec 无穷维子范畴仍待 Lean 扩展）"),
