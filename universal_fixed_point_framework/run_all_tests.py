...
SCRIPTS = [
    ...
    ("scripts/paperX_shale_spectral.py", "页岩油气成藏谱流应用推演 (7 项，6/7 通过 + 2 项诚实负结果登记)：M0 文献锚定压汞分形（[L1] 公式，[L3]-[L5] 维数恢复 4/4）+ M1 真实数据多段分形（USGS Tuscaloosa 31 样品 [L6]，分段 R² 0.894→0.971，大孔段 D=2.395 物理合理、小孔段高压区 D>3 与 [L1] 高压段分形性差一致）+ M2 真实数据深化（可动-分形 ρ=+0.214 弱正不显著，负结果登记；多段分形正发现 +0.062）+ M3 产油页岩文献锚定（长7段 [L2] 排序 ρ_s=-1.00，与 Tuscaloosa +0.214 相反，可动-分形关系依赖页岩类型）+ B1 文献量级验证（α=d_f-1 量级偏低，负结果登记）+ B2 超压临界（ν=0.5021）+ B3 突破通道盒计数——真实数据已入库 scripts/data/tuscaloosa_micp/（诚实边界）"),
]
