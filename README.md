...
| Paper XLIV（光子生成的拓扑转变机制与可证伪预言） | 1 | v0.40：拓扑转变公理体系 + 方向性阶跃 + 双层正交 + 六项远期可证伪预言 P1–P6（均尚未实验验证）；正交性三路径推导 + 摘要新增 + 笔记引用清除 + 法拉第笼类比升级为结构同构声明 |
| Paper XLV（耗散流体有效场论的谱语言翻译） | 1 | ✅ v1.3（2026-08-24）：CGL 核心结构（CTP/r-a/KMS/BRST）谱语言重构 + 三类新构造 + 可证伪预言，数值 11/11 |
...
| Paper XLVII（仿形感应理论） | 1 | 🆕 v0.3（2026-08-31）：电磁感应 = 正交分量运动模式的仿形拟合，公理 M1–M4 + 五推论谱系；工程术语纯结构占位符声明；Lean 形式化零 sorry |
| Paper XLVIII（拓扑禁戒频率） | 1 | 🆕 v0.5（2026-08-31）：谱类型不匹配 = 第三约束维度，Harper/强磁场氢原子/Rydberg 四系统数值验证 + SDSS 白矮星观测验证 $p=0.00786$ + 四类显性证伪检验 + 3 颗反常发射线星 + NMLO 算子（$m=3.39$，$R^2=0.80$）+ NLCT 非局域相关时间标度方程（$\beta=2+D$，$\tau_{\text{NL}}=D/(1+D)\,T$） |
