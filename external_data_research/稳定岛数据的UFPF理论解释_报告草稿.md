# 稳定岛数据的 UFPF 跨框架理论解释——报告草稿 v1.1（同步正式版 v1.1 数据归属重大修正）

> **文档编号**：UFPF-EXT-EDRN-STABLEISLAND-01
> **版本**：v1.1（草稿，同步正式版 v1.1 数据归属重大修正）
> **日期**：2026-08-16
> **编号结构**：`UFPF` = 解释框架归属 ｜ `EXT` = 外部数据跨框架解释 ｜ `EDRN` = 数据来源（李广好 EDRN 项目） ｜ `STABLEISLAND` = 数据主题 ｜ `01` = 系列序号

> **【修订说明（v1.0→v1.1）】**：本版相对 v1.0 草稿执行数据归属重大修正。依据 EDRN 项目 README 确认：EDRN 理论框架提出者、仓库维护者、第一作者为**李广好**（邮箱 448798112@qq.com，GitHub 仓库 luoxuejian000/edrn-dmrg-verification，Apache-2.0 许可），负责全部脚本设计、CSV 生成、形式化协议与论文 PDF 创作；**于见隐**仅为知乎专栏原文《我们在量子世界里，发现了一座神秘的"稳定岛"》的作者。v1.0 草稿将数据/脚本/代码/协议/论文/仓库创作者错误归属为于见隐，本版一律修正为李广好，并将"双轨创作声明"升级为"三轨创作声明"（第一层A=李广好EDRN框架与代码仓库、第一层B=于见隐知乎定性意象、第二层=王斌UFPF跨框架解释）。凡指代数据/脚本/CSV/代码/协议/论文/仓库创作者处均改署李广好；凡指代知乎专栏原文/定性意象处保留于见隐。
